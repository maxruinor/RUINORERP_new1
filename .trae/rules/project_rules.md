...
# 代码创建规则
windows.Form的窗体 要符合vs IDE的 规范， 包括命名空间，类名， 注释， 缩进， 格式等。定义要在设计器文件中， 代码要在代码文件中。定义，实例化，加入顺序要符合vs IDE的规范。
WPF的窗体 要注意WPF的特点，也要符合vs IDE的 规范， 包括命名空间，类名， 注释， 缩进， 格式等。定义要在xaml文件中， 代码要在代码文件中。定义，实例化，加入顺序要符合vs IDE的规范。
## 类创建检查
...
3. 查看相关文件夹结构
4.不满足搜索要求时禁止创建新类
5.各个实体方法属性。优先使用存在的， 没有就要声明定义好再使用。 