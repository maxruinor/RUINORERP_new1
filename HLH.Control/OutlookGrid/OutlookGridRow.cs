...
namespace OutlookStyleControls
{
    ...
    public class OutlookGridRow : DataGridViewRow
    {
        ...
        protected override void Paint(System.Drawing.Graphics graphics, System.Drawing.Rectangle clipBounds, System.Drawing.Rectangle rowBounds, int rowIndex, DataGridViewElementStates rowState, bool isFirstDisplayedRow, bool isLastVisibleRow)
        {
            if (this.isGroupRow)
            {
                ...
                graphics.DrawString(group.Text, grid.Font, Brushes.Black, rowHeadersWidth - grid.HorizontalScrollingOffset + 20, rowBounds.Bottom - 18);

                if (group.GroupListShowText != null)
                {
                    string[] MyArray = new string[2];
                    foreach (String MyString in group.GroupListShowText)
                    {
                        MyArray = MyString.Split(',');
                        //Ëã¿í¶È
                        int MyWith = 0;
                        for (int i = 0; i < grid.Columns[MyArray[0]].Index - 1; i++)
                        {
                            MyWith = MyWith + grid.Columns[i].Width;
                        }


                        graphics.DrawString(MyArray[1].ToString(), grid.Font, Brushes.Black, MyWith - grid.HorizontalScrollingOffset + 100, rowBounds.Bottom - 18);
                    }

                }

                //  graphics.DrawString("»ã×Ü", grid.Font, Brushes.Black, rowHeadersWidth - grid.HorizontalScrollingOffset + 230, rowBounds.Bottom - 18);
                //draw bottom line
    ...
    internal class OutlookGridRowComparer : IComparer
    {
        ...
        public int Compare(object x, object y)
        {
            ...
            if (obj1.Cells[this.columnIndex].Value.ToString() == "" || obj2.Cells[this.columnIndex].Value.ToString() == "")
            {
            ...
            switch (obj1.Cells[this.columnIndex].ValueType.FullName)
            {
                ...
                case "System.Int32":
                    {
                        return decimal.Compare(Convert.ToDecimal(obj1.Cells[this.columnIndex].Value), Convert.ToDecimal(obj2.Cells[this.columnIndex].Value)) * (direction == ListSortDirection.Ascending ? 1 : -1);
                ...
                case "System.DateTime":
                    {
                        return DateTime.Compare(Convert.ToDateTime(obj1.Cells[this.columnIndex].Value), Convert.ToDateTime(obj2.Cells[this.columnIndex].Value)) * (direction == ListSortDirection.Ascending ? 1 : -1);
