namespace HLH.Lib.Net
{
    ...
    public class RequestInfo
    {
        ...
        public RequestInfo(string url, CookieContainer cookiecontaniner)
        {
            Cookiecontaniner = cookiecontaniner;
            Url = url;
            AllowAutoRedirect = true;
        }
        private CookieContainer cookiecontaniner = new CookieContainer();

        public CookieContainer Cookiecontaniner
        {
            get { return cookiecontaniner; }
            set { cookiecontaniner = value; }
        }
    ...
    public class RequestHttpWebRequest
    {
        ...
        private void InitWebRequest(RequestInfo info, Action<ResponseInfo> act, out HttpWebRequest webRequest, out StateObject state)
        {
            ...
            if (info.Cookiecontaniner.Count > 0)
            {
                webRequest.CookieContainer = info.Cookiecontaniner;
            }
