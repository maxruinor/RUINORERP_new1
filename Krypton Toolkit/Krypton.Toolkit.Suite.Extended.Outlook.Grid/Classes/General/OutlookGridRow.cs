    ...
    public class OutlookGridRow : DataGridViewRow
    {
        ...
        protected override void Paint(Graphics graphics, Rectangle clipBounds, Rectangle rowBounds, int rowIndex, DataGridViewElementStates rowState, bool isFirstDisplayedRow, bool isLastVisibleRow)
        {
            if (_isGroupRow)
            {
                ...
                string groupText = _group.Text;

                //处理统计加总小计
                if (grid.SubtotalColumns != null && grid.SubtotalColumns.Count > 0 && _group.Rows.Count > 0)
                {
                    string tempGroupTotalText = string.Empty;
                    foreach (var item in grid.SubtotalColumns)
                    {
                        //tempGroupTotalText+=
                        if (grid.Columns.Contains(item))
                        {
                            DataGridViewColumn dc = grid.Columns[item];
                            //string DataTypeName = _group.Rows[0].Cells[item].Value.GetType().Name;
                            Decimal sum = 0;
                            for (global::System.Int32 i = 0; i < _group.Rows.Count; i++)
                            {
                                if (_group.Rows[i].Cells[item].Value != null)
                                {
                                    sum += Decimal.Parse(_group.Rows[i].Cells[item].Value.ToString());
                                }
                            }
                            tempGroupTotalText += dc.HeaderText + ":" + sum.ToString() + ",";
                        }


                    }
                    tempGroupTotalText = tempGroupTotalText.TrimEnd(',');
                    groupText = _group.Text + $"【{tempGroupTotalText}】";
                    #region 如果有统计列
