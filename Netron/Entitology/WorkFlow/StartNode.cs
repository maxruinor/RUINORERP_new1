...
namespace Netron.GraphLib.Entitology
{
    ...
    [NetronGraphShape("开始", "8ED1469D-90B2-43ab-B000-4FF5C682F531", "Workflow", "Netron.GraphLib.Entitology.StartNode",
         "流程开始的节点图形")]
    ...
    public class JsonPropertyContractResolver : DefaultContractResolver
    {
        ...
        protected override IList<JsonProperty> CreateProperties(Type type, MemberSerialization memberSerialization)
        {
            IList<JsonProperty> props = base.CreateProperties(type, memberSerialization).ToList().FindAll(p => lstInclude.Contains(p.PropertyName));//需要输出的属性  } }
            return props;
        }
    }
