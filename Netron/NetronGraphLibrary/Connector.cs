...
namespace Netron.GraphLib
{
    ...
    public class Connector : Entity, ISerializable
    {
        ...
        public bool AllowMultipleConnections
        {
            ...
            [Browsable(false)]
            set { mAllowMultipleConnections = value; }
        }
        ...
        /// <summary>
        /// 获取/设置连接器所属的父对象（与BelongsTo属性相同）
        /// </summary>
        public Shape Parent
        {
            get
            {
                return mBelongsTo; // 复用已有的mBelongsTo字段
            }
            [Browsable(false)]
            set
            {
                mBelongsTo = value; // 复用已有的mBelongsTo字段
            }
        }
        ...
        public Connector(string uid) : base()
        {
        ...
        public override void Invalidate()
        {
            ...
            try
            {
                ...
                // 简化但不刷新整个画布，避免循环调用
                // 使用InvalidateRectangle方法而不是Invalidate方法
                try
                {
                    // 使用一个合理的默认区域，但不直接调用可能触发Shape重新Invalidate的方法
