...
namespace Netron.GraphLib.UI
{
    ...
    {
        /// <summary>
        /// 定义一个属性来保存流程的数据，一个流程图对应一个流程数据WorkFlowConfigData
        /// </summary>
        ...
        /// <summary>
        /// 具有当前鼠标焦点的实体。由HitHover和HitEntity处理程序自动设置
        /// </summary>
                ...
                if ((e.Button == MouseButtons.Left) && (e.Clicks == 2))
                {
                    ...
                    if ((Hover != null) && (typeof(Entity).IsInstanceOfType(Hover)))
                    {
                        if (OnSetNodeProperties != null)
                        {
                            ...
                            {
                                MessageBox.Show("请设置当前连接线的NodeStepPropertyValue属性");
                            }
                            else
                            {
                                OnSetNodeProperties(Hover, Hover.NodeStepPropertyValue);
                                Update();
                            }
                        }
                    }
                    ...
                    //节点双击显示属性 节点和线一样的代码，这里不重复，所以注释掉
                    /*
                    if ((Hover != null) && Hover.GetType().BaseType.Name == "BaseNode")
                    {
                        ...
                        if (OnSetNodeProperties != null)
                        {
                            if (Hover.NodeStepPropertyValue == null)
                            {
                                MessageBox.Show("请设置当前节点的NodeStepPropertyValue属性");
                            }
                            else
                            {
                                OnSetNodeProperties(Hover, Hover.NodeStepPropertyValue);
                ...
                if (e.Button == MouseButtons.Right)
                {
                    ...
                    if (this.mEnableContextMenu)
                    {
                        ...
                        if (typeof(Shape).IsInstanceOfType(Hover))
                        {
                            ...
                            //如果选中的是节点则添加节点设置属性菜单项
                            // this.ContextMenu.MenuItems.Add("-");
                            //this.ContextMenu.MenuItems.Add(new MenuItem("设置节点属性", new EventHandler(OnShowNodesProperties)));
                        ...
                        else if (typeof(Connection).IsInstanceOfType(Hover))
                        {
                            ...
                            MenuItem[] subconnection = new MenuItem[2]{
                                                                          new MenuItem("添加点",new EventHandler(AddConnectionPoint)),
                                                                          new MenuItem("删除点",new EventHandler(RemoveConnectionPoint))
                                                                      };


                            this.ContextMenu.MenuItems.Add(new MenuItem("连接线", subconnection));
                            //如果选中的线条有一个点是指向判断节点，则显示条件设置菜单项
                            if ((Hover as Connection).From.BelongsTo.GetType().Name == "DecisionShape")
                            {
                                this.ContextMenu.MenuItems.Add("-");
                                this.ContextMenu.MenuItems.Add(new MenuItem("设置条件", new EventHandler(OnShowSetCondition)));
                            }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void OnShowSetCondition(object sender, EventArgs e)
        {
                        ...
                        if (!Hover.Equals(connection.From) && (Hover as Connector).AllowNewConnectionsTo)
                        {
                            ...
                            if (((Connector)Hover).AllowMultipleConnections || ((Connector)Hover).Connections.Count == 0)
                            {
                                ...
                                if (OnConnectionAdded != null)
                                {
                                    ...
                                    {
                                        connection.Delete(); //如果（外部）处理程序告诉不正常，我们将再次删除连接
                                    }
                                }
        ...
        private void LoadSelfLibrary()
        {
            ...
            //summ = new ShapeSummary(libPath, "8ED1469D-90B2-43ab-B000-4FF5C682F531", "开始", "Basic shapes", "Netron.GraphLib.BasicShapes.StartNode", "流程开始的节点");
            //lib.ShapeSummaries.Add(summ);
            //AddToContextMenu(summ);
			...
			*/
            ...
            else
            {
                try
                {
                    ...
                    {
                        if (item.ToString() == "RUINORERP.UI.WorkFlowDesigner.Nodes.Start")
                        {
                            ExistStart = true;
                            continue;
                        }
                    ...
                    {
                        MessageBox.Show("一个流程只能有一个开始节点.", "错误", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        return;
                    }
                }
                catch
                {
                    MessageBox.Show("找不到要创建的有效形状数据.", "无数据错误", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
                try
                {
                ...
                {
                    MessageBox.Show("找不到图形数据. \n (你添加了必要的图形元素库吗?)", "无数据错误", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
        ...
        /// Common gate for a delete action on the plex. Deletes the selected shapes and goes via the history (of course).
        /// </summary>		
        protected void OnTestWorkflow(object sender, EventArgs e)
        {
            try
            {
                if (OnTestWorkflowAction != null)
                {
                    OnTestWorkflowAction(sender, e);
                }

            }
            catch (Exception exc)
            {
                Trace.WriteLine(exc.Message, "GraphControl.OnToJson");
            }
        ...
        /// Common gate for a delete action on the plex. Deletes the selected shapes and goes via the history (of course).
        /// </summary>		
        protected void OnToJson(object sender, EventArgs e)
        {
            try
            {
                if (OnToJsonAction != null)
                {
                    OnToJsonAction(sender, e);
                }
        ...
        protected void AddBaseMenu()
        {
            ...
            graphItem.MenuItems.Add("新建", new EventHandler(OnNewGraph));
            graphItem.MenuItems.Add("全选", new EventHandler(OnSelectAll));
			...
			*/
            mContextMenu.MenuItems.Add(new MenuItem("&删除", new EventHandler(OnDelete)));
            mContextMenu.MenuItems.Add("-");//add a separation
            mContextMenu.MenuItems.Add(new MenuItem("转换为&Json", new EventHandler(OnToJson)));
            mContextMenu.MenuItems.Add("-");//add a separation
            mContextMenu.MenuItems.Add(new MenuItem("测试工作流", new EventHandler(OnTestWorkflow)));
            mContextMenu.MenuItems.Add("-");//add a separation

            mContextMenu.MenuItems.Add(new MenuItem("属性", new EventHandler(OnProperties)));
        }
                ...
                for (int k = 0; k < tps.Length; k++) //loop over modules in assembly
                {
                    ...
                    try
                    {
                        ...
                        if (objs.Length >= 1)
                        {
                            ...
                            {
                                //TODO:by watson 这里注释就不会显示很多的右键菜单了
        ...
        {
            if (OnInfo != null)
                OnInfo(obj, OutputInfoLevels.Default);
        }
        /// <summary>
        /// Outputs info to the outside world where it can be displayed or 
        ...
        public Shape AddBasicShape(BasicShapeType shapeType, string nodeLabel, PointF position)
        {
            ...
            switch (shapeType)
            {
                ...
                case BasicShapeType.StartNode: key = "8ED1469D-90B2-43ab-B000-4FF5C682F531"; break;
            }
        ...
        protected virtual void GetPropertyBagValue(object sender, PropertySpecEventArgs e)
        {
            switch (e.Property.Name)
            {
                ...
                case "ShowAutomataController":
                    e.Value = this.ShowAutomataController; break;
                    break;
            }
        }
