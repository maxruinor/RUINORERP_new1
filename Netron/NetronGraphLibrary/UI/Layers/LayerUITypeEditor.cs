    ...
    public class LayerUITypeEditor : UITypeEditor
    {
        ...
        public override void PaintValue(PaintValueEventArgs args)
        {
            ...
            try
            {
                ...
                //获取图层的颜色需要长时间的深入访问。。。
                if (((PropertyBag)args.Context.Instance).Owner is Shape)
                {
                    args.Graphics.FillRectangle(new SolidBrush(((Shape)((PropertyBag)args.Context.Instance).Owner).Layer.LayerColor), r);
                }
                if (((PropertyBag)args.Context.Instance).Owner is Connection)
                {
                    args.Graphics.FillRectangle(new SolidBrush(((Connection)((PropertyBag)args.Context.Instance).Owner).Layer.LayerColor), r);
                }
                //args.Graphics.FillRectangle(new SolidBrush(((Shape)((PropertyBag)args.Context.Instance).Owner).Layer.LayerColor), r);
