    ...
    public class PropertyBag : ICustomTypeDescriptor
    {
        ...
        public override string ToString()
        {
            ...
            foreach (PropertySpec spec in this.Properties)
            {
                ...
                try
                {
                    var pro = this.owner.GetType().GetProperty(spec.Name);
                    if (pro != null)
                    {
                        var strValue = pro.GetValue(this.owner, null);
                        if (strValue != null)
                        {
                            sb.Append(strValue.ToString());
                        }
                ...
                catch
                {
					...
					/*propertybag的理念是对类进行屏蔽，因此命名
					*不一定反映真实的类成员，或者可以定义某个包属性
					*作为类成员的组合。一般来说，没有转换，而且事实上
					*真正与属性绑定使其难以捕捉实际值
					*/
