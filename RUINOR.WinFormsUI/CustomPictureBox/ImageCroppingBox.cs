...
namespace RUINOR.WinFormsUI.CustomPictureBox
{
    public class ImageCroppingBox : Control
    {
        ...
        /// 当前处理图片的原始文件路径（包括名称）
        /// </summary>
        public string fileOldNamewithPath { get; set; }
        /// <summary>
        /// 当前处理图片的原始文件路径（包括名称）
        /// </summary>
        public string fileNewNamewithPath { get; set; }
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            if (this._Image != null)
            {
                //Bitmap NewPic = new Bitmap(this._Image, GetNewSize(this.ClientRectangle.Width, this.ClientRectangle.Height, _Image.Width, _Image.Height));
                //this._Image = NewPic;
                //g.DrawImage(NewPic, Point.Empty);//原图
        ...
        private Size GetNewSize(int maxWidth, int maxHeight, int width, int height)
        {
            ...
            else if ((sw / sh) > (mw / mh))
            {
                ...
                h = (w * sh) / sw;
                ScalingRatio = width / w;
            }
            else
            {
                h = maxHeight;
                w = (h * sw) / sh;
                ScalingRatio = height / h;
            }
        ...
        protected virtual void DrawMagnifier(Graphics g, Point pt)
        {
            ...
            Image bmpNew = ImageCroppingHelper.ZoomImage(bmpSrc, 7F);//将图放大7倍
            //放大镜区域
        ...
        public virtual Image GetSelectedImageNoZoom()
        {
            ...
            double dw = Convert.ToDouble(this._SelectedRectangle.Width) * ScalingRatio;
            ...
            newPoint.X = Convert.ToInt32(xx);
            newPoint.Y = Convert.ToInt32(yy);
            using (Graphics g = Graphics.FromImage(img))
            {
                g.DrawImage(this.originalImage,
                    new Rectangle(Point.Empty, img.Size),
                    new Rectangle(newPoint, img.Size),
                    GraphicsUnit.Pixel);
            }
            return img;
        }
...
public class ImageCroppingHelper
{
    ...
    public static Image ZoomImage(Image image, float scale)
    {
        ...
        if (scale <= 0)
            throw new ArgumentException("刻度必须大于零");
