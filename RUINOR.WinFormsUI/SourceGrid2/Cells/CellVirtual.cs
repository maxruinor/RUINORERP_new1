...
namespace SourceGrid2.Cells.Virtual
{
	...
	public abstract class CellVirtual : ICellVirtual
	{
		...
		public abstract object GetTagValue(Position p_Position);
		...
		public abstract void SetTagValue(Position p_Position, object p_TagValue);
		...
		public virtual string GetDisplayText(Position p_Position)
		{
			try
			{
				...
				if (l_TmpVal is double)
				{
					return Convert.ToString(Math.Round((double)l_TmpVal,this.DecLen));
				}
				if (m_CellModel==null)
				{
					if (l_TmpVal==null)
		...
		public virtual void OnDoubleClick (PositionEventArgs e)
		{
			...
			if (this.DoubleClick!=null) this.DoubleClick(this,EventArgs.Empty);
		}
		...
		/// <param name="e"></param>
		/// 
		public void CheckArrowClick()
		{
			if ( this is Cell &&this.Arrow)
			{
				Cell cc=(Cell)this;
				
			   string s=Convert.ToString(cc.Value);
			   Point p=Control.MousePosition;
				p=cc.Grid.PointToClient(p);
				Rectangle rr=cc.DisplayRectangle;
				if (p.X-rr.Left<12 && (s!=null && s!=""))
				{
					//进行编辑
					if( cc.Grid!=null) cc.Grid.InvokeCellArrowClick(this,EventArgs.Empty);
					return;
		...
		/// </summary>
		/// <param name="e"></param>
		public virtual void OnTagValueChanged(PositionEventArgs e)
		{
			for (int i = 0; i < m_BehaviorModels.Count; i++)
				m_BehaviorModels[i].OnTagValueChanged(e);
		}
		...
		public virtual void OnEditEnded(PositionCancelEventArgs e)
		{
			...
			if (this.Grid!=null) this.Grid.InvokeCellEditEnd(this,EventArgs.Empty);
		}
		#endregion
