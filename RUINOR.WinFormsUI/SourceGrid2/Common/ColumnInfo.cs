...
namespace SourceGrid2
{
    ...
    public class ColumnInfo
    {
        ...
        private ColumnInfo(GridVirtual p_Grid, string columnName)
        {
            m_Grid = p_Grid;
            ...
            if (Grid.ColumnsCount > Index)
            {
                for (int r = 0; r < Grid.RowsCount; r++)
                {
                    ...
                    if (l_CellVirtual != null && l_CellVirtual.CanReceiveFocus)
                        return Grid.SetFocusCell(l_NewFocus);
                }
        ...
        public class ColumnInfoCollection : ICollection
        {
            ...
            public void Insert(int p_Index, List<string> ColName)
            {
                InsertRange(p_Index, ColName);
            }
            ...
            /// <param name="p_StartIndex"></param>
            /// <param name="p_Count"></param>
            public void InsertRange(int p_StartIndex, List<string> ColNames)
            {
                if (IsValidRangeForInsert(p_StartIndex, ColNames.Count) == false)
                    throw new SourceGridException("Invalid index");

                //TODO si potrebbe ottimizzare aumentando la capacity
                for (int c = 0; c < ColNames.Count; c++)
                {
                    m_List.Insert(p_StartIndex + c, new ColumnInfo(m_Grid, ColNames[0]));
                }

                if (AutoCalculateLeft)
                    CalculateLeft(p_StartIndex);

                OnColumnsAdded(new IndexRangeEventArgs(p_StartIndex, ColNames.Count));
            }
