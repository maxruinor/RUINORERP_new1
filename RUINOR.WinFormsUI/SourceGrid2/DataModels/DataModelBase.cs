...
namespace SourceGrid2.DataModels
{
    ...
    public class DataModelBase : SourceLibrary.ComponentModel.Validator.ValidatorTypeConverter, IDataModel
    {
        ...
        public virtual object GetEditedTagValue()
        {
            throw new SourceGridException("No valid cell editor found");
        }
        ...
        /// <param name="p_Cell"></param>
        /// <param name="p_Position"></param>
        /// <param name="p_NewTagValue"></param>
        /// <returns></returns>
        public virtual bool SetCellTagValue(Cells.ICellVirtual p_Cell, Position p_Position, object p_NewTagValue)
        {
            if (EnableEdit)
            {
                ValidatingTagValueEventArgs l_cancelEvent = new ValidatingTagValueEventArgs(p_Cell, p_NewTagValue);
                ////开始验证值了
                OnValidatingForTagValue(l_cancelEvent);
                //check if cancel == true 
                if (l_cancelEvent.Cancel == false)
                {
                    object l_PrevTagValue = p_Cell.GetTagValue(p_Position);
                    try
                    {
                        p_Cell.SetTagValue(p_Position, l_cancelEvent.NewValue);

                        //开始验证值了 
                        OnValidatedForTagValue(new CellEventArgs(p_Cell));

                    }
                    catch (Exception ex)
                    {
                        Console.Write(ex.Message);
                        p_Cell.SetTagValue(p_Position, l_PrevTagValue);
                        //throw err;
                        l_cancelEvent.Cancel = true;//di fatto ?fallita la validazione del dato
                    }
                }
        ...
        protected virtual void OnValidatedForTagValue(CellEventArgs e)
        {
            if (m_ValidatedForTagValue != null)
                m_ValidatedForTagValue(this, e);
        }

        protected virtual void OnValidatingForTagValue(ValidatingTagValueEventArgs e)
        {
            if (m_ValidatingForTagValue != null)
                m_ValidatingForTagValue(this, e);
        }


        private event ValidatingCellEventHandler m_Validating;
        private event CellEventHandler m_Validated;
        private event ValidatingTagValueEventHandler m_ValidatingForTagValue;
        private event CellEventHandler m_ValidatedForTagValue;


        public event CellEventHandler ValidatedForTagValue
        {
            add { m_ValidatedForTagValue += value; }
            remove { m_ValidatedForTagValue -= value; }
        }
        ...
        /// Validating event
        /// </summary>
        public event ValidatingTagValueEventHandler ValidatingForTagValue
        {
            add { m_ValidatingForTagValue += value; }
            remove { m_ValidatingForTagValue -= value; }
        }
        ...
        public virtual object GetEditedKey()
        {
            return null;
        }
    }
