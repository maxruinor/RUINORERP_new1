	...
	public abstract class EditorControlBase : DataModelBase
	{
		...
		public override bool InternalApplyEdit()
		{
			if (IsEditing == true && EnableEdit == true)
			{
				...
				try
				{
					EditCell.Key=this.GetEditedKey();
					SetCellTagValue(EditCell, EditPosition, GetEditedTagValue());
		...
		public override bool InternalEndEdit(bool p_Cancel)
		{
			if (IsEditing)
			{
				...
				if (l_bSuccess)
				{
					if (EditCell == null)
					{
						System.Diagnostics.Debug.Assert(true); //non dovrebbe succedere ma in debug un paio di volte ?capitato ma non sono riuscito a riprodurlo
					}
					else
					{
