...
namespace RUINORERP.Business.AutoMapper
{
    ...
    /// </summary>
    public class CustomProfileAll : Profile
    {
        /// <summary>
        /// 配置构造函数，用来创建关系映射
        /// </summary>
        public CustomProfileAll()
        {
           ...
           // CreateMap<tb_PurOrder, tb_PurEntry>();
                ...
                //context => (context.GetType().GetProperties().ForEach(c => c.DeclaringType is Class && !context.IsSourceValueNull)
                // && !context.GetType().Equals(Activator.CreateInstance(context.GetType()))
                //)));

            //    .ForAllMembers(opt => opt.Condition(
            //    context => (context.GetType().GetProperties.Employee_ID.IsClass && !context.IsSourceValueNull)
