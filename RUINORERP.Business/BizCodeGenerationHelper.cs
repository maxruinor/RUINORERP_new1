...
namespace RUINORERP.Business
{
    ...
    public class BizCodeGenerator
    {
        ...
        public string GetBizBillNo(BizType bt)
        {
            ...
            {
                case BizType.销售订单:
                    rule = "{S:SO}{D:yyMMdd}{redis:{S:SO}{D:yyMM}/000}";
                    break;
                case BizType.销售出库单:
                    rule = "{S:SOD}{Hex:yyMM}{redis:{S:SOD}{D:yyMM}/0000}";
                    break;
                case BizType.销售退回单:
                    rule = "{S:SODR}{D:yyMMdd}{redis:{S:SODR}{D:yyMM}/000}";
                    break;
                case BizType.采购订单:
                    rule = "{S:PO}{Hex:yyMMdd}{redis:{S:PO}{D:yyMM}/000}";
                    break;
                case BizType.采购入库单:
                ...
                case BizType.BOM物料清单://标准配方  BS， 订单配方 BO
                    rule = "{S:BS}{D:yyMMdd}{redis:{S:BS}{D:yyMM}/000}";
                    break;
                case BizType.其他费用支出:
                    rule = "{S:EXP}{D:yyMMdd}{redis:{S:EXP}{D:yyMM}/000}";
                    break;
                case BizType.其他费用收入:
                    rule = "{S:INC}{D:yyMMdd}{redis:{S:INC}{D:yyMM}/000}";
                    break;
                case BizType.费用报销单:
                    rule = "{S:EC}{D:yyMMdd}{redis:{S:EC}{D:yyMM}/000}";
                    break;
                case BizType.生产计划单:
                    rule = "{S:PP}{D:yyMMdd}{redis:{S:PP}{D:yyMM}/00}";
                    break;
                case BizType.生产需求分析:
                    rule = "{S:PD}{D:yyMMdd}{redis:{S:PD}{D:yyMM}/00}";
                    break;
                case BizType.制令单:
                    rule = "{S:MO}{D:yyMMdd}{redis:{S:MO}{D:yyMM}/00}";
                    break;
                case BizType.请购单:
                    rule = "{S:RP}{D:yyMMdd}{redis:{S:RP}{D:yyMM}/000}";
                    break;
                case BizType.生产领料单://Production Requisition Document
                    rule = "{S:PRD}{D:yyMMdd}{redis:{S:PRD}{D:yyMM}/000}";
                    break;
                case BizType.生产退料单://Production Requisition Document
                    rule = "{S:PRR}{D:yyMMdd}{redis:{S:PRR}{D:yyMM}/000}";
                    break;
                case BizType.缴库单://Production Requisition Document
                    rule = "{S:PR}{D:yyMMdd}{redis:{S:PR}{D:yyMM}/000}";
                    break;
                case BizType.产品分割单://Production Requisition Document
                    rule = "{S:PS}{D:yyMMdd}{redis:{S:PS}{D:yyMM}/00}";
                    break;
                case BizType.产品组合单://Production Requisition Document
                    rule = "{S:PM}{D:yyMMdd}{redis:{S:PM}{D:yyMM}/00}";
                    break;
                case BizType.借出单://Production Requisition Document
                    rule = "{S:JC}{D:yyMMdd}{redis:{S:JC}{D:yyMM}/000}";
                    break;
