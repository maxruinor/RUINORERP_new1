...
namespace RUINORERP.Business.CommService
{
    ...
    /*
    public class EntityAwareStatusMachine : StatusMachine
    {
        private BaseEntity _entity;

        public EntityAwareStatusMachine(
            BaseEntity entity,
            IWorkflowNotificationService notificationService)
            : base(entity.DataStatus, entity.ApprovalStatus, entity.ApprovalResult, notificationService)
        {
            _entity = entity;
        ...
        private void SubscribeToEntityChanges()
        {
            _entity.PropertyChanged += (s, e) =>
            {
                ...
                {
                    case nameof(DataStatus):
                        if (CurrentDataStatus != _entity.DataStatus)
                        {
                            CurrentDataStatus = _entity.DataStatus;
                            OnStatusChange();
                        }
                        break;

                    case nameof(ApprovalStatus):
                        if (ApprovalStatus != _entity.ApprovalStatus)
                        {
                    ...
                    case nameof(ApprovalResult):
                        ApprovalResult = _entity.ApprovalResult;
                        break;
                }
            };
        }
        ...
        {
            // 同步到实体
            _entity.DataStatus = CurrentDataStatus;
            _entity.ApprovalStatus = ApprovalStatus;
            _entity.ApprovalResult = ApprovalResult;
    ...
    // 在状态机中添加多级审批状态
    public class MultiLevelStatusMachine : BusinessStatusMachine
    {
        private int _approvalLevel;

        public MultiLevelStatusMachine(
            DataStatus initialDataStatus,
            bool initialApprovalResult
           )
            : base(initialDataStatus, initialApprovalResult)
        {
        }
        ...
        public void Approve(int level)
        {
            ...
            {
                // 第二级审批逻辑
                if (CurrentDataStatus == DataStatus.确认 && ApprovalStatus == ApprovalStatus.已审核)
                {
