...
namespace RUINORERP.Business.Processor
{
    ...
    public class QueryField
    {
        ...
        /// </summary>
        public string FriendlyFieldNameFromRelated { get; set; }

        /// <summary>
        /// 为了显示ID对应的其它列，要同时存在于引用表中
        /// 替换的字段，有时查一个单据时，是按编号来查的，比方在销售出库单中。查询条件中包含销售订单数据，保存了订单ID和编号，但是编号并不是和原始订单表中的单号相同，还要指向一次到最终的表中的单号。要么这里处理。要么所有引用相关表 除了ID还要编号姓名等，还要两个表中字段相同，不然在databindhelper引用式生成取值时会出错
        /// </summary>
