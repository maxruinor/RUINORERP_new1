...
namespace RUINORERP.Business
{
    ...
    public partial class tb_BizTypeValidator:BaseValidatorGeneric<tb_BizType>
    {
     ...
     public tb_BizTypeValidator(ApplicationContext appContext = null) : base(appContext)
     {
        
 
        
     
 RuleFor(tb_BizType =>tb_BizType.TypeName).MaximumMixedLength(50).WithMessage("类型名称:不能超过最大长度,50.");
 RuleFor(tb_BizType =>tb_BizType.TypeName).NotEmpty().WithMessage("类型名称:不能为空。");

 RuleFor(tb_BizType =>tb_BizType.TypeDesc).MaximumMixedLength(100).WithMessage("描述:不能超过最大长度,100.");
