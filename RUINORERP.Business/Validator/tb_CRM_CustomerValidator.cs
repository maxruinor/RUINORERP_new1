...
{
    /// <summary>
    /// 机会客户，公海客户 CRM系统中使用，给成交客户作外键引用验证类
    /// </summary>
    ...
    {
     public tb_CRM_CustomerValidator() 
     {
 ...
 RuleFor(tb_CRM_Customer =>tb_CRM_Customer.CustomerStatus).NotEmpty().When(x => x.CustomerStatus.HasValue);
 RuleFor(tb_CRM_Customer =>tb_CRM_Customer.GetCustomerSource).MaximumLength(125).WithMessage("获客来源:不能超过最大长度,125.");
