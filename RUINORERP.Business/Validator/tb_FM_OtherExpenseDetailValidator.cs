...
namespace RUINORERP.Business
{
    ...
    public partial class tb_FM_OtherExpenseDetailValidator:AbstractValidator<tb_FM_OtherExpenseDetail>
    {
     ...
     {
     //***** 
 RuleFor(tb_FM_OtherExpenseDetail =>tb_FM_OtherExpenseDetail.ExpenseMainID).NotNull().WithMessage("费用:不能为空。");
 RuleFor(tb_FM_OtherExpenseDetail =>tb_FM_OtherExpenseDetail.ExpenseName).MaximumLength(300).WithMessage("事由:不能超过最大长度,300.");
 RuleFor(tb_FM_OtherExpenseDetail =>tb_FM_OtherExpenseDetail.ExpenseName).NotEmpty().WithMessage("事由:不能为空。");
 RuleFor(tb_FM_OtherExpenseDetail =>tb_FM_OtherExpenseDetail.Employee_ID).Must(CheckForeignKeyValue).WithMessage("经办人:下拉选择值不正确。");
