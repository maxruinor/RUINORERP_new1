...
namespace RUINORERP.Business
{
 ...
 RuleFor(tb_Location =>tb_Location.LocationCode).MaximumLength(50).WithMessage("仓库代码:不能超过最大长度,50.");
 RuleFor(tb_Location =>tb_Location.LocationCode).NotEmpty().WithMessage("仓库代码:不能为空。");
 RuleFor(tb_Location =>tb_Location.Tel).MaximumLength(20).WithMessage("电话:不能超过最大长度,20.");
 RuleFor(tb_Location =>tb_Location.Name).MaximumLength(50).WithMessage("仓库名称:不能超过最大长度,50.");
 RuleFor(tb_Location =>tb_Location.Name).NotEmpty().WithMessage("仓库名称:不能为空。");
 RuleFor(tb_Location =>tb_Location.Desc).MaximumLength(100).WithMessage("描述:不能超过最大长度,100.");
