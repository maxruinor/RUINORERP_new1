...
namespace RUINORERP.Business
{
    ...
    public partial class tb_OpeningInventoryValidator:BaseValidatorGeneric<tb_OpeningInventory>
    {
     ...
     public tb_OpeningInventoryValidator(ApplicationContext appContext = null) : base(appContext)
     {
...
//***** 
 ...
 RuleFor(tb_OpeningInventory =>tb_OpeningInventory.RefNO).MaximumMixedLength(50).WithMessage("引用单据:不能超过最大长度,50.");

 RuleFor(tb_OpeningInventory =>tb_OpeningInventory.RefBizType).MaximumMixedLength(50).WithMessage("单据类型:不能超过最大长度,50.");

 RuleFor(tb_OpeningInventory =>tb_OpeningInventory.Notes).MaximumMixedLength(100).WithMessage("备注:不能超过最大长度,100.");
