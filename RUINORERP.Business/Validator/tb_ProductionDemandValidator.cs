...
{
    /// <summary>
    /// 生产需求分析表 是一个中间表，由计划生产单或销售订单带入数据来分析，产生采购订单再产生制令单，分析时有三步，库存不足项（包括有成品材料所有项），采购产品建议，自制品成品建议,中间表保存记录而已，操作UI上会有生成采购订单，或生产单等操作验证类
    /// </summary>
    public partial class tb_ProductionDemandValidator : AbstractValidator<tb_ProductionDemand>
    {
        public tb_ProductionDemandValidator()
        {
            ...
            RuleFor(tb_ProductionDemand => tb_ProductionDemand.PPID).Must(CheckForeignKeyValue).WithMessage("来源单据:下拉选择值不正确。");
