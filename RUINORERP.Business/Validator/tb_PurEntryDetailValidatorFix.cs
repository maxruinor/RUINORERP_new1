...
namespace RUINORERP.Business
{
    ...
    public partial class tb_PurEntryDetailValidator : BaseValidatorGeneric<tb_PurEntryDetail>
    {
        public override void Initialize()
        {
            ...
            RuleFor(x => x.UnitPrice).GreaterThan(0).When(c =>c.IsGift==null || (c.IsGift.HasValue && c.IsGift == false)).WithMessage("采购入库明细中，单价：非赠品时要大于零。");
            RuleFor(x => x.SubtotalAmount).GreaterThan(0).When(c => c.IsGift == null || (c.IsGift.HasValue && c.IsGift == false)).WithMessage("采购入库明细中，成交小计：要大于零。");
