...
namespace RUINORERP.Business
{
    ...
    public partial class tb_PurEntryReDetailValidator : AbstractValidator<tb_PurEntryReDetail>
    {
        public tb_PurEntryReDetailValidator()
        {
            ...
            RuleFor(tb_PurEntryReDetail => tb_PurEntryReDetail.ProdDetailID).Must(CheckForeignKeyValue).WithMessage("产品:下拉选择值不正确。");
            RuleFor(tb_PurEntryReDetail => tb_PurEntryReDetail.property).MaximumLength(255).WithMessage("属性:不能超过最大长度,255.");
            //***** 
            RuleFor(tb_PurEntryReDetail => tb_PurEntryReDetail.Quantity).NotNull().WithMessage("数量:不能为空。");
            RuleFor(x => x.Quantity).GreaterThanOrEqualTo(1).WithMessage("数量:不能小于或等于零。");
