...
namespace RUINORERP.Business
{
 ...
 RuleFor(tb_PurEntryRe =>tb_PurEntryRe.PurEntryReNo).MaximumMixedLength(50).WithMessage("退回单号:不能超过最大长度,50.");
 ...
 RuleFor(tb_PurEntryRe =>tb_PurEntryRe.PurEntryNo).MaximumMixedLength(50).WithMessage("入库单号:不能超过最大长度,50.");
...
//***** 
 ...
 RuleFor(tb_PurEntryRe =>tb_PurEntryRe.ShippingWay).MaximumMixedLength(50).WithMessage("发货方式:不能超过最大长度,50.");

 RuleFor(tb_PurEntryRe =>tb_PurEntryRe.TrackNo).MaximumMixedLength(50).WithMessage("物流单号:不能超过最大长度,50.");


 RuleFor(tb_PurEntryRe =>tb_PurEntryRe.PayStatus).NotEmpty().When(x => x.PayStatus.HasValue);

 RuleFor(tb_PurEntryRe =>tb_PurEntryRe.Currency_ID).NotEmpty().When(x => x.Currency_ID.HasValue);
 ...
 RuleFor(tb_PurEntryRe =>tb_PurEntryRe.Notes).MaximumMixedLength(255).WithMessage("备注:不能超过最大长度,255.");

 RuleFor(tb_PurEntryRe =>tb_PurEntryRe.ApprovalOpinions).MaximumMixedLength(200).WithMessage("审批意见:不能超过最大长度,200.");
...
//***** 
 ...
 RuleFor(tb_PurEntryRe =>tb_PurEntryRe.VoucherNO).MaximumMixedLength(50).WithMessage("凭证号码:不能超过最大长度,50.");
