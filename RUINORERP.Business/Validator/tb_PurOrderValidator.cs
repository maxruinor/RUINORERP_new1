...
namespace RUINORERP.Business
{
    ...
    {
        public tb_PurOrderValidator()
        {
            RuleFor(tb_PurOrder => tb_PurOrder.PurOrderNo).MaximumLength(100).WithMessage("采购单号:不能超过最大长度,100.");
            ...
            RuleFor(tb_PurOrder => tb_PurOrder.Notes).MaximumLength(1000).WithMessage("备注:不能超过最大长度,1000.");
            ...
            RuleFor(tb_PurOrder => tb_PurOrder.ApprovalOpinions).MaximumLength(200).WithMessage("审批意见:不能超过最大长度,200.");
            ...
            RuleFor(tb_PurOrder => tb_PurOrder.RefNO).MaximumLength(50).WithMessage("引用单据:不能超过最大长度,50.");
            ...
            RuleFor(x => x.PurDate).NotNull().DependentRules(() => {
              //  RuleFor(x => x.PurDate).NotNull();
                RuleFor(x => x.PreDeliveryDate).GreaterThan(c=>c.PurDate.AddDays(0)).WithMessage("预交日期必须大于采购日期。");
            });
