...
namespace RUINORERP.Business
{
    ...
    public partial class tb_SaleOrderValidator : AbstractValidator<tb_SaleOrder>
    {
        public tb_SaleOrderValidator()
        {
            ...
            RuleFor(tb_SaleOrder => tb_SaleOrder.PayStatus).GreaterThan(0).WithMessage("付款状态:必须选择。");

            RuleFor(tb_SaleOrder => tb_SaleOrder.PlatformOrderNo).NotEmpty().When(c => c.IsFromPlatform == true).WithMessage("平台单时，平台单号必须填写。");
