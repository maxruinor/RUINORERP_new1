...
namespace RUINORERP.Business
{
 ...
 RuleFor(tb_SaleOutDetail =>tb_SaleOutDetail.property).MaximumMixedLength(255).WithMessage("属性:不能超过最大长度,255.");
...
//***** 
 ...
 RuleFor(tb_SaleOutDetail =>tb_SaleOutDetail.Summary).MaximumMixedLength(1000).WithMessage("摘要:不能超过最大长度,1000.");

 RuleFor(tb_SaleOutDetail =>tb_SaleOutDetail.CustomerPartNo).MaximumMixedLength(50).WithMessage("客户型号:不能超过最大长度,50.");
...
//***** 
 ...
 RuleFor(tb_SaleOutDetail =>tb_SaleOutDetail.SaleFlagCode).MaximumMixedLength(200).WithMessage("标识代码:不能超过最大长度,200.");

 RuleFor(tb_SaleOutDetail =>tb_SaleOutDetail.SaleOrderDetail_ID).NotEmpty().When(x => x.SaleOrderDetail_ID.HasValue);

 RuleFor(x => x.AllocatedFreightIncome).PrecisionScale(19,4,true).WithMessage("运费收入分摊:小数位不能超过4。");
