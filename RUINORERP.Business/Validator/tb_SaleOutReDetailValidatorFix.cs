...
namespace RUINORERP.Business
{
    ...
    public partial class tb_SaleOutReDetailValidator : BaseValidatorGeneric<tb_SaleOutReDetail>
    {
        ...
        public override void Initialize()
        {
            ...
            RuleFor(x => x.UnitCommissionAmount).NotEqual(0).When(c => c.CommissionAmount != 0).WithMessage("明细中，佣金小计：单品佣金非零时不能为零。");
            RuleFor(x => x.TransactionPrice).GreaterThan(0).When(c => c.Gift == false).WithMessage("明细中，实际退款单价：非赠品时要大于零。");
