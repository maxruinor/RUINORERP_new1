...
namespace RUINORERP.Business
{
 ...
 RuleFor(tb_PurEntryRe =>tb_PurEntryRe.PurEntryRENo).MaximumLength(50).WithMessage("退回单号:不能超过最大长度,50.");
...
//***** 
 ...
 RuleFor(tb_PurEntryRe =>tb_PurEntryRe.ShippingWay).MaximumLength(50).WithMessage("发货方式:不能超过最大长度,50.");
 RuleFor(tb_PurEntryRe =>tb_PurEntryRe.TrackNo).MaximumLength(50).WithMessage("物流单号:不能超过最大长度,50.");
 ...
 RuleFor(tb_PurEntryRe =>tb_PurEntryRe.Notes).MaximumLength(255).WithMessage("备注:不能超过最大长度,255.");
 RuleFor(tb_PurEntryRe =>tb_PurEntryRe.ApprovalOpinions).MaximumLength(200).WithMessage("审批意见:不能超过最大长度,200.");
...
//***** 
 ...
 RuleFor(tb_PurEntryRe =>tb_PurEntryRe.VoucherNO).MaximumLength(50).WithMessage("凭证号码:不能超过最大长度,50.");
