...
namespace RUINORERP.Business
{
 ...
 RuleFor(tb_PurOrderRe =>tb_PurOrderRe.PurReturnNo).MaximumLength(50).WithMessage("退回单号:不能超过最大长度,50.");
 ...
 RuleFor(tb_PurOrderRe =>tb_PurOrderRe.ReturnAddress).MaximumLength(255).WithMessage("退回地址:不能超过最大长度,255.");
 RuleFor(tb_PurOrderRe =>tb_PurOrderRe.ShippingWay).MaximumLength(50).WithMessage("发货方式:不能超过最大长度,50.");
 RuleFor(tb_PurOrderRe =>tb_PurOrderRe.TrackNo).MaximumLength(50).WithMessage("物流单号:不能超过最大长度,50.");
 ...
 RuleFor(tb_PurOrderRe =>tb_PurOrderRe.Notes).MaximumLength(255).WithMessage("备注:不能超过最大长度,255.");
 RuleFor(tb_PurOrderRe =>tb_PurOrderRe.ApprovalOpinions).MaximumLength(200).WithMessage("审批意见:不能超过最大长度,200.");
//***** 
