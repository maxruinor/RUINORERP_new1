...
namespace RUINORERP.Business
{
    ...
    public partial class tb_SaleOrderValidator:AbstractValidator<tb_SaleOrder>
    {
     ...
     {
      RuleFor(tb_SaleOrder =>tb_SaleOrder.SOrderNo).MaximumLength(50).WithMessage("订单编号:不能超过最大长度,50.");
 RuleFor(tb_SaleOrder =>tb_SaleOrder.SOrderNo).NotEmpty().WithMessage("订单编号:不能为空。");
 RuleFor(tb_SaleOrder =>tb_SaleOrder.PayStatus).NotEmpty().When(x => x.PayStatus.HasValue);
...
//***** 
 ...
 RuleFor(tb_SaleOrder =>tb_SaleOrder.ShippingAddress).MaximumLength(500).WithMessage("发货地址:不能超过最大长度,500.");
 RuleFor(tb_SaleOrder =>tb_SaleOrder.ShippingWay).MaximumLength(50).WithMessage("发货方式:不能超过最大长度,50.");
 RuleFor(tb_SaleOrder =>tb_SaleOrder.TrackNo).MaximumLength(50).WithMessage("物流单号:不能超过最大长度,50.");
 ...
 RuleFor(tb_SaleOrder =>tb_SaleOrder.Notes).MaximumLength(1500).WithMessage("备注:不能超过最大长度,1500.");
 RuleFor(tb_SaleOrder =>tb_SaleOrder.ApprovalOpinions).MaximumLength(255).WithMessage("审批意见:不能超过最大长度,255.");
...
//***** 
 ...
 RuleFor(tb_SaleOrder =>tb_SaleOrder.PlatformOrderNo).MaximumLength(100).WithMessage("平台单号:不能超过最大长度,100.");
//***** 
