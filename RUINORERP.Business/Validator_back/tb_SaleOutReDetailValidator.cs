...
//***** 
 RuleFor(tb_SaleOutReDetail =>tb_SaleOutReDetail.OrderSubtotalOutQty).NotNull().WithMessage("订单累计数:不能为空。");
//***** 
 RuleFor(tb_SaleOutReDetail =>tb_SaleOutReDetail.OrderReturnSubtotalQty).NotNull().WithMessage("订单退回数:不能为空。");
 ...
 RuleFor(x => x.TaxAmount).PrecisionScale(19,4,true).WithMessage(":小数位不能超过4。");
