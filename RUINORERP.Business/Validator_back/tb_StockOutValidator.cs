...
namespace RUINORERP.Business
{
 ...
 RuleFor(tb_StockOut =>tb_StockOut.CustomerVendor_ID).Must(CheckForeignKeyValueCanNull).WithMessage("外部接收单位:下拉选择值不正确。");
 RuleFor(tb_StockOut =>tb_StockOut.CustomerVendor_ID).NotEmpty().When(x => x.CustomerVendor_ID.HasValue);
 RuleFor(tb_StockOut =>tb_StockOut.Employee_ID).Must(CheckForeignKeyValueCanNull).WithMessage("内部接收人员:下拉选择值不正确。");
 RuleFor(tb_StockOut =>tb_StockOut.Employee_ID).NotEmpty().When(x => x.Employee_ID.HasValue);
 RuleFor(tb_StockOut =>tb_StockOut.BillNo).MaximumLength(50).WithMessage("其它出库编号:不能超过最大长度,50.");
//***** 
 ...
 RuleFor(tb_StockOut =>tb_StockOut.Notes).MaximumLength(255).WithMessage("备注:不能超过最大长度,255.");
//***** 
 RuleFor(tb_StockOut =>tb_StockOut.DataStatus).NotNull().WithMessage("数据状态:不能为空。");
 RuleFor(tb_StockOut =>tb_StockOut.ApprovalOpinions).MaximumLength(500).WithMessage("审批意见:不能超过最大长度,500.");
...
//***** 
 ...
 RuleFor(tb_StockOut =>tb_StockOut.RefNO).MaximumLength(50).WithMessage("引用单号:不能超过最大长度,50.");
