...
namespace RUINORERP.Business
{
    ...
    public partial class tb_AS_RepairInStockController<T>:BaseController<T> where T : class
    {
       ...
       // private readonly ApplicationContext _appContext;
        ...
        public tb_AS_RepairInStockController(ILogger<tb_AS_RepairInStockController<T>> logger, IUnitOfWorkManage unitOfWorkManage,tb_AS_RepairInStockServices tb_AS_RepairInStockServices , ApplicationContext appContext = null): base(logger, unitOfWorkManage, appContext)
        {
            _logger = logger;
        ...
        public async Task<ReturnResults<tb_AS_RepairInStock>> SaveOrUpdate(tb_AS_RepairInStock entity)
        {
            ...
            try
            {
                ...
                if (entity.RepairInStockID > 0)
                {
                    ...
                    if (rs)
                    {
                        MyCacheManager.Instance.UpdateEntityList<tb_AS_RepairInStock>(entity);
                    }
                    Returnobj = entity;
                }
                else
        ...
        public async override Task<bool> BaseDeleteAsync(T model)
        {
            ...
            {
                ////生成时暂时只考虑了一个主键的情况
                MyCacheManager.Instance.DeleteEntityList<tb_AS_RepairInStock>(entity);
            }
            return rs;
        }
        ...
        public async override Task<bool> BaseDeleteAsync(List<T> models)
        {
            ...
            if (c>0)
            {
                rs=true;
                ////生成时暂时只考虑了一个主键的情况
                 long[] result = entitys.Select(e => e.RepairInStockID).ToArray();
                MyCacheManager.Instance.DeleteEntityList<tb_AS_RepairInStock>(result);
            }
            return rs;
        }
        ...
        public tb_AS_RepairInStock AddReEntity(tb_AS_RepairInStock entity)
        {
            ...
            MyCacheManager.Instance.UpdateEntityList<tb_AS_RepairInStock>(AddEntity);
            entity.ActionStatus = ActionStatus.无操作;
            return AddEntity;
        }
        ...
        public async Task<List<long>> AddAsync(List<tb_AS_RepairInStock> infos)
        {
            ...
            {
                 MyCacheManager.Instance.UpdateEntityList<tb_AS_RepairInStock>(infos);
            }
            return ids;
        }
        ...
        public async Task<bool> DeleteAsync(long id)
        {
            ...
            if (rs)
            {
                MyCacheManager.Instance.DeleteEntityList<tb_AS_RepairInStock>(id);
            }
            return rs;
        }
         ...
         public async Task<bool> DeleteAsync(long[] ids)
        {
            ...
            if (rs)
            {
                MyCacheManager.Instance.DeleteEntityList<tb_AS_RepairInStock>(ids);
            }
            return rs;
        }
        ...
        public virtual async Task<List<tb_AS_RepairInStock>> QueryAsync()
        {
            ...
            MyCacheManager.Instance.UpdateEntityList<tb_AS_RepairInStock>(list);
            return list;
        }
        ...
        {
            var querySqlQueryable = _unitOfWorkManage.GetDbClient().Queryable<tb_AS_RepairInStock>().Where(useLike,dto);
            return await querySqlQueryable.ToListAsync();
        }
