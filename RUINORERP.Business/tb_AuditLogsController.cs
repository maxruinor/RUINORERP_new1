    ...
    public partial class tb_AuditLogsController<T>:BaseController<T> where T : class
    {
       ...
       // private readonly ApplicationContext _appContext;
        ...
        public async override Task<ReturnMainSubResults<T>> BaseSaveOrUpdateWithChild<C>(T model) where C : class
        {
            ...
            if (entity.Audit_ID > 0)
            {
                rs = await _unitOfWorkManage.GetDbClient().UpdateNav<tb_AuditLogs>(entity as tb_AuditLogs)
        ...
        {
            rs = await _unitOfWorkManage.GetDbClient().InsertNav<tb_AuditLogs>(entity as tb_AuditLogs)
        //这里一般是子表，或没有一对多外键的情况 ，用自动的只是为了语法正常一般不会调用这个方法
                .IncludesAllFirstLayer()//自动更新导航 只能两层。这里项目中有时会失效，具体看文档
                                .ExecuteCommandAsync();
        }
        ...
        {
            var querySqlQueryable = _unitOfWorkManage.GetDbClient().Queryable<tb_AuditLogs>().Where(useLike,dto);
            return await querySqlQueryable.ToListAsync();
        }
