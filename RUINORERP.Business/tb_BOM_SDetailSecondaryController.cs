...
namespace RUINORERP.Business
{
    ...
    public partial class tb_BOM_SDetailSecondaryController<T>:BaseController<T> where T : class
    {
       ...
       // private readonly ApplicationContext _appContext;
        ...
        public tb_BOM_SDetailSecondaryController(ILogger<tb_BOM_SDetailSecondaryController<T>> logger, IUnitOfWorkManage unitOfWorkManage,tb_BOM_SDetailSecondaryServices tb_BOM_SDetailSecondaryServices , ApplicationContext appContext = null): base(logger, unitOfWorkManage, appContext)
        {
            _logger = logger;
        ...
        public async Task<ReturnResults<tb_BOM_SDetailSecondary>> SaveOrUpdate(tb_BOM_SDetailSecondary entity)
        {
            ...
            try
            {
                ...
                if (entity.SecID > 0)
                {
                    ...
                    if (rs)
                    {
                        MyCacheManager.Instance.UpdateEntityList<tb_BOM_SDetailSecondary>(entity);
                    }
                    Returnobj = entity;
                }
                else
        ...
        public async override Task<bool> BaseDeleteAsync(T model)
        {
            ...
            {
                ////生成时暂时只考虑了一个主键的情况
                MyCacheManager.Instance.DeleteEntityList<tb_BOM_SDetailSecondary>(entity);
            }
            return rs;
        }
        ...
        public async override Task<bool> BaseDeleteAsync(List<T> models)
        {
            ...
            if (c>0)
            {
                rs=true;
                ////生成时暂时只考虑了一个主键的情况
                 long[] result = entitys.Select(e => e.SecID).ToArray();
                MyCacheManager.Instance.DeleteEntityList<tb_BOM_SDetailSecondary>(result);
            }
            return rs;
        }
        ...
        public async override Task<ReturnMainSubResults<T>> BaseSaveOrUpdateWithChild<C>(T model) where C : class
        {
            ...
            if (entity.SecID > 0)
            {
                rs = await _unitOfWorkManage.GetDbClient().UpdateNav<tb_BOM_SDetailSecondary>(entity as tb_BOM_SDetailSecondary)
        ...
        {
            rs = await _unitOfWorkManage.GetDbClient().InsertNav<tb_BOM_SDetailSecondary>(entity as tb_BOM_SDetailSecondary)
                //这里一般是子表，或没有一对多外键的情况 ，用自动的只是为了语法正常一般不会调用这个方法
                .IncludesAllFirstLayer()//自动更新导航 只能两层。这里项目中有时会失效，具体看文档
                                .ExecuteCommandAsync();
        }
        ...
        public tb_BOM_SDetailSecondary AddReEntity(tb_BOM_SDetailSecondary entity)
        {
            ...
            MyCacheManager.Instance.UpdateEntityList<tb_BOM_SDetailSecondary>(AddEntity);
            entity.ActionStatus = ActionStatus.无操作;
            return AddEntity;
        }
        ...
        public async Task<List<long>> AddAsync(List<tb_BOM_SDetailSecondary> infos)
        {
            ...
            {
                 MyCacheManager.Instance.UpdateEntityList<tb_BOM_SDetailSecondary>(infos);
            }
            return ids;
        }
        ...
        public async Task<bool> DeleteAsync(long id)
        {
            ...
            if (rs)
            {
                MyCacheManager.Instance.DeleteEntityList<tb_BOM_SDetailSecondary>(id);
            }
            return rs;
        }
         ...
         public async Task<bool> DeleteAsync(long[] ids)
        {
            ...
            if (rs)
            {
                MyCacheManager.Instance.DeleteEntityList<tb_BOM_SDetailSecondary>(ids);
            }
            return rs;
        }
        ...
        public virtual async Task<List<tb_BOM_SDetailSecondary>> QueryAsync()
        {
            ...
            MyCacheManager.Instance.UpdateEntityList<tb_BOM_SDetailSecondary>(list);
            return list;
        }
        ...
        {
            var querySqlQueryable = _unitOfWorkManage.GetDbClient().Queryable<tb_BOM_SDetailSecondary>().Where(useLike,dto);
            return await querySqlQueryable.ToListAsync();
        }
