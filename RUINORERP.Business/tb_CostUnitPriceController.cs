...
namespace RUINORERP.Business
{
    ...
    public partial class tb_CostUnitPriceController<T>:BaseController<T> where T : class
    {
       ...
       // private readonly ApplicationContext _appContext;
        ...
        public tb_CostUnitPriceController(ILogger<tb_CostUnitPriceController<T>> logger, IUnitOfWorkManage unitOfWorkManage,tb_CostUnitPriceServices tb_CostUnitPriceServices ,EventDrivenCacheManager eventDrivenCacheManager, ApplicationContext appContext = null): base(logger, unitOfWorkManage, appContext)
        {
            _logger = logger;
        ...
        public async Task<ReturnResults<tb_CostUnitPrice>> SaveOrUpdate(tb_CostUnitPrice entity)
        {
            ...
            try
            {
                ...
                if (entity.ID > 0)
                {
                    ...
                    if (rs)
                    {
                        _eventDrivenCacheManager.UpdateEntity<tb_CostUnitPrice>(entity);
                    }
                    Returnobj = entity;
                }
                else
        ...
        public async override Task<bool> BaseDeleteAsync(T model)
        {
            ...
            {
                ////生成时暂时只考虑了一个主键的情况
                _eventDrivenCacheManager.DeleteEntity<tb_CostUnitPrice>(entity.PrimaryKeyID);
            }
            return rs;
        }
        ...
        public async override Task<bool> BaseDeleteAsync(List<T> models)
        {
            ...
            {
                rs=true;
                _eventDrivenCacheManager.DeleteEntityList<tb_CostUnitPrice>(entitys);
            }
            return rs;
        }
        ...
        public tb_CostUnitPrice AddReEntity(tb_CostUnitPrice entity)
        {
            tb_CostUnitPrice AddEntity =  _tb_CostUnitPriceServices.AddReEntity(entity);
             ...
             _eventDrivenCacheManager.UpdateEntity<tb_CostUnitPrice>(AddEntity);
            entity.ActionStatus = ActionStatus.无操作;
            return AddEntity;
        }
        ...
        public async Task<List<long>> AddAsync(List<tb_CostUnitPrice> infos)
        {
            ...
            {
                 _eventDrivenCacheManager.UpdateEntityList<tb_CostUnitPrice>(infos);
            }
            return ids;
        }
        ...
        public async Task<bool> DeleteAsync(tb_CostUnitPrice entity)
        {
            ...
            if (rs)
            {
                _eventDrivenCacheManager.DeleteEntity<tb_CostUnitPrice>(entity);
        ...
        public async Task<bool> DeleteAsync(long id)
        {
            ...
            if (rs)
            {
               _eventDrivenCacheManager.DeleteEntity<tb_CostUnitPrice>(id);
            }
            return rs;
        }
         ...
         public async Task<bool> DeleteAsync(long[] ids)
        {
            ...
            if (rs)
            {
                   ...
                   _eventDrivenCacheManager.DeleteEntities<tb_CostUnitPrice>(ids.Cast<object>().ToArray());
            }
            return rs;
        }
        ...
        public virtual async Task<List<tb_CostUnitPrice>> QueryAsync()
        {
            ...
            foreach (var item in list)
            {
             ...
             _eventDrivenCacheManager.UpdateEntityList<tb_CostUnitPrice>(list);
            return list;
        }
        ...
        {
            var querySqlQueryable = _unitOfWorkManage.GetDbClient().Queryable<tb_CostUnitPrice>().WhereCustom(useLike,dto);
            return await querySqlQueryable.ToListAsync();
        }
