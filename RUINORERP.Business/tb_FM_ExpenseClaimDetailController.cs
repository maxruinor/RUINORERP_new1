...
namespace RUINORERP.Business
{
    ...
    public partial class tb_FM_ExpenseClaimDetailController<T>:BaseController<T> where T : class
    {
       ...
       // private readonly ApplicationContext _appContext;
        ...
        public tb_FM_ExpenseClaimDetailController(ILogger<tb_FM_ExpenseClaimDetailController<T>> logger, IUnitOfWorkManage unitOfWorkManage,tb_FM_ExpenseClaimDetailServices tb_FM_ExpenseClaimDetailServices , ApplicationContext appContext = null): base(logger, unitOfWorkManage, appContext)
        {
            _logger = logger;
        ...
        public async Task<ReturnResults<tb_FM_ExpenseClaimDetail>> SaveOrUpdate(tb_FM_ExpenseClaimDetail entity)
        {
            ...
            try
            {
                ...
                if (entity.ClaimSubID > 0)
                {
                    ...
                    if (rs)
                    {
                        MyCacheManager.Instance.UpdateEntityList<tb_FM_ExpenseClaimDetail>(entity);
                    }
                    Returnobj = entity;
                }
                else
        ...
        public async override Task<bool> BaseDeleteAsync(T model)
        {
            ...
            {
                ////生成时暂时只考虑了一个主键的情况
                MyCacheManager.Instance.DeleteEntityList<tb_FM_ExpenseClaimDetail>(entity);
            }
            return rs;
        }
        ...
        public async override Task<bool> BaseDeleteAsync(List<T> models)
        {
            ...
            if (c>0)
            {
                rs=true;
                ////生成时暂时只考虑了一个主键的情况
                 long[] result = entitys.Select(e => e.ClaimSubID).ToArray();
                MyCacheManager.Instance.DeleteEntityList<tb_FM_ExpenseClaimDetail>(result);
            }
            return rs;
        }
        ...
        public async override Task<ReturnMainSubResults<T>> BaseSaveOrUpdateWithChild<C>(T model) where C : class
        {
            ...
            if (entity.ClaimSubID > 0)
            {
                rs = await _unitOfWorkManage.GetDbClient().UpdateNav<tb_FM_ExpenseClaimDetail>(entity as tb_FM_ExpenseClaimDetail)
        ...
        {
            rs = await _unitOfWorkManage.GetDbClient().InsertNav<tb_FM_ExpenseClaimDetail>(entity as tb_FM_ExpenseClaimDetail)
                //这里一般是子表，或没有一对多外键的情况 ，用自动的只是为了语法正常一般不会调用这个方法
                .IncludesAllFirstLayer()//自动更新导航 只能两层。这里项目中有时会失效，具体看文档
                                .ExecuteCommandAsync();
        }
        ...
        public tb_FM_ExpenseClaimDetail AddReEntity(tb_FM_ExpenseClaimDetail entity)
        {
            ...
            MyCacheManager.Instance.UpdateEntityList<tb_FM_ExpenseClaimDetail>(AddEntity);
            entity.ActionStatus = ActionStatus.无操作;
            return AddEntity;
        }
        ...
        public async Task<List<long>> AddAsync(List<tb_FM_ExpenseClaimDetail> infos)
        {
            ...
            {
                 MyCacheManager.Instance.UpdateEntityList<tb_FM_ExpenseClaimDetail>(infos);
            }
            return ids;
        }
        ...
        public async Task<bool> DeleteAsync(long id)
        {
            ...
            if (rs)
            {
                MyCacheManager.Instance.DeleteEntityList<tb_FM_ExpenseClaimDetail>(id);
            }
            return rs;
        }
         ...
         public async Task<bool> DeleteAsync(long[] ids)
        {
            ...
            if (rs)
            {
                MyCacheManager.Instance.DeleteEntityList<tb_FM_ExpenseClaimDetail>(ids);
            }
            return rs;
        }
        ...
        public virtual async Task<List<tb_FM_ExpenseClaimDetail>> QueryAsync()
        {
            ...
            MyCacheManager.Instance.UpdateEntityList<tb_FM_ExpenseClaimDetail>(list);
            return list;
        }
        ...
        {
            var querySqlQueryable = _unitOfWorkManage.GetDbClient().Queryable<tb_FM_ExpenseClaimDetail>().Where(useLike,dto);
            return await querySqlQueryable.ToListAsync();
        }
