    ...
    public partial class tb_FM_Initial_PayAndReceivableController<T>:BaseController<T> where T : class
    {
       ...
       // private readonly ApplicationContext _appContext;
        ...
        public ValidationResult Validator(tb_FM_Initial_PayAndReceivable info)
        {
           ...
           tb_FM_Initial_PayAndReceivableValidator validator = _appContext.GetRequiredService<tb_FM_Initial_PayAndReceivableValidator>();
        ...
        public async override Task<ReturnMainSubResults<T>> BaseSaveOrUpdateWithChild<C>(T model) where C : class
        {
            ...
            if (entity.Init_PRID > 0)
            {
                                 ...
                                 var result= await _unitOfWorkManage.GetDbClient().Updateable<tb_FM_Initial_PayAndReceivable>(entity as tb_FM_Initial_PayAndReceivable)
                    .ExecuteCommandAsync();
                    if (result > 0)
                    {
                        rs = true;
        ...
        {
                                  var result= await _unitOfWorkManage.GetDbClient().Insertable<tb_FM_Initial_PayAndReceivable>(entity as tb_FM_Initial_PayAndReceivable)
                    .ExecuteCommandAsync();
                    if (result > 0)
                    {
                        rs = true;
