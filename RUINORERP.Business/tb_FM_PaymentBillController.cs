       ...
       // private readonly ApplicationContext _appContext;
        ...
        public async override Task<ReturnMainSubResults<T>> BaseSaveOrUpdateWithChild<C>(T model) where C : class
        {
            ...
            if (entity.Payment_id > 0)
            {
                                 ...
                                 var result= await _unitOfWorkManage.GetDbClient().Updateable<tb_FM_PaymentBill>(entity as tb_FM_PaymentBill)
                    .ExecuteCommandAsync();
                    if (result > 0)
                    {
                        rs = true;
        ...
        {
                                  var result= await _unitOfWorkManage.GetDbClient().Insertable<tb_FM_PaymentBill>(entity as tb_FM_PaymentBill)
                    .ExecuteCommandAsync();
                    if (result > 0)
                    {
                        rs = true;
