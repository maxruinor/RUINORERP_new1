...
namespace RUINORERP.Business
{
    ...
    public partial class tb_FM_PaymentRecordController<T> : BaseController<T> where T : class
    {
        ...
        public async override Task<ReturnMainSubResults<T>> BaseSaveOrUpdateWithChild<C>(T model) where C : class
        {
            ...
            try
            {
                ...
                if (entity.PaymentId > 0)
                {
                    ...
                    await _unitOfWorkManage.GetDbClient().Updateable<tb_FM_PaymentRecord>(entity as tb_FM_PaymentRecord)
                      .ExecuteCommandAsync();
                }
                else
                {
                    await _unitOfWorkManage.GetDbClient().Insertable<tb_FM_PaymentRecord>(entity as tb_FM_PaymentRecord)
        ...
        {
            tb_FM_PaymentRecord entity = model as tb_FM_PaymentRecord;
            int rs = await _unitOfWorkManage.GetDbClient().Deleteable<tb_FM_PaymentRecord>(m => m.PaymentId == entity.PaymentId)
                                       .ExecuteCommandAsync();
            if (rs > 0)
            {
                //////生成时暂时只考虑了一个主键的情况
