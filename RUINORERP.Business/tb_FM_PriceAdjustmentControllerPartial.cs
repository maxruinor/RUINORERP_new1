    ...
    public partial class tb_FM_PriceAdjustmentController<T> : BaseController<T> where T : class
    {
                ...
                else
                {
                    ...
                    if (rmr.Succeeded)
                    {
                        ...
                        else
                        {
                            ...
                            ApprovalResult.Succeeded = false;
                            throw new Exception($"审核{payTypeText}失败");
                        }
                    }
                    else
                    {
                        ...
                        ApprovalResult.Succeeded = false;
                        throw new Exception($"保存{payTypeText}失败");
                    }
                }
        ...
        public async Task<bool> BaseLogicDeleteAsync(tb_FM_PriceAdjustment ObjectEntity)
        {
            ...
            if (count > 0)
            {
                ...
                return true;
                ////生成时暂时只考虑了一个主键的情况
                // Cache.EntityCacheHelper.DeleteEntityList<tb_FM_PriceAdjustmentController>(entity);
            }
            return false;
        }
