...
namespace RUINORERP.Business
{
    ...
    public partial class tb_FM_PriceAdjustmentDetailController<T>:BaseController<T> where T : class
    {
       ...
       // private readonly ApplicationContext _appContext;
        ...
        public tb_FM_PriceAdjustmentDetailController(ILogger<tb_FM_PriceAdjustmentDetailController<T>> logger, IUnitOfWorkManage unitOfWorkManage,tb_FM_PriceAdjustmentDetailServices tb_FM_PriceAdjustmentDetailServices , ApplicationContext appContext = null): base(logger, unitOfWorkManage, appContext)
        {
            _logger = logger;
        ...
        public async Task<ReturnResults<tb_FM_PriceAdjustmentDetail>> SaveOrUpdate(tb_FM_PriceAdjustmentDetail entity)
        {
            ...
            try
            {
                ...
                if (entity.AdjustDetailID > 0)
                {
                    ...
                    if (rs)
                    {
                        MyCacheManager.Instance.UpdateEntityList<tb_FM_PriceAdjustmentDetail>(entity);
                    }
                    Returnobj = entity;
                }
                else
        ...
        public async override Task<bool> BaseDeleteAsync(T model)
        {
            ...
            {
                ////生成时暂时只考虑了一个主键的情况
                MyCacheManager.Instance.DeleteEntityList<tb_FM_PriceAdjustmentDetail>(entity);
            }
            return rs;
        }
        ...
        public async override Task<bool> BaseDeleteAsync(List<T> models)
        {
            ...
            if (c>0)
            {
                rs=true;
                ////生成时暂时只考虑了一个主键的情况
                 long[] result = entitys.Select(e => e.AdjustDetailID).ToArray();
                MyCacheManager.Instance.DeleteEntityList<tb_FM_PriceAdjustmentDetail>(result);
            }
            return rs;
        }
        ...
        public tb_FM_PriceAdjustmentDetail AddReEntity(tb_FM_PriceAdjustmentDetail entity)
        {
            ...
            MyCacheManager.Instance.UpdateEntityList<tb_FM_PriceAdjustmentDetail>(AddEntity);
            entity.ActionStatus = ActionStatus.无操作;
            return AddEntity;
        }
        ...
        public async Task<List<long>> AddAsync(List<tb_FM_PriceAdjustmentDetail> infos)
        {
            ...
            {
                 MyCacheManager.Instance.UpdateEntityList<tb_FM_PriceAdjustmentDetail>(infos);
            }
            return ids;
        }
        ...
        public async Task<bool> DeleteAsync(long id)
        {
            ...
            if (rs)
            {
                MyCacheManager.Instance.DeleteEntityList<tb_FM_PriceAdjustmentDetail>(id);
            }
            return rs;
        }
         ...
         public async Task<bool> DeleteAsync(long[] ids)
        {
            ...
            if (rs)
            {
                MyCacheManager.Instance.DeleteEntityList<tb_FM_PriceAdjustmentDetail>(ids);
            }
            return rs;
        }
        ...
        public virtual async Task<List<tb_FM_PriceAdjustmentDetail>> QueryAsync()
        {
            ...
            MyCacheManager.Instance.UpdateEntityList<tb_FM_PriceAdjustmentDetail>(list);
            return list;
        }
        ...
        {
            var querySqlQueryable = _unitOfWorkManage.GetDbClient().Queryable<tb_FM_PriceAdjustmentDetail>().Where(useLike,dto);
            return await querySqlQueryable.ToListAsync();
        }
