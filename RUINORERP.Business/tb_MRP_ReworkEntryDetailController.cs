...
namespace RUINORERP.Business
{
    ...
    public partial class tb_MRP_ReworkEntryDetailController<T>:BaseController<T> where T : class
    {
       ...
       // private readonly ApplicationContext _appContext;
        ...
        public tb_MRP_ReworkEntryDetailController(ILogger<tb_MRP_ReworkEntryDetailController<T>> logger, IUnitOfWorkManage unitOfWorkManage,tb_MRP_ReworkEntryDetailServices tb_MRP_ReworkEntryDetailServices , ApplicationContext appContext = null): base(logger, unitOfWorkManage, appContext)
        {
            _logger = logger;
        ...
        public async Task<ReturnResults<tb_MRP_ReworkEntryDetail>> SaveOrUpdate(tb_MRP_ReworkEntryDetail entity)
        {
            ...
            try
            {
                ...
                if (entity.ReworkEntryCID > 0)
                {
                    ...
                    if (rs)
                    {
                        MyCacheManager.Instance.UpdateEntityList<tb_MRP_ReworkEntryDetail>(entity);
                    }
                    Returnobj = entity;
                }
                else
        ...
        public async override Task<bool> BaseDeleteAsync(T model)
        {
            ...
            {
                ////生成时暂时只考虑了一个主键的情况
                MyCacheManager.Instance.DeleteEntityList<tb_MRP_ReworkEntryDetail>(entity);
            }
            return rs;
        }
        ...
        public async override Task<bool> BaseDeleteAsync(List<T> models)
        {
            ...
            if (c>0)
            {
                rs=true;
                ////生成时暂时只考虑了一个主键的情况
                 long[] result = entitys.Select(e => e.ReworkEntryCID).ToArray();
                MyCacheManager.Instance.DeleteEntityList<tb_MRP_ReworkEntryDetail>(result);
            }
            return rs;
        }
        ...
        public async override Task<ReturnMainSubResults<T>> BaseSaveOrUpdateWithChild<C>(T model) where C : class
        {
            ...
            if (entity.ReworkEntryCID > 0)
            {
                rs = await _unitOfWorkManage.GetDbClient().UpdateNav<tb_MRP_ReworkEntryDetail>(entity as tb_MRP_ReworkEntryDetail)
        ...
        {
            rs = await _unitOfWorkManage.GetDbClient().InsertNav<tb_MRP_ReworkEntryDetail>(entity as tb_MRP_ReworkEntryDetail)
                //这里一般是子表，或没有一对多外键的情况 ，用自动的只是为了语法正常一般不会调用这个方法
                .IncludesAllFirstLayer()//自动更新导航 只能两层。这里项目中有时会失效，具体看文档
                                .ExecuteCommandAsync();
        }
        ...
        public tb_MRP_ReworkEntryDetail AddReEntity(tb_MRP_ReworkEntryDetail entity)
        {
            ...
            MyCacheManager.Instance.UpdateEntityList<tb_MRP_ReworkEntryDetail>(AddEntity);
            entity.ActionStatus = ActionStatus.无操作;
            return AddEntity;
        }
        ...
        public async Task<List<long>> AddAsync(List<tb_MRP_ReworkEntryDetail> infos)
        {
            ...
            {
                 MyCacheManager.Instance.UpdateEntityList<tb_MRP_ReworkEntryDetail>(infos);
            }
            return ids;
        }
        ...
        public async Task<bool> DeleteAsync(long id)
        {
            ...
            if (rs)
            {
                MyCacheManager.Instance.DeleteEntityList<tb_MRP_ReworkEntryDetail>(id);
            }
            return rs;
        }
         ...
         public async Task<bool> DeleteAsync(long[] ids)
        {
            ...
            if (rs)
            {
                MyCacheManager.Instance.DeleteEntityList<tb_MRP_ReworkEntryDetail>(ids);
            }
            return rs;
        }
        ...
        public virtual async Task<List<tb_MRP_ReworkEntryDetail>> QueryAsync()
        {
            ...
            MyCacheManager.Instance.UpdateEntityList<tb_MRP_ReworkEntryDetail>(list);
            return list;
        }
        ...
        {
            var querySqlQueryable = _unitOfWorkManage.GetDbClient().Queryable<tb_MRP_ReworkEntryDetail>().Where(useLike,dto);
            return await querySqlQueryable.ToListAsync();
        }
