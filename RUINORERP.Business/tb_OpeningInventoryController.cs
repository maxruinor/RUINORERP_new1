       ...
       // private readonly ApplicationContext _appContext;
        ...
        public async override Task<ReturnMainSubResults<T>> BaseSaveOrUpdateWithChild<C>(T model) where C : class
        {
            ...
            if (entity.OI_ID > 0)
            {
                                 ...
                                 var result= await _unitOfWorkManage.GetDbClient().Updateable<tb_OpeningInventory>(entity as tb_OpeningInventory)
                    .ExecuteCommandAsync();
                    if (result > 0)
                    {
                        rs = true;
        ...
        {
                                  var result= await _unitOfWorkManage.GetDbClient().Insertable<tb_OpeningInventory>(entity as tb_OpeningInventory)
                    .ExecuteCommandAsync();
                    if (result > 0)
                    {
                        rs = true;
