       ...
       // private readonly ApplicationContext _appContext;
        ...
        public async override Task<ReturnMainSubResults<T>> BaseSaveOrUpdateWithChild<C>(T model) where C : class
        {
            ...
            if (entity.Paytype_DetailID > 0)
            {
                                 ...
                                 var result= await _unitOfWorkManage.GetDbClient().Updateable<tb_PaymentMethodDetail>(entity as tb_PaymentMethodDetail)
                    .ExecuteCommandAsync();
                    if (result > 0)
                    {
                        rs = true;
        ...
        {
                                  var result= await _unitOfWorkManage.GetDbClient().Insertable<tb_PaymentMethodDetail>(entity as tb_PaymentMethodDetail)
                    .ExecuteCommandAsync();
                    if (result > 0)
                    {
                        rs = true;
