...
namespace RUINORERP.Business
{
    ...
    public partial class tb_ProcessDefinitionController<T>:BaseController<T> where T : class
    {
       ...
       // private readonly ApplicationContext _appContext;
        ...
        public tb_ProcessDefinitionController(ILogger<BaseController<T>> logger, IUnitOfWorkManage unitOfWorkManage,tb_ProcessDefinitionServices tb_ProcessDefinitionServices , ApplicationContext appContext = null): base(logger, unitOfWorkManage, appContext)
        {
            _logger = logger;
        ...
        public async Task<ReturnResults<tb_ProcessDefinition>> SaveOrUpdate(tb_ProcessDefinition entity)
        {
            ...
            try
            {
                ...
                if (entity.ProcessDefinition_Id > 0)
                {
                    ...
                    if (rs)
                    {
                        MyCacheManager.Instance.UpdateEntityList<tb_ProcessDefinition>(entity);
                    }
                    Returnobj = entity;
                }
                else
        ...
        public async override Task<bool> BaseDeleteAsync(T model)
        {
            ...
            {
                ////生成时暂时只考虑了一个主键的情况
                MyCacheManager.Instance.DeleteEntityList<tb_ProcessDefinition>(entity);
            }
            return rs;
        }
        ...
        public async override Task<bool> BaseDeleteAsync(List<T> models)
        {
            ...
            if (c>0)
            {
                rs=true;
                ////生成时暂时只考虑了一个主键的情况
                 long[] result = entitys.Select(e => e.ProcessDefinition_Id).ToArray();
                MyCacheManager.Instance.DeleteEntityList<tb_ProcessDefinition>(result);
            }
            return rs;
        }
        ...
        public async override Task<ReturnMainSubResults<T>> BaseSaveOrUpdateWithChild<C>(T model) where C : class
        {
            ...
            if (entity.ProcessDefinition_Id > 0)
            {
                rs = await _unitOfWorkManage.GetDbClient().UpdateNav<tb_ProcessDefinition>(entity as tb_ProcessDefinition)
        ...
        {
            rs = await _unitOfWorkManage.GetDbClient().InsertNav<tb_ProcessDefinition>(entity as tb_ProcessDefinition)
        //这里一般是子表，或没有一对多外键的情况 ，用自动的只是为了语法正常一般不会调用这个方法
                .IncludesAllFirstLayer()//自动更新导航 只能两层。这里项目中有时会失效，具体看文档
                                .ExecuteCommandAsync();
        }
        ...
        public tb_ProcessDefinition AddReEntity(tb_ProcessDefinition entity)
        {
            ...
            MyCacheManager.Instance.UpdateEntityList<tb_ProcessDefinition>(AddEntity);
            entity.ActionStatus = ActionStatus.无操作;
            return AddEntity;
        }
        ...
        public async Task<List<long>> AddAsync(List<tb_ProcessDefinition> infos)
        {
            ...
            {
                 MyCacheManager.Instance.UpdateEntityList<tb_ProcessDefinition>(infos);
            }
            return ids;
        }
        ...
        public async Task<bool> DeleteAsync(long id)
        {
            ...
            if (rs)
            {
                MyCacheManager.Instance.DeleteEntityList<tb_ProcessDefinition>(id);
            }
            return rs;
        }
         ...
         public async Task<bool> DeleteAsync(long[] ids)
        {
            ...
            if (rs)
            {
                MyCacheManager.Instance.DeleteEntityList<tb_ProcessDefinition>(ids);
            }
            return rs;
        }
        ...
        public virtual async Task<List<tb_ProcessDefinition>> QueryAsync()
        {
            ...
            MyCacheManager.Instance.UpdateEntityList<tb_ProcessDefinition>(list);
            return list;
        }
        ...
        {
            var querySqlQueryable = _unitOfWorkManage.GetDbClient().Queryable<tb_ProcessDefinition>().Where(useLike,dto);
            return await querySqlQueryable.ToListAsync();
        }
