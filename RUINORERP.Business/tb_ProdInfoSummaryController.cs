...
namespace RUINORERP.Business
{
    ...
    public partial class tb_ProdInfoSummaryController<T>:BaseController<T> where T : class
    {
       ...
       // private readonly ApplicationContext _appContext;
        ...
        public tb_ProdInfoSummaryController(ILogger<BaseController<T>> logger, IUnitOfWorkManage unitOfWorkManage,tb_ProdInfoSummaryServices tb_ProdInfoSummaryServices , ApplicationContext appContext = null): base(logger, unitOfWorkManage, appContext)
        {
            _logger = logger;
        ...
        public async override Task<ReturnMainSubResults<T>> BaseSaveOrUpdateWithChild<C>(T model) where C : class
        {
            ...
            if (entity.id > 0)
            {
                rs = await _unitOfWorkManage.GetDbClient().UpdateNav<tb_ProdInfoSummary>(entity as tb_ProdInfoSummary)
        ...
        {
            rs = await _unitOfWorkManage.GetDbClient().InsertNav<tb_ProdInfoSummary>(entity as tb_ProdInfoSummary)
        //这里一般是子表，或没有一对多外键的情况 ，用自动的只是为了语法正常一般不会调用这个方法
                .IncludesAllFirstLayer()//自动更新导航 只能两层。这里项目中有时会失效，具体看文档
                                .ExecuteCommandAsync();
        }
