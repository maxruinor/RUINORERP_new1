...
namespace RUINORERP.Business
{
    ...
    public partial class tb_ProdReturningDetailController<T>:BaseController<T> where T : class
    {
       ...
       // private readonly ApplicationContext _appContext;
        ...
        public tb_ProdReturningDetailController(ILogger<tb_ProdReturningDetailController<T>> logger, IUnitOfWorkManage unitOfWorkManage,tb_ProdReturningDetailServices tb_ProdReturningDetailServices , ApplicationContext appContext = null): base(logger, unitOfWorkManage, appContext)
        {
            _logger = logger;
        ...
        public async Task<ReturnResults<tb_ProdReturningDetail>> SaveOrUpdate(tb_ProdReturningDetail entity)
        {
            ...
            try
            {
                ...
                if (entity.ReturnSub_ID > 0)
                {
                    ...
                    if (rs)
                    {
                        MyCacheManager.Instance.UpdateEntityList<tb_ProdReturningDetail>(entity);
                    }
                    Returnobj = entity;
                }
                else
        ...
        public async override Task<bool> BaseDeleteAsync(T model)
        {
            ...
            {
                ////生成时暂时只考虑了一个主键的情况
                MyCacheManager.Instance.DeleteEntityList<tb_ProdReturningDetail>(entity);
            }
            return rs;
        }
        ...
        public async override Task<bool> BaseDeleteAsync(List<T> models)
        {
            ...
            if (c>0)
            {
                rs=true;
                ////生成时暂时只考虑了一个主键的情况
                 long[] result = entitys.Select(e => e.ReturnSub_ID).ToArray();
                MyCacheManager.Instance.DeleteEntityList<tb_ProdReturningDetail>(result);
            }
            return rs;
        }
        ...
        public async override Task<ReturnMainSubResults<T>> BaseSaveOrUpdateWithChild<C>(T model) where C : class
        {
            ...
            if (entity.ReturnSub_ID > 0)
            {
                rs = await _unitOfWorkManage.GetDbClient().UpdateNav<tb_ProdReturningDetail>(entity as tb_ProdReturningDetail)
        ...
        {
            rs = await _unitOfWorkManage.GetDbClient().InsertNav<tb_ProdReturningDetail>(entity as tb_ProdReturningDetail)
        //这里一般是子表，或没有一对多外键的情况 ，用自动的只是为了语法正常一般不会调用这个方法
                .IncludesAllFirstLayer()//自动更新导航 只能两层。这里项目中有时会失效，具体看文档
                                .ExecuteCommandAsync();
        }
        ...
        public tb_ProdReturningDetail AddReEntity(tb_ProdReturningDetail entity)
        {
            ...
            MyCacheManager.Instance.UpdateEntityList<tb_ProdReturningDetail>(AddEntity);
            entity.ActionStatus = ActionStatus.无操作;
            return AddEntity;
        }
        ...
        public async Task<List<long>> AddAsync(List<tb_ProdReturningDetail> infos)
        {
            ...
            {
                 MyCacheManager.Instance.UpdateEntityList<tb_ProdReturningDetail>(infos);
            }
            return ids;
        }
        ...
        public async Task<bool> DeleteAsync(long id)
        {
            ...
            if (rs)
            {
                MyCacheManager.Instance.DeleteEntityList<tb_ProdReturningDetail>(id);
            }
            return rs;
        }
         ...
         public async Task<bool> DeleteAsync(long[] ids)
        {
            ...
            if (rs)
            {
                MyCacheManager.Instance.DeleteEntityList<tb_ProdReturningDetail>(ids);
            }
            return rs;
        }
        ...
        public virtual async Task<List<tb_ProdReturningDetail>> QueryAsync()
        {
            ...
            MyCacheManager.Instance.UpdateEntityList<tb_ProdReturningDetail>(list);
            return list;
        }
        ...
        {
            var querySqlQueryable = _unitOfWorkManage.GetDbClient().Queryable<tb_ProdReturningDetail>().Where(useLike,dto);
            return await querySqlQueryable.ToListAsync();
        }
