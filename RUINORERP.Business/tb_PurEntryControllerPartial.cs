...
namespace RUINORERP.Business
{
    public partial class tb_PurEntryController<T>
    {
        ...
        /// <returns></returns>
        public async virtual Task<ReturnResults<bool>> BatchApproval(List<tb_PurEntry> entitys, ApprovalEntity approvalEntity)
        {
            ...
            try
            {
                ...
                {
                    for (int ii = 0; ii < entitys.Count; ii++)
                    {
                        //处理采购订单
                        entitys[ii].tb_purorder = _unitOfWorkManage.GetDbClient().Queryable<tb_PurOrder>()
                             ...
                             .Where(c => c.PurOrder_ID == entitys[ii].PurOrder_ID)
                             .Single();


                        //如果入库明细中的产品。不存在于订单中。审核失败。
                        foreach (var child in entitys[ii].tb_PurEntryDetails)
                        {
                            if (!entitys[ii].tb_purorder.tb_PurOrderDetails.Any(c => c.ProdDetailID == child.ProdDetailID))
                            {
                                rs.Succeeded = false;
                        ...
                        foreach (tb_PurEntryDetail child in entitys[ii].tb_PurEntryDetails)
                        {
                            ...
                            .Where(c => c.Employee_ID == entitys[ii].Employee_ID.Value && c.ProdDetailID == child.ProdDetailID).First();
                            //如果存在则更新，否则插入
                            if (priceRecord == null)
                            {
                                priceRecord = new tb_PriceRecord();
                            }
                            priceRecord.Employee_ID = entitys[ii].Employee_ID.Value;
                            ...
                            if (rr.Succeeded)
                            {
                                ...
                                if (Opening)
                                {
                                    ...
                                    oinv.InitInvDate = entitys[ii].EntryDate;
                                    oinv.RefBillID = entitys[ii].PurEntryID;
                                    oinv.RefNO = entitys[ii].PurEntryNo;
                                    oinv.InitQty = 0;
                        ...
                        foreach (var item in entitys[ii].tb_purorder.tb_PurEntries)
                        {
                            detailList.AddRange(item.tb_PurEntryDetails);
                        }

                        //分两种情况处理。
                        for (int i = 0; i < entitys[ii].tb_purorder.tb_PurOrderDetails.Count; i++)
                        {
                            //如果当前订单明细行，不存在于入库明细行。直接跳过。这种就是多行多品被删除时。不需要比较


                            string prodName = entitys[ii].tb_purorder.tb_PurOrderDetails[i].tb_proddetail.tb_prod.CNName +
                                      entitys[ii].tb_purorder.tb_PurOrderDetails[i].tb_proddetail.tb_prod.Specifications;
                            //明细中有相同的产品或物品。
                            var aa = entitys[ii].tb_purorder.tb_PurOrderDetails.Select(c => c.ProdDetailID).ToList().GroupBy(x => x).Where(x => x.Count() > 1).Select(x => x.Key).ToList();
                            if (aa.Count > 0 && entitys[ii].tb_purorder.tb_PurOrderDetails[i].PurOrder_ChildID > 0)
                            {
                                #region //如果存在不是引用的明细,则不允许入库。这样不支持手动添加的情况。
                                if (entitys[ii].tb_PurEntryDetails.Any(c => c.PurOrder_ChildID == 0))
                                {
                                    //如果存在不是引用的明细,则不允许入库。这样不支持手动添加的情况。
                                    string msg = $"采购订单:{entitys[ii].tb_purorder.PurOrderNo}的【{prodName}】在订单明细中拥有多行记录，必须使用引用的方式添加，审核失败！";
                                ...
                                var inQty = detailList.Where(c => c.ProdDetailID == entitys[ii].tb_purorder.tb_PurOrderDetails[i].ProdDetailID && c.PurOrder_ChildID == entitys[ii].tb_purorder.tb_PurOrderDetails[i].PurOrder_ChildID).Sum(c => c.Quantity);
                                if (inQty > entitys[ii].tb_purorder.tb_PurOrderDetails[i].Quantity)
                                {
                                    string msg = $"采购订单:{entitys[ii].tb_purorder.PurOrderNo}的【{prodName}】的入库数量不能大于订单中对应行的数量\r\n" + $"                                    或存在针对当前采购订单重复录入了采购入库单，审核失败！";
                                    MessageBox.Show(msg, "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                    _unitOfWorkManage.RollbackTran();
                                    _logger.LogInformation(msg);
                                    return rs;
                                ...
                                {
                                    var RowQty = entitys[ii].tb_PurEntryDetails.Where(c => c.ProdDetailID == entitys[ii].tb_purorder.tb_PurOrderDetails[i].ProdDetailID && c.PurOrder_ChildID == entitys[ii].tb_purorder.tb_PurOrderDetails[i].PurOrder_ChildID).Sum(c => c.Quantity);
                                    //算出交付的数量
                                    entitys[ii].tb_purorder.tb_PurOrderDetails[i].DeliveredQuantity += RowQty;
                                    //如果已交数据大于 订单数量 给出警告实际操作中 使用其他方式将备品入库
                                    if (entitys[ii].tb_purorder.tb_PurOrderDetails[i].DeliveredQuantity > entitys[ii].tb_purorder.tb_PurOrderDetails[i].Quantity)
                                    {
                                        throw new Exception($"入库单：{entitys[ii].PurEntryNo}审核时，对应的订单：{entitys[ii].tb_purorder.PurOrderNo}，入库总数量不能大于订单数量！");
                                    }
                                }
                            }
                            else
                            {
                                //一对一时
                                var inQty = detailList.Where(c => c.ProdDetailID == entitys[ii].tb_purorder.tb_PurOrderDetails[i].ProdDetailID).Sum(c => c.Quantity);
                                if (inQty > entitys[ii].tb_purorder.tb_PurOrderDetails[i].Quantity)
                                {
                                ...
                                {
                                    //当前行累计到交付
                                    var RowQty = entitys[ii].tb_PurEntryDetails.Where(c => c.ProdDetailID == entitys[ii].tb_purorder.tb_PurOrderDetails[i].ProdDetailID).Sum(c => c.Quantity);
                                    entitys[ii].tb_purorder.tb_PurOrderDetails[i].DeliveredQuantity += RowQty;
                                    //如果已交数据大于 订单数量 给出警告实际操作中 使用其他方式将备品入库
                                    if (entitys[ii].tb_purorder.tb_PurOrderDetails[i].DeliveredQuantity > entitys[ii].tb_purorder.tb_PurOrderDetails[i].Quantity)
                                    {
                        ...
                        //更新已交数量
                        int poCounter = await _unitOfWorkManage.GetDbClient().Updateable<tb_PurOrderDetail>(entitys[ii].tb_purorder.tb_PurOrderDetails).ExecuteCommandAsync();
                        if (poCounter > 0)
                        {
                            if (AuthorizeController.GetShowDebugInfoAuthorization(_appContext))
                            {
                                _logger.Info(entitys[ii].PurEntryNo + "==>" + entitys[ii].PurOrder_NO + $"对应 的订单更新成功===重点代码 看已交数量是否正确");
                            }
                        }

                        //这部分是否能提出到上一级公共部分？
                        entitys[ii].DataStatus = (int)DataStatus.确认;
                        entitys[ii].ApprovalOpinions = approvalEntity.ApprovalComments;
                        //后面已经修改为
                        entitys[ii].ApprovalResults = approvalEntity.ApprovalResults;
                        entitys[ii].ApprovalStatus = (int)ApprovalStatus.已审核;
                        BusinessHelper.Instance.ApproverEntity(entitys[ii]);
                        //只更新指定列
                        // var result = _unitOfWorkManage.GetDbClient().Updateable<tb_Stocktake>(entity).UpdateColumns(it => new { it.DataStatus, it.ApprovalOpinions }).ExecuteCommand();
                        int counter = await _unitOfWorkManage.GetDbClient().Updateable<tb_PurEntry>(entitys[ii]).ExecuteCommandAsync();
                        if (counter > 0)
                        {
                            if (AuthorizeController.GetShowDebugInfoAuthorization(_appContext))
                            {
                                _logger.Info(entitys[ii].PurEntryNo + "==>" + "状态更新成功");
                            }
                        }

                        //采购入库单，如果来自于采购订单，则要把入库数量累加到订单中的已交数量 TODO 销售也会有这种情况
                        if (entitys[ii].tb_purorder != null && (entitys[ii].TotalQty == entitys[ii].tb_purorder.TotalQty || entitys[ii].tb_purorder.tb_PurOrderDetails.Sum(c => c.DeliveredQuantity) == entitys[ii].tb_purorder.TotalQty))
                        {
                            entitys[ii].tb_purorder.DataStatus = (int)DataStatus.完结;
                            ...
                            int poendcounter = await _unitOfWorkManage.GetDbClient().Updateable(entitys[ii].tb_purorder)
                                .UpdateColumns(t => new { t.DataStatus }).ExecuteCommandAsync();
                            if (poendcounter > 0)
                            {
                                if (AuthorizeController.GetShowDebugInfoAuthorization(_appContext))
                                {
                                    _logger.Info(entitys[ii].tb_purorder.PurOrderNo + "==>" + "结案状态更新成功");
                                }
                            }
                        }
                    }
