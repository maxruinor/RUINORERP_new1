                    ...
                    if (entity.SaleOut_MainID.HasValue && entity.SaleOut_MainID.Value > 0)
                    {
                        ...
                        if (entity.tb_saleout != null)
                        {
                            ...
                            for (int i = 0; i < entity.tb_saleout.tb_saleorder.tb_SaleOrderDetails.Count; i++)
                            {
                                ...
                                orderDetailLines = entity.tb_saleout.tb_saleorder.tb_SaleOrderDetails.Where(c => c.ProdDetailID == orderDetail.ProdDetailID).ToList();
                                List<tb_SaleOutDetail> outDetails = entity.tb_saleout.tb_SaleOutDetails.Where(c => c.ProdDetailID == orderDetail.ProdDetailID && c.Location_ID == orderDetail.Location_ID).ToList();
