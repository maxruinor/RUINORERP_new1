...
namespace RUINORERP.Business
{
    ...
    public partial class tb_StockInDetailController<T>:BaseController<T> where T : class
    {
       ...
       // private readonly ApplicationContext _appContext;
        ...
        public tb_StockInDetailController(ILogger<BaseController<T>> logger, IUnitOfWorkManage unitOfWorkManage,tb_StockInDetailServices tb_StockInDetailServices , ApplicationContext appContext = null): base(logger, unitOfWorkManage, appContext)
        {
            _logger = logger;
        ...
        public async override Task<ReturnMainSubResults<T>> BaseSaveOrUpdateWithChild<C>(T model) where C : class
        {
            ...
            if (entity.Sub_ID > 0)
            {
                rs = await _unitOfWorkManage.GetDbClient().UpdateNav<tb_StockInDetail>(entity as tb_StockInDetail)
        ...
        {
            rs = await _unitOfWorkManage.GetDbClient().InsertNav<tb_StockInDetail>(entity as tb_StockInDetail)
        //这里一般是子表，或没有一对多外键的情况 ，用自动的只是为了语法正常一般不会调用这个方法
                .IncludesAllFirstLayer()//自动更新导航 只能两层。这里项目中有时会失效，具体看文档
                                .ExecuteCommandAsync();
        }
        ...
        {
            var querySqlQueryable = _unitOfWorkManage.GetDbClient().Queryable<tb_StockInDetail>().Where(useLike,dto);
            return await querySqlQueryable.ToListAsync();
        }
