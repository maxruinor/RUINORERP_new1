...
namespace RUINORERP.Business
{
    public partial class tb_StockOutController<T>
    {
        ...
        /// <returns></returns>
        public async virtual Task<ReturnResults<bool>> AdjustingAsync(tb_StockOut entity, ApprovalEntity approvalEntity)
        {
                ...
                else
                {
                    ...
                    foreach (var child in entity.tb_StockOutDetails)
                    {
                        if (!_appContext.SysConfig.CheckNegativeInventory && child.Qty < 0)
                        {
                            ...
                            approvalEntity.ApprovalComments = "系统设置不请允许负库存，请检查出库数量与库存相关数据";
                            break;
                        }
