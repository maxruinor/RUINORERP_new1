       ...
       // private readonly ApplicationContext _appContext;
        ...
        public async override Task<ReturnMainSubResults<T>> BaseSaveOrUpdateWithChild<C>(T model) where C : class
        {
            ...
            if (entity.StockTransferDetaill_ID > 0)
            {
                                 ...
                                 var result= await _unitOfWorkManage.GetDbClient().Updateable<tb_StockTransferDetail>(entity as tb_StockTransferDetail)
                    .ExecuteCommandAsync();
                    if (result > 0)
                    {
                        rs = true;
        ...
        {
            var querySqlQueryable = _unitOfWorkManage.GetDbClient().Queryable<tb_StockTransferDetail>().WhereCustom(useLike,dto);
            return await querySqlQueryable.ToListAsync();
        }
