...
namespace RUINORERP.Common.Helper
{
    ...
    public static class DynamicLinqFactory
    {
        ...
        {
            var outList = new List<DynamicLinqHelper>();
            var tokenss = Regex.Matches(FormatString(str), _pattern, RegexOptions.Compiled);

            MatchCollection matches = Regex.Matches(FormatString(str), _pattern, RegexOptions.Compiled);
            var tokens = matches.Cast<Match>().Select(m => m.Groups[1].Value.Trim()).ToList();
