...
namespace RUINORERP.Common.Helper
{
    ...
    public class ExpressionToSql
    {
        ...
        {
            string left = DealExpression(exp.Left);
            string right = DealExpression(exp.Right, true);

            // 处理 AndAlso True / OrElse False 等冗余条件
            if (exp.NodeType == ExpressionType.AndAlso && (right == "1" || right == "1=1"))
            {
                // 左边 AND True 可以简化为只保留左边
                return left;
            }
            if (exp.NodeType == ExpressionType.OrElse && (right == "0" || right == "1=0"))
            {
                // 左边 OR False 可以简化为只保留左边
                return left;
            }

            return left + NullValueDeal(exp.NodeType, right);
        }
