# Fix encoding for system introduction file
# Using only ASCII characters to avoid encoding issues

Write-Host "Starting to fix system introduction file encoding..."

# Define file path
$file = "e:\CodeRepository\SynologyDrive\RUINORERP\RUINORERP.Helper\系统简介.htm"

# Check if file exists
if (Test-Path $file) {
    Write-Host "File found, processing..."
    
    # Read file content
    $content = Get-Content -Path $file -Encoding UTF8
    $contentString = $content -join "`r`n"
    
    Write-Host "Content read successfully"
    
    # Fix common encoding issues - replace garbled text with correct Chinese characters
    # CSS comment fixes
