...
namespace RUINORERP.Lib.BusinessImage
{
    ...
    public abstract class ImageCacheServiceBase : IDisposable
    {
        ...
        public virtual void AddImageInfo(object imageInfo)
        {
            if (imageInfo == null)
                return;

            // ✅ 通过反射获取FileId,不依赖具体类型
            var fileIdProperty = imageInfo.GetType().GetProperty("FileId");
            if (fileIdProperty == null)
                return;
                
            var fileIdValue = fileIdProperty.GetValue(imageInfo);
            if (fileIdValue == null || !(fileIdValue is long fileId) || fileId <= 0)
                return;
        ...
        public virtual void AddImageInfos(IEnumerable<object> imageInfos)
        {
            if (imageInfos == null)
                return;
