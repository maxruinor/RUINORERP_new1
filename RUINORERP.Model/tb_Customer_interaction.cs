...
namespace RUINORERP.Model
{
    ...
    {
        public tb_Customer_interaction()
        {
            base.FieldNameList = fieldNameList;
            if (!PK_FK_ID_Check())
            {
                throw new Exception("tb_Customer_interaction" + "外键ID与对应主主键名称不一致。请修改数据库");
            }
        }
...
return rs;
        ...
        public override ConcurrentDictionary<string, string> FieldNameList
        {
            get
            {
                if (fieldNameList == null)
                {
                    ...
                    Type type = typeof(tb_Customer_interaction);
        ...
        {
            tb_Customer_interaction loctype = (tb_Customer_interaction)this.MemberwiseClone(); //创建当前对象的浅拷贝。
            return loctype;
        }
    }
}
