...
namespace RUINORERP.Model
{
    ...
    public partial class tb_FM_OtherExpenseDetail: BaseEntity, ICloneable
    {
        public tb_FM_OtherExpenseDetail()
        {
            ...
            if (!PK_FK_ID_Check())
            {
                throw new Exception("其它费用记录表，账户管理，财务系统中使用,像基础资料一样单表操作简单tb_FM_OtherExpenseDetail" + "外键ID与对应主主键名称不一致。请修改数据库");
            }
        }
        ...
        [SugarColumn(ColumnDataType = "bigint", SqlParameterDbType ="Int64",  ColumnName = "DepartmentID" , DecimalDigits = 0,IsNullable = true,ColumnDescription = "归属部门" )]
