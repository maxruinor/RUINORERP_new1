...
namespace RUINORERP.Model
{
    ...
    [Serializable()]
    [Description("应付款单")]
    [SugarTable("tb_FM_Payable")]
    public partial class tb_FM_Payable: BaseEntity, ICloneable
    {
        public tb_FM_Payable()
        {
            base.FieldNameList = fieldNameList;
            if (!PK_FK_ID_Check())
            {
                throw new Exception("应付款单tb_FM_Payable" + "外键ID与对应主主键名称不一致。请修改数据库");
            }
        }
        ...
        [SugarColumn(ColumnDataType = "bigint", SqlParameterDbType ="Int64",  ColumnName = "PayableID" , DecimalDigits = 0,IsNullable = false,ColumnDescription = "应付款单" , IsPrimaryKey = true)]
        public long PayableID
        { 
            get{return _PayableID;}
            set{
            SetProperty(ref _PayableID, value);
                base.PrimaryKeyID = _PayableID;
            }
        }
        ...
        /// 预付单
        /// </summary>
        [AdvQueryAttribute(ColName = "PrePayID",ColDesc = "预付单")] 
        [SugarColumn(ColumnDataType = "bigint", SqlParameterDbType ="Int64",  ColumnName = "PrePayID" , DecimalDigits = 0,IsNullable = true,ColumnDescription = "预付单" )]
        ...
        //[Browsable(false)]打印报表时的数据源会不显示
        [SugarColumn(IsIgnore = true)]
        [Navigate(NavigateType.OneToMany, nameof(tb_FM_PayableDetail.PayableID))]
        public virtual List<tb_FM_PayableDetail> tb_FM_PayableDetails { get; set; }
        //tb_FM_PayableDetail.PayableID)
        //PayableID.FK_FM_PAYABLEDETAIL_REF_FM_PAYABLE)
        //tb_FM_Payable.PayableID)
...
return rs;
        ...
        public override ConcurrentDictionary<string, string> FieldNameList
        {
            get
            {
                if (fieldNameList == null)
                {
                    ...
                    Type type = typeof(tb_FM_Payable);
        ...
        {
            tb_FM_Payable loctype = (tb_FM_Payable)this.MemberwiseClone(); //创建当前对象的浅拷贝。
            return loctype;
        }
    }
}
