...
namespace RUINORERP.Model
{
    ...
    /// </summary>
    [Serializable()]
    [Description("应付明细 如果一个采购订单多次送货时，采购入库单即可对应这里的明细")]
    [SugarTable("tb_FM_PayableDetail")]
    public partial class tb_FM_PayableDetail: BaseEntity, ICloneable
    {
        public tb_FM_PayableDetail()
        {
            base.FieldNameList = fieldNameList;
            if (!PK_FK_ID_Check())
            {
                throw new Exception("应付明细 如果一个采购订单多次送货时，采购入库单即可对应这里的明细tb_FM_PayableDetail" + "外键ID与对应主主键名称不一致。请修改数据库");
            }
        }


        #region 属性
        private long _PayableDetail_ID;
        /// <summary>
        /// 应付款单明细
        /// </summary>
 
        [SugarColumn(ColumnDataType = "bigint", SqlParameterDbType ="Int64",  ColumnName = "PayableDetail_ID" , DecimalDigits = 0,IsNullable = false,ColumnDescription = "应付款单明细" , IsPrimaryKey = true)]
        public long PayableDetail_ID
        { 
            get{return _PayableDetail_ID;}
            set{
            SetProperty(ref _PayableDetail_ID, value);
                base.PrimaryKeyID = _PayableDetail_ID;
            }
        }
        ...
        /// 应付款单
        /// </summary>
        [AdvQueryAttribute(ColName = "PayableID",ColDesc = "应付款单")] 
        [SugarColumn(ColumnDataType = "bigint", SqlParameterDbType ="Int64",  ColumnName = "PayableID" , DecimalDigits = 0,IsNullable = true,ColumnDescription = "应付款单" )]
        [FKRelationAttribute("tb_FM_Payable","PayableID")]
        ...
        [Navigate(NavigateType.OneToOne, nameof(PayableID))]
        public virtual tb_FM_Payable tb_fm_payable { get; set; }
...
return rs;
        ...
        public override ConcurrentDictionary<string, string> FieldNameList
        {
            get
            {
                if (fieldNameList == null)
                {
                    ...
                    Type type = typeof(tb_FM_PayableDetail);
        ...
        {
            tb_FM_PayableDetail loctype = (tb_FM_PayableDetail)this.MemberwiseClone(); //创建当前对象的浅拷贝。
            return loctype;
        }
    }
}
