...
namespace RUINORERP.Model
{
    ...
    public partial class tb_FM_PrePaymentReceipt: BaseEntity, ICloneable
    {
        public tb_FM_PrePaymentReceipt()
        {
            base.FieldNameList = fieldNameList;
            if (!PK_FK_ID_Check())
            {
                throw new Exception("预收款单tb_FM_PrePaymentReceipt" + "外键ID与对应主主键名称不一致。请修改数据库");
            }
        }
        ...
        [SugarColumn(ColumnDataType = "bigint", SqlParameterDbType ="Int64",  ColumnName = "PrePaymentReceiptID" , DecimalDigits = 0,IsNullable = false,ColumnDescription = "预收款单" , IsPrimaryKey = true)]
        public long PrePaymentReceiptID
        { 
            get{return _PrePaymentReceiptID;}
            set{
            SetProperty(ref _PrePaymentReceiptID, value);
                base.PrimaryKeyID = _PrePaymentReceiptID;
            }
        }
        ...
        [SugarColumn(ColumnDataType = "money", SqlParameterDbType ="Decimal",  ColumnName = "TotalPreAmount" , DecimalDigits = 4,IsNullable = true,ColumnDescription = "预收总金额" )]
        ...
        [SugarColumn(ColumnDataType = "money", SqlParameterDbType ="Decimal",  ColumnName = "UnassignedAmount" , DecimalDigits = 4,IsNullable = true,ColumnDescription = "剩余金额" )]
        ...
        //[Browsable(false)]打印报表时的数据源会不显示
        [SugarColumn(IsIgnore = true)]
        [Navigate(NavigateType.OneToMany, nameof(tb_FM_PaymentReceivable.PrePaymentReceiptID))]
        public virtual List<tb_FM_PaymentReceivable> tb_FM_PaymentReceivables { get; set; }
        //tb_FM_PaymentReceivable.PrePaymentReceiptID)
        //PrePaymentReceiptID.FK_FM_PAYMENTRECEIVABLE_REF_FM_PREPAYMENTRECEIPT)
        //tb_FM_PrePaymentReceipt.PrePaymentReceiptID)

        //[Browsable(false)]打印报表时的数据源会不显示
        [SugarColumn(IsIgnore = true)]
        [Navigate(NavigateType.OneToMany, nameof(tb_FM_PrePaymentReceiptDetail.PrePaymentReceiptID))]
        public virtual List<tb_FM_PrePaymentReceiptDetail> tb_FM_PrePaymentReceiptDetails { get; set; }
        //tb_FM_PrePaymentReceiptDetail.PrePaymentReceiptID)
        //PrePaymentReceiptID.FK_FM_PREPAYMNENTRECEIPTDETAIL_REF_FM_PREPAYMENTRECEIPT)
        //tb_FM_PrePaymentReceipt.PrePaymentReceiptID)
...
return rs;
        ...
        public override ConcurrentDictionary<string, string> FieldNameList
        {
            get
            {
                if (fieldNameList == null)
                {
                    ...
                    Type type = typeof(tb_FM_PrePaymentReceipt);
        ...
        {
            tb_FM_PrePaymentReceipt loctype = (tb_FM_PrePaymentReceipt)this.MemberwiseClone(); //创建当前对象的浅拷贝。
            return loctype;
        }
    }
}
