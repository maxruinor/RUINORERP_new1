    ...
    public class CacheHelper
    {
        ...
        */
        ...
        public void SetDictDataSource<T>(Expression<Func<T, long>> expkey, Expression<Func<T, string>> expvalue, bool LoadData = true) where T : class
        {
            ...
            {
                frmMain.Instance.PrintInfoLog("SetDictDataSource:" + tableName + "===>" + ex.Message);
            }
            stopwatch.Stop();
            frmMain.Instance.PrintInfoLog($"初始化SetDictDataSource: {tableName} 执行时间：{stopwatch.ElapsedMilliseconds} 毫秒");
        }
            ...
            foreach (string typeName in typeNames)
            {
                switch (typeName)
                {
                    ...
                    default:
                        ...
                        frmMain.Instance.PrintInfoLog("SetDictDataSource 未知类型" + typeName);
                        break;
                }
        ...
        public void InitDict(bool LoadData = true)
        {
            ...
            {
                frmMain.Instance.PrintInfoLog("InitDict" + ex.Message);
            }
        }
