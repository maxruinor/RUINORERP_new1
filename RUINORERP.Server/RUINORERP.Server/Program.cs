...
#pragma warning restore CS0168 // 声明了变量但从未使用
        ...
        private static void CreatSocketServer(IHost host)
        {
            ...
            .AddServer<ServiceforLander<LanderPackageInfo>, LanderPackageInfo, LanderCommandLinePipelineFilter>(builder =>
               ...
               //注册会话处理程序（连接和关闭Session事件）
               .UseSessionHandler(async (session) =>
               {
                   // sessionListLander.Add(session as SessionforLander);
                   // PrintMsg($"{DateTime.Now} [SessionforLander] Session-登录服务器 connected: {session.RemoteEndPoint}");
                   await Task.Delay(0);
               }, async (session, reason) =>
               {
                   //sessionListLander.Remove(session as SessionforLander);
                   // PrintMsg($"{DateTime.Now} [SessionforLander] Session-登录服务器 {session.RemoteEndPoint} closed: {reason}");
                   await Task.Delay(0);
               })
                ...
                commandOptions.AddCommand<LanderCommand>();
                   ...
                   /*
                           ...
                           .AddServer<ServiceforBiz<BizPackageInfo>, BizPackageInfo, BizPipelineFilter>(builder =>
                           {
                               ...
                               .UsePackageDecoder<MyPackageDecoder>()//注册自定义解码器
                               .UseSession<SessionforBiz>();
                           //注册会话处理程序（连接和关闭Session事件）
                           .UseSessionHandler(async (session) =>
                           {
                               sessionListBiz.TryAdd(session.SessionID, session as SessionforBiz);
                               PrintMsg($"{DateTime.Now} [SessionforBiz-业务服务器] Session connected: {session.RemoteEndPoint}");
                               await Task.Delay(0);
                           }, async (session, reason) =>
                           {
                               ...
                               //{
                               //   // SephirothServer.CommandServer.RoleService.角色离线(sg);
                               //}
                               PrintMsg($"{DateTime.Now} [SessionforBiz-业务服务器] Session {session.RemoteEndPoint} closed: {reason}");
                       ...
                       {
                           logging.ClearProviders(); //移除默认添加的日志提供程序
                           ...
                           if (isWindows)
                           {
                               ...
                               //logging.AddEventLog();//日志写入到事件查看器
                               logging.AddFile();
                               //logging.AddLog4Net();
                           }
                       }).UseLog4Net()
