...
namespace RUINORERP.Server.ServerSession
{
    ...
    /// - 会话状态管理已迁移到: RUINORERP.Server.Network.Models.SessionState
    ...
    public class SessionforBiz : AppSession
    {
        ...
        public virtual void AddSendData(OriginalData d)
        {
            ...
            {
                // 使用增强的通讯适配器
               // EncryptedData gde = LegacyCommAdapter.EncryptionServerPackToClient(d);
               // AddSendData(gde);
            }
            catch (Exception ex)
            {
        ...
        public virtual void AddSendData(byte cmd, byte[] one, byte[] two)
        {
            // 使用新的通讯组件构建数据
            ...
            try
            {
               // EncryptedData gde = LegacyCommAdapter.EncryptionServerPackToClient(gd);
               // AddSendData(gde);
            }
            catch (Exception ex)
            {
