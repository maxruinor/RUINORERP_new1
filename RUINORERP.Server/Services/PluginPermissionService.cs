...
namespace RUINORERP.Server.Services
{
    ...
    {
        // 模拟用户权限数据存储（使用ConcurrentDictionary保证线程安全）
        // 在实际应用中，这应该从数据库或缓存中获取
        private static readonly ConcurrentDictionary<string, HashSet<string>> UserPermissions = new ConcurrentDictionary<string, HashSet<string>>(
        ...
        public bool CheckPluginPermission(string username, string pluginPermissionKey)
        {
            ...
            // 检查用户是否存在
            if (!UserPermissions.TryGetValue(username, out var userPerms))
                return false;
        ...
        {
            if (string.IsNullOrEmpty(username) || !UserPermissions.TryGetValue(username, out var userPerms))
