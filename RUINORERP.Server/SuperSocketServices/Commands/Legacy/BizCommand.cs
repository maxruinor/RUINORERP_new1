...
{
    // 自定义CommandScheduler类来替换PacketSpec的CommandScheduler
    public class CommandScheduler
    {
        private readonly CommandDispatcher _commandDispatcher;
        private readonly ConcurrentBag<IServerCommand> _commandQueue;
        private readonly CancellationTokenSource _cancellationTokenSource;
        private Task _processingTask;
        private bool _isRunning = false;

        public CommandScheduler(RUINORERP.Server.CommandService.CommandDispatcher commandDispatcher)
        {
            _commandDispatcher = commandDispatcher;
            _commandQueue = new ConcurrentBag<IServerCommand>();
            _cancellationTokenSource = new CancellationTokenSource();
        }

        // 替换PacketSpec中的EnqueueCommand方法
        ...
        // 添加Add方法来兼容原有的代码
        public void Add(IServerCommand command)
        {
        ...
        public void StartProcessing()
        {
            ...
            _processingTask = Task.Run(async () =>
            {
                ...
                {
                    if (_commandQueue.TryTake(out IServerCommand command))
                    {
                        try
                        {
                            // 使用服务器端的命令分发器
                            await _commandDispatcher.DispatchAsync(new BlockingCollection<IServerCommand>(), command, _cancellationTokenSource.Token);
                        }
                        catch (Exception ex)
                        {
    ...
    // [Command(Key = unchecked((short) (0xFF * 256 + 0xFE)))]
      [Command(Key = "KXGame")]
      [Obsolete("此命令类已过时，请使用 Network/Commands/ 目录下的相应命令处理器替代", false)]
    public class BizCommand : IAsyncCommand<PacketModel>
    {
        ...
        public async ValueTask ExecuteAsync(IAppSession session, BizPackageInfo package, CancellationToken cancellationToken)
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    ClientCommand CleintCmd = (ClientCommand)gd.Cmd;// gp.GetClientCmd(gd);
                    ...
                    switch (CleintCmd)
                    {
                        ...
                        case ClientCommand.复合型登陆请求:
                            ...
                            MixLoginCommand.DataPacket = gd;
                            MixLoginCommand.RequestSession = fromPlayer;
                            _commandScheduler.EnqueueCommand(MixLoginCommand); // 修复方法调用
                        ...
                        case ClientCommand.更新缓存:
                            ...
                            break;
                        case ClientCommand.删除缓存:
                            UserService.接收删除缓存指令(fromPlayer, gd);
                            break;
                        case ClientCommand.请求协助处理:
                            SystemService.process请求协助处理(gd);
                            break;
                        //case ClientCommand.单据锁定:
                        ...
                        //    break;
                        //case ClientCommand.单据锁定释放:
                        //    SystemService.process单据审核锁定释放(fromPlayer, gd);
                        //    break;

                        case ClientCommand.复合型锁单处理:
                            // 创建一个命令实例 
                            //  实际收到一个转发指令
        ...
        private void InstructionService_OtherEvent(ClientCommand msg, OriginalData gd)
        {
...
#pragma warning restore CS0169 // 从不使用字段“BizCommand.Timer时间检测定时器”
        ...
        private void HeartbeatCmd(IAppSession session, BizPackageInfo package)
        {
            ...
            // 使用统一的包处理器替代PacketProcess
            var packetProcessor = new UnifiedPacketProcessor(PlayerSession);
            if (package != null)
            {
