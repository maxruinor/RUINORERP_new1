...
namespace RUINORERP.TopServer.UserStatusMonitoring
{
    ...
    {
        // 用户信息字典，键为服务器实例ID，值为该实例下的用户信息字典（键为用户ID）
        private ConcurrentDictionary<Guid, ConcurrentDictionary<long, UserInfo>> _userInfos;
        ...
        public UserManager()
        {
            _userInfos = new ConcurrentDictionary<Guid, ConcurrentDictionary<long, UserInfo>>();
        ...
        {
            if (_userInfos.TryGetValue(instanceId, out ConcurrentDictionary<long, UserInfo> users))
            {
                return users.Values;
            }
        ...
        public void AddOrUpdateUser(Guid instanceId, UserInfo userInfo)
        {
            ...
            // 获取或创建服务器实例的用户字典
            var users = _userInfos.GetOrAdd(instanceId, new ConcurrentDictionary<long, UserInfo>());
        ...
        public void RemoveUser(Guid instanceId, long userId)
        {
            if (userId <= 0)
            {
                return;
        ...
        {
            if (activityInfo == null || activityInfo.UserId <= 0)
            {
                return;
            }
