...
RUINORERP.TopServer 是 RUINORERP 系统的顶级管理服务器，负责管理所有业务服务器实例的注册、监控和管理。

具体的目标是管理好子服务器的注册情况，续费时间提醒，用户登录情况，使用情况和服务状态监控。
...
```
RUINORERP.UI (客户端) → RUINORERP.Server (业务服务器) → RUINORERP.TopServer (管理服务器)
```
