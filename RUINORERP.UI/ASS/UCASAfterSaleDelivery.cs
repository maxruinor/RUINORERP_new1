...
namespace RUINORERP.UI.ASS
{
    ...
    public partial class UCASAfterSaleDelivery : BaseBillEditGeneric<tb_AS_AfterSaleDelivery, tb_AS_AfterSaleDeliveryDetail>, IPublicEntityObject
    {
        ...
        protected async override Task<bool> AntiCloseCaseAsync()
        {
            ...
            // 使用增强后的通用意见窗体（反结案）
            CommonUI.frmGenericOpinion<tb_AS_AfterSaleDelivery> frm = new CommonUI.frmGenericOpinion<tb_AS_AfterSaleDelivery>();
            frm.FormTitle = "售后发货单反结案确认";
            ...
            if (frm.ShowDialog() == DialogResult.OK)
            {
                ...
                if (rs.Succeeded)
                {
                    ...
                    await MainForm.Instance.AuditLogHelper.CreateAuditLog<tb_AS_AfterSaleDelivery>("反结案", EditEntity, $"反结案意见:{EditEntity.Notes}");
                    Refreshs();
                }
                else
                {
        ...
        protected async override Task<bool> CloseCaseAsync()
        {
            ...
            if (frm.ShowDialog() == DialogResult.OK)
            {
                ...
                if (rs.Succeeded)
                {
                    ...
                    //OriginalData od = ActionForClient.工作流审批(pkid, (int)BizType.盘点单, ae.ApprovalResults, ae.ApprovalComments);
                    //MainForm.Instance.ecs.AddSendData(od);
                await    MainForm.Instance.AuditLogHelper.CreateAuditLog<tb_AS_AfterSaleDelivery>("结案", EditEntity, $"结案意见:{EditEntity.Notes}");
                    Refreshs();
                }
                else
                {
