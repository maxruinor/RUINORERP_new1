...
namespace RUINORERP.UI.ATechnologyStack.AOP
{
    /// <summary>
    /// 拦截器BlogLogAOP 继承IInterceptor接口
    /// </summary>
    public class DbBizLogAOP : IInterceptor
    {
        public string UserName { get; set; }

        public delegate void ShowLogDelegate(Exception ex);
        /// <summary>
        /// 验证数据
        /// </summary>
        ...
        public DbBizLogAOP(ApplicationContext apeContext)
        {
            _appContext = apeContext;
        }


        //public DbBizLogAOP(string _userName)
        //{
        //    userName = _userName;
        //}
        ...
        {
            UserName = _appContext?.CurUserInfo?.UserName;
            string json;
            try
            {
            ...
            catch (Exception ex)// 同步2
            {
                ...
                //}
                MainForm.Instance.uclog.AddLog(UserName + "db数据异常：" + ex.Message, Global.UILogType.错误);
                if (OnShowLog != null)
                {
                    OnShowLog(ex);
                }
