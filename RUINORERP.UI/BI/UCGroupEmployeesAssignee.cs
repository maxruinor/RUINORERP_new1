...
{
    public partial class UCGroupEmployeesAssignee : KryptonForm
    {
        public UCGroupEmployeesAssignee()
        {
            InitializeComponent();
        }
        ...
        private async void LoadGroups()
        {
            ...
            var lambda = Expressionable.Create<tb_ProjectGroup>()
                             ...
                             .And(t => t.ProjectGroup_ID == ProjectGroupID)
        ...
        {
            List<tb_ProjectGroupEmployees> list = new List<tb_ProjectGroupEmployees>();
            foreach (DataGridViewRow dr in dataGridView1.Rows)
            {
                object obj = dr.DataBoundItem;
                if (obj is tb_ProjectGroupEmployees groupEmployees)
                {
                    if ((groupEmployees.ProjectGroupEmpID == 0 && groupEmployees.Assigned) || (groupEmployees.ProjectGroupEmpID > 0))
                    {
                        list.Add(groupEmployees);
                    }
                }
            }

            //批量更新 批量插入 如果存在则更新，不存在则更新
            await MainForm.Instance.AppContext.Db.Insertable(list.Where(c => c.ProjectGroupEmpID == 0).ToList()).ExecuteReturnSnowflakeIdListAsync();
            await MainForm.Instance.AppContext.Db.Updateable(list.Where(c => c.ProjectGroupEmpID == 0).ToList()).ExecuteCommandAsync();
