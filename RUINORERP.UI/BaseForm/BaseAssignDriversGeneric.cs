...
namespace RUINORERP.UI.SysConfig
{
    ...
    /// R为关系品，Resource：权限， 
    /// Assignee：员工，被分配对象
    public partial class BaseAssignDriversGeneric<Assignee, Resource, Relationship> : BaseAssignDrivers where Assignee : class 
        where Resource : class
           where Relationship : class
    {
        public BaseAssignDriversGeneric()
        {
            InitializeComponent();
        }

        BaseController<Resource> ctrRole = Startup.GetFromFacByName<BaseController<Resource>>(typeof(Resource).Name + "Controller");
        
        /// <summary>
        /// 加载用户
        /// 加载被分配的对象
        /// </summary>
        private async void LoadAssignee()
        {
            TreeView1.Nodes.Clear();
            BaseController<Assignee> ctr = Startup.GetFromFacByName<BaseController<Assignee>>(typeof(Assignee).Name + "Controller");
            var list = await ctr.BaseQueryAsync();
            foreach (var item in list)
            {
                ...
                long pkid = (long)ReflectionHelper.GetPropertyValue(item, PKCol);
                string colName = UIHelper.ShowGridColumnsNameValue<Assignee>(PKCol, pkid);
                NodeText = colName;
                //if (item.tb_employee != null)
                //{
        ...
        private void UCUserAuthorization_Load(object sender, EventArgs e)
        {
            ...
            // FieldNameList = UIHelper.GetFieldNameColList(typeof(SelectDto), typeof(Resource));
            dataGridView1.FieldNameList = UIHelper.GetFieldNameColList(typeof(Relationship));
            dataGridView1.ColumnHeadersDefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter;
            //属性–Columns–DefaultCellStyle–Alignment–MiddleRight
            //这里是不是与那个缓存 初始化时的那个字段重复？
            ///显示列表对应的中文
            // FieldNameList = UIHelper.GetFieldNameList<Resource>();
            //重构？
            dataGridView1.NeedSaveColumnsXml = true;
            dataGridView1.XmlFileName = "UCUserAuthorization";
            InitListData();
            /*
            combinedType = Common.EmitHelper.MergeTypesNew(typeof(SelectDto), typeof(Resource));
            object InstObj = Activator.CreateInstance(combinedType);
            List<object> objlist = new List<object>();
            objlist.Add(InstObj);*/
            bindingSourceList.DataSource = new List<Relationship>();
        ...
        private async void TreeView1_AfterSelect(object sender, TreeViewEventArgs e)
        {
            ...
            BaseController<Relationship> ctr = Startup.GetFromFacByName<BaseController<Relationship>>(typeof(Relationship).Name + "Controller");

   
            e.Node.Checked = true;
            node_AfterCheck(sender, e);
            List<Resource> listRole = await ctrRole.BaseQueryAsync();
            List<Relationship> listUR = await ctr.BaseQueryAsync();
            Assignee user = TreeView1.SelectedNode.Tag as Assignee;
            user.Relationships = new List<Relationship>();
            foreach (Resource item in listRole)
            {
                var urnew = listUR.FirstOrDefault(c => c.RoleID == item.RoleID && c.User_ID == user.User_ID);
                if (urnew == null)
                {
                    //没有添加到的显示一下，给保存修改准备数据
                    urnew = new Relationship();
                    urnew.User_ID = user.User_ID;
                    urnew.RoleID = item.RoleID;
                    urnew.Authorized = false;
                    urnew.DefaultRole = false;
                    user.Relationships.Add(urnew);
                }
                else
                {
                    user.Relationships.Add(urnew);
            ...
            bindingSourceList.DataSource = user.Relationships;
            dataGridView1.DataSource = bindingSourceList;
        ...
        private async void btnSave_Click(object sender, EventArgs e)
        {
            ...
            //  List<Relationship> urs = new List<Relationship>();
            foreach (DataGridViewRow dr in dataGridView1.Rows)
            {
                object obj = dr.DataBoundItem;
                if (obj is Relationship ur)
                {
                    //urs.Add(ur);
                    ReturnResults<Relationship> rr = await ctr.BaseSaveOrUpdate(ur);
                }
                /*
                ...
                if (TreeView1.SelectedNode.Checked)
                {
                    ...
                    //如果这个人名下有这个角色就跳过
                    // bool hasRole = user.Relationships != null && user.Relationships.Where(r => r.RoleID == obj.GetPropertyValue<Relationship>(s => s.RoleID).ToLong()).Any();
                    var ur = user.Relationships.FirstOrDefault(c => c.RoleID == obj.GetPropertyValue<Relationship>(s => s.RoleID).ToLong() && c.User_ID == user.User_ID);
                    if (ur == null)
                    {
                        ur = new Relationship();
                        ur.Authorized = obj.GetPropertyValue<SelectDto>(s => s.UserSelect).ToBool();
                        ur.User_ID = user.User_ID;
                        ur.RoleID = obj.GetPropertyValue<Relationship>(s => s.RoleID).ToLong();
                        await ctr.AddReEntityAsync(ur);
                        user.Relationships.Add(ur);
                    }
                    else
                    {
                        ur.Authorized = obj.GetPropertyValue<SelectDto>(s => s.UserSelect).ToBool();
                        ...
                        //
                        /*
                         bool hasRole = user.Relationships != null && user.Relationships.Where(r => r.Authorized).Any();
                        if (hasRole)
                        {
                            continue;
                        }
                        else
                        {
                            ...
                            {
                                Relationship ur = new Relationship();
                                ur.User_ID = user.User_ID;
                                ur.RoleID = obj.GetPropertyValue<Relationship>(s => s.RoleID).ToLong();
                                ur.Authorized = true;
                                await ctr.AddReEntityAsync(ur);
        ...
        private void DataGridView1_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            ...
            //动态字典值显示
            string colName = UIHelper.ShowGridColumnsNameValue<Relationship>(colDbName, e.Value);
        ...
        private void chkALL_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                object obj = dr.DataBoundItem;
               // obj.SetPropertyValue<Relationship>(s => s.Authorized, chkALL.Checked);
            }
        ...
        {
            string tableName = typeof(Relationship).Name;
            foreach (var field in typeof(Relationship).GetProperties())
            {
                //获取指定类型的自定义特性
        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            //指定要控制的列名
            //Expression<Func<Relationship, object>> expSelected = c => c.DefaultRole;
