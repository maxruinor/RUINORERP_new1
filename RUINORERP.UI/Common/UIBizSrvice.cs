...
namespace RUINORERP.UI.Common
{
    public static class UIBizSrvice
    {
        ...
        private static async Task SaveGridSettingsAsync(tb_UIMenuPersonalization menuPersonalization, tb_UIGridSetting gridSetting, List<SGColDisplayHandler> columnDisplays)
        {
            ...
            // 执行比较（使用之前提供的ColumnComparer类）
            //var result = ColumnComparer.CompareColumns<SGColDisplayHandler>(oldColumns, newColumns);
            var result = oldColumns.CompareColumns(newColumns);
            if (gridSetting.UIGID == 0)
            {
                gridSetting.ColsSetting = json;
    ...
    {
        // 比较两个列配置列表，返回差异结果
        public static ColumnComparisonResult CompareColumns(
            List<ColDisplayController> oldColumns,
            List<ColDisplayController> newColumns)
        {
            var result = new ColumnComparisonResult();

            // 使用哈希表加速查找
            var oldColumnLookup = oldColumns.ToDictionary(c => c.ColName);
            var newColumnLookup = newColumns.ToDictionary(c => c.ColName);

            // 查找新增的列
            result.AddedColumns = newColumns
                .Where(c => !oldColumnLookup.ContainsKey(c.ColName))
                .ToList();

            // 查找已删除的列
            result.RemovedColumns = oldColumns
                .Where(c => !newColumnLookup.ContainsKey(c.ColName))
                .ToList();

            // 查找名称相同但属性不同的列
            foreach (var newCol in newColumns)
            {
                if (oldColumnLookup.TryGetValue(newCol.ColName, out var oldCol))
                {
                    var differences = CompareColumnProperties(oldCol, newCol);
                    if (differences.Any())
                    {
                        result.ModifiedColumns.Add(new ModifiedColumn
                        {
                            ColumnName = newCol.ColName,
        ...
        // 比较两个列的具体属性差异
        private static List<PropertyDifference> CompareColumnProperties(
            ColDisplayController oldCol,
            ColDisplayController newCol)
        {
            var differences = new List<PropertyDifference>();

            // 比较所有需要检查的属性
            AddDifferenceIfChanged(differences, nameof(ColDisplayController.ColDisplayText), oldCol.ColDisplayText, newCol.ColDisplayText);
            AddDifferenceIfChanged(differences, nameof(ColDisplayController.IsFixed), oldCol.IsFixed, newCol.IsFixed);
            AddDifferenceIfChanged(differences, nameof(ColDisplayController.ColDisplayIndex), oldCol.ColDisplayIndex, newCol.ColDisplayIndex);
            AddDifferenceIfChanged(differences, nameof(ColDisplayController.ColWidth), oldCol.ColWidth, newCol.ColWidth);
            AddDifferenceIfChanged(differences, nameof(ColDisplayController.Visible), oldCol.Visible, newCol.Visible);
            AddDifferenceIfChanged(differences, nameof(ColDisplayController.Disable), oldCol.Disable, newCol.Disable);
            AddDifferenceIfChanged(differences, nameof(ColDisplayController.IsPrimaryKey), oldCol.IsPrimaryKey, newCol.IsPrimaryKey);
