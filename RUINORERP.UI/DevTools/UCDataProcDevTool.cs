...
namespace RUINORERP.UI.DevTools
{
    ...
    [MenuAttrAssemblyInfo( "字符处理", ModuleMenuDefine.模块定义.系统设置, ModuleMenuDefine.系统设置.系统工具)]
    public partial class UCDataProcDevTool : UserControl
    {
        public UCDataProcDevTool()
        {
            InitializeComponent();
            _main = this;
        ...
        private static UCDataProcDevTool _main;
        internal static UCDataProcDevTool Instance
        {
            get { return _main; }
        }
        ...
        private void UCDataProcDevTool_Load(object sender, EventArgs e)
        {
            if (rdbMySQL.Checked)
            {
                IsMysql = true;
            ...
            Modules.Add("智能打包处理");
            RUINORERP.UI.Common.ComboBoxHelper.InitDropList(Modules, cmbProcessTemplates, true);
            //加载处理的方法模块
            ...
            RUINORERP.UI.Common.ComboBoxHelper.InitDropList(typelist, cmbtables, false);
        ...
        private void LoadProcessColumns()
        {
            ...
            RUINORERP.UI.Common.ComboBoxHelper.InitDropList(tclist, cmbColumns, true);
        ...
        public void Query()
        {
            ...
            if (cmbtables.SelectedItem != null && cmbtables.SelectedItem.ToString() != "请选择")
            {
                ...
                RUINORERP.UI.MainForm.Instance.PrintInfoLog("查询内容表行数:" + dsContent.Tables[0].Rows.Count.ToString());
            }
        ...
        public string ProcessForDecimal(string sourceString, bool isSmart, string times)
        {
            ...
            {
                RUINORERP.UI.MainForm.Instance.PrintInfoLog("当前选择的字段值，转换为 数字类型时出错。");
                return sourceString;
            }
            ...
            {
                #region 智能分析
                RUINORERP.UI.MainForm.Instance.PrintInfoLog("当前没有实现这个方法，请使用手工指定的方式。");
                return sourceString;
                #endregion
            }
            else
            {
                ...
                {
                    RUINORERP.UI.MainForm.Instance.PrintInfoLog("手工指定的倍数必须为int数值型。");
                    return sourceString;
                }
        ...
        void uchtml_OtherEvent(UCHTMLStructuralAnalysis uc, object Parameters)
        {
            ...
            if (dataGridView1.SelectedRows != null && dataGridView1.SelectedRows.Count > 0)
            {
                ...
                RUINORERP.UI.MainForm.Instance.PrintInfoLog("数据处理完成," + "消耗时间：" + span.TotalSeconds.ToString() + "秒", Color.Green);
            }
            else
            {
        ...
        private void btnProcess_Click(object sender, EventArgs e)
        {
            ...
            if (dataGridView1.SelectedRows != null && dataGridView1.SelectedRows.Count > 0)
            {
                ...
                RUINORERP.UI.MainForm.Instance.PrintInfoLog(sbForLog.ToString(), Color.Red);
                RUINORERP.UI.MainForm.Instance.PrintInfoLog("数据处理完成,共" + sqlList.Count + "行。", Color.Green);
            }
            else
            {
            ...
            if (dataGridView1.SelectedRows != null)
            {
                if (MessageBox.Show("本操作是不能恢复的,同时对应产品图片也会删除？", "删除确认", MessageBoxButtons.YesNo, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
                {
                    ...
                    foreach (string item in ImagesPath)
                    {
                        ...
                        RUINORERP.UI.MainForm.Instance.PrintInfoLog("成功删除图片：" + item);
                    }
        ...
        private void cmbColumns_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cmbColumns.SelectedItem != null)
            {
                ...
                RUINORERP.UI.MainForm.Instance.PrintInfoLog(cmbColumns.SelectedItem.ToString() + ",已经在剪贴版中.");
            }
        }

        private void 复制产品描述ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (dataGridView1.CurrentRow != null)
            {
