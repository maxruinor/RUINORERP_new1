    ...
    public partial class UCFMStatementQuery : BaseBillQueryMC<tb_FM_Statement, tb_FM_StatementDetail>
    {
        ...
        private async void NewSumDataGridView_转为收付款单(object sender, EventArgs e)
        {
            ...
            foreach (var item in selectlist)
            {
                ...
                //只有审核状态才可以转换为收款单
                bool canConvert = item.StatementStatus == (int)StatementStatus.确认 && item.ApprovalStatus == (int)ApprovalStatus.审核通过 && item.ApprovalResults.HasValue && item.ApprovalResults.Value;
