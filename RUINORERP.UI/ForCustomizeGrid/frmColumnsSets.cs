...
namespace RUINORERP.UI.ForCustomizeGrid
{
    public partial class frmColumnsSets : KryptonForm
    {
        ...
        /// 显示列的顺序
        /// </summary>
       // public List<KeyValuePair<string, int>> ColOrder { get; set; } = new List<KeyValuePair<string, int>>();
       ...
       // public ConcurrentDictionary<string, KeyValuePair<string, bool>> FieldNameList { get => _FieldNameList; set => _FieldNameList = value; }
        ...
        private void frmColumnsSets_Load(object sender, EventArgs e)
        {
            ...
            */
            ...
            //{
            //    if (FieldNameList.ContainsKey(item.Key) && FieldNameList[item.Key].Key.Trim().Length > 0)
            //    {
            //        //listView1.Items.Insert(0, new ListViewItem(item.Key.ToString()));
            //        ListViewItem lvi = new ListViewItem();
            //        KeyValuePair<string, bool> kv;
            //        FieldNameList.TryGetValue(item.Key, out kv);
