    ...
    public partial class FrmLogin : Krypton.Toolkit.KryptonForm
    {
        ...
        private async Task InitializeConnectionAndWelcomeFlowAsync()
        {
            ...
            try
            {
                ...
                // 使用CancellationTokenSource实现可取消的连接操作
                using (var cts = new CancellationTokenSource(TimeSpan.FromSeconds(10)))
                {
                    try
                    {
                        bool connectResult = await connectionManager.ConnectAsync(serverIP, serverPort, cts.Token);
                        ...
                        var completedTask = await Task.WhenAny(welcomeTask, Task.Delay(welcomeTimeout, cts.Token));
        ...
        private async void btnok_Click(object sender, EventArgs e)
        {
            ...
            await Task.Run(async () =>
            {
                ...
                catch (Exception ex)
                {
                    ...
                    if (this.InvokeRequired)
                    {
                        this.Invoke(new Action(() => HandleLoginException(ex)));
                    }
                    else
                    {
        ...
        private async Task PerformLoginAsync()
        {
            ...
            try
            {
                using (StatusBusy busy = new StatusBusy("正在登录..."))
                {
                    ...
                    {
                        ShowErrorMessage("端口号格式不正确，请检查服务器配置。", "配置错误");
                        return;
                    }
                    ...
                    try
                    {
                        (loginSucceed, isInitPwd) = await PTPrincipal.Login(txtUserName.Text, txtPassWord.Text, Program.AppContextData);
            
                        if (!loginSucceed)
                        {
                            ShowErrorMessage("账号密码有误", "登录错误");
                            return;
                        }
                    }
                    catch (Exception ex)
                    {
                        ...
                        {
                            ShowErrorMessage("数据库连接失败，请检查 appsettings.json 中的连接字符串配置。", "数据库连接错误");
                        }
                        else
                        {
                            ShowErrorMessage($"登录验证失败：{ex.Message}", "登录错误");
                        }
                        return;
                    }
            
                    // 如果是超级管理员且为 admin 用户，直接完成登录
                    ...
                    // 验证连接状态（关键修复：添加连接状态验证）
                    if (!connectionManager.IsConnected)
                    {
                        ShowErrorMessage("服务器连接失败，请检查网络连接和服务器状态。", "连接错误");
                        return;
                    }
            
                    // 验证欢迎流程是否完成（关键修复：添加欢迎流程验证）
                    if (!_welcomeCompleted)
                    {
                        ShowErrorMessage("服务器欢迎流程未完成，请稍后重试或联系管理员。", "系统错误");
                        return;
                    }
        ...
        private void HandleLoginException(Exception ex)
        {
            ...
            ShowErrorMessage(errorMessage, "登录错误");
        }
        ...
        /// <returns>是否为数据库连接错误</returns>
        private bool IsDatabaseConnectionError(Exception ex)
        {
            // 检查异常消息中是否包含数据库连接相关的关键词
            string[] databaseErrorKeywords = {
                "database", "数据库", "connection", "连接", 
                "sql", "mssql", "sqlserver", "sql server",
                "timeout", "超时", "network", "网络",
                "provider", "提供程序", "connection string", "连接字符串",
                "login failed", "登录失败", "cannot open", "无法打开"
            };

            string errorMessage = ex.Message.ToLower();
            string stackTrace = ex.StackTrace?.ToLower() ?? "";

            // 检查异常消息和堆栈跟踪中是否包含数据库错误关键词
            foreach (string keyword in databaseErrorKeywords)
            {
                if (errorMessage.Contains(keyword) || stackTrace.Contains(keyword))
                {
                    return true;
                }
            }
            ...
            {
                return IsDatabaseConnectionError(ex.InnerException);
            }
        ...
        private async Task HandleLoginSuccess(LoginResponse loginResponse, bool isInitPwd)
        {
            try
            {
                ...
                // 执行系统更新检测（在后台线程中执行，避免阻塞登录流程）
                await Task.Run(async () =>
                {
                    try
                    {
                        MainForm.Instance.logger?.LogDebug("开始执行系统更新检测...");
                        
                        // 使用较短的超时时间（5秒）避免阻塞登录流程
                        using (var cts = new CancellationTokenSource(TimeSpan.FromSeconds(5)))
                        {
                            var updateTask = MainForm.Instance.UpdateSys(false, false);
                            var completedTask = await Task.WhenAny(updateTask, Task.Delay(TimeSpan.FromSeconds(5), cts.Token));
                            
                            if (completedTask == updateTask)
                            {
                                bool updateResult = await updateTask;
                                MainForm.Instance.logger?.LogDebug("系统更新检测完成，结果: {UpdateResult}", updateResult);
                            }
                            else
                            {
                                MainForm.Instance.logger?.LogDebug("系统更新检测超时，继续登录流程");
                            }
                        }
                    }
                    catch (Exception ex)
                    {
                        MainForm.Instance.logger?.LogWarning(ex, "系统更新检测过程中发生异常，但不影响登录流程");
                    }
                });
