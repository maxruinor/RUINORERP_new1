...
namespace RUINORERP.UI.MRP.PPLAN
{
    [MenuAttrAssemblyInfo("生产计划统计", ModuleMenuDefine.模块定义.生产管理, ModuleMenuDefine.生产管理.制造规划, BizType.生产计划统计)]
    public partial class UCProductionPlanStatistics : BaseNavigatorGeneric<View_ProductionPlanItems, View_ProductionPlanItems>
    {
        public UCProductionPlanStatistics()
        {
            InitializeComponent();
            //生成查询条件的相关实体 是不是也一个组，主子表呢？
            ReladtedEntityType = typeof(View_ProductionPlanItems);
        ...
        private void UCSaleOutStatistics_Load(object sender, EventArgs e)
        {
            ...
            //不能在上面的构造函数里面初始化
            base._UCMasterQuery.GridRelated.SetRelatedInfo<View_ProductionPlanItems,tb_SaleOrder>(c => c.SaleOrderNo, r => r.SOrderNo);
            base._UCMasterQuery.GridRelated.SetRelatedInfo<View_ProductionPlanItems, tb_ProductionPlan>(c => c.PPNo, r => r.PPNo);
            ...
            base._UCMasterQuery.ColDisplayTypes.Add(typeof(tb_ProductionPlan));
            base._UCMasterQuery.ColDisplayTypes.Add(typeof(tb_ProductionPlanDetail));
        ...
        {
            BaseProcessor baseProcessor = Startup.GetFromFacByName<BaseProcessor>(typeof(View_ProductionPlanItems).Name + "Processor");
