    ...
    public partial class UCManufacturingOrder : BaseBillEditGeneric<tb_ManufacturingOrder, tb_ManufacturingOrderDetail>
    {
        ...
        private void Sgh_OnCalculateColumnValue(object rowObj, SourceGridDefine griddefine, SourceGrid.Position Position)
        {
            ...
            try
            {
                ...
                //    //自制
                //}
                EditEntity.TotalMaterialCost = details.Sum(c => c.SubtotalUnitCost).ToRoundDecimalPlaces(precision);
                EditEntity.TotalProductionCost = (EditEntity.TotalMaterialCost + EditEntity.ApportionedCost + EditEntity.TotalManuFee).ToRoundDecimalPlaces(precision);
            }
            catch (Exception ex)
            {
...
*/
        ...
        */
        ...
        protected async override Task<bool> CloseCaseAsync()
        {
            ...
            //已经审核的并且通过的情况才能结案
            List<tb_ManufacturingOrder> needCloseCases = EditEntitys.Where(c => c.DataStatus == (int)DataStatus.确认 && c.ApprovalStatus == (int)ApprovalStatus.审核通过 && c.ApprovalResults.HasValue && c.ApprovalResults.Value).ToList();
