...
namespace RUINORERP.UI.MRP.MP
{
    ...
    [MenuAttrAssemblyInfo("生产领料单查询", ModuleMenuDefine.模块定义.生产管理, ModuleMenuDefine.生产管理.制程生产, BizType.生产领料单)]
    public partial class UCMaterialRequisitionQuery : BaseBillQueryMC<tb_MaterialRequisition, tb_MaterialRequisitionDetail>
    {
        public UCMaterialRequisitionQuery()
        {
            InitializeComponent();
        ...
        {
            _UCBillMasterQuery.GridRelated.SetRelatedInfo<tb_MaterialRequisition, tb_ManufacturingOrder>(c => c.MONO, r => r.MONO);
            base.SetGridViewDisplayConfig();
        }
        public override void BuildColNameDataDictionary()
        {
            //固定值也包括枚举值,也可以将没有缓存的提前查询出来给
            System.Linq.Expressions.Expression<Func<tb_MaterialRequisition, int?>> exprDataStatus;
            ...
            System.Linq.Expressions.Expression<Func<tb_MaterialRequisition, int?>> exprApprovalStatus;
            ...
            System.Linq.Expressions.Expression<Func<tb_MaterialRequisitionDetail, long>> expProdDetailID;
        ...
        /// </summary>
        /// <returns></returns>
        public async override Task<ApprovalEntity> Review(List<tb_MaterialRequisition> EditEntitys)
        {
            if (EditEntitys == null)
            {
                return null;
            ...
            //如果已经审核并且通过，则不能重复审核
            List<tb_MaterialRequisition> needApprovals = EditEntitys.Where(
        ...
        /// <returns></returns>
        public async override Task<bool> ReReview(List<tb_MaterialRequisition> EditEntitys)
        {
            if (EditEntitys == null)
            {
                return false;
            }
            foreach (tb_MaterialRequisition EditEntity in EditEntitys)
            {
                #region 反审
                //反审，要审核过，并且通过了，才能反审。
        ...
        */
        ...
        public async override Task<bool> CloseCase(List<tb_MaterialRequisition> EditEntitys)
        {
            if (EditEntitys == null)
            {
                return false;
            ...
            //已经审核的并且通过的情况才能结案
            List<tb_MaterialRequisition> needCloseCases = EditEntitys.Where(c => c.DataStatus == (int)DataStatus.确认 && c.ApprovalStatus == (int)ApprovalStatus.审核通过 && c.ApprovalResults.HasValue && c.ApprovalResults.Value).ToList();
