...
namespace RUINORERP.UI.MRP.MP
{
    ...
    public partial class UCMaterialReturn : BaseBillEditGeneric<tb_MaterialReturn, tb_MaterialReturnDetail>
    {
        ...
        public override async void BindData(tb_MaterialReturn entityPara, ActionStatus actionStatus)
        {
            ...
            if (entity != null)
            {
                ...
                else
                {
                    ...
                    if (entity.BillNo.IsNullOrEmpty())
                    {
                        ...
                        entity.BillNo = await bizCodeService.GenerateBizBillNoAsync(BizType.生产退料单);
                    }
        ...
        private void UcSaleOrderEdit_Load(object sender, EventArgs e)
        {
            ...
            sgd.SetDependencyObject<ProductSharePart, tb_MaterialReturnDetail>(MainForm.Instance.View_ProdDetailList);
        ...
        protected async override Task<bool> CloseCaseAsync()
        {
            ...
            //已经审核的并且通过的情况才能结案
            List<tb_MaterialReturn> needCloseCases = EditEntitys.Where(c => c.DataStatus == (int)DataStatus.确认 && c.ApprovalStatus == (int)ApprovalStatus.审核通过 && c.ApprovalResults.HasValue && c.ApprovalResults.Value).ToList();
