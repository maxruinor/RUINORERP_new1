...
namespace RUINORERP.UI.MRP.PQC
{
    ...
    [MenuAttrAssemblyInfo("返工入库单查询", ModuleMenuDefine.模块定义.生产管理, ModuleMenuDefine.生产管理.生产品控, BizType.返工入库单)]
    public partial class UCMRPReworkEntryQuery : BaseBillQueryMC<tb_MRP_ReworkEntry, tb_MRP_ReworkEntryDetail>
    {
        public UCMRPReworkEntryQuery()
        {
            InitializeComponent();
        ...
        {
            _UCBillMasterQuery.GridRelated.SetRelatedInfo<tb_MRP_ReworkEntry,tb_MRP_ReworkReturn>(c => c.ReworkReturnID, r => r.ReworkReturnNo);
        ...
        public override void BuildColNameDataDictionary()
        {
            ...
            System.Linq.Expressions.Expression<Func<tb_MRP_ReworkEntryDetail, long>> expProdDetailID;
        ...
        {
            //创建表达式
            var lambda = Expressionable.Create<tb_MRP_ReworkEntry>()
        ...
        /// </summary>
        /// <returns></returns>
        public async override Task<ApprovalEntity> Review(List<tb_MRP_ReworkEntry> EditEntitys)
        {
            if (EditEntitys == null)
            {
                return null;
            ...
            //如果已经审核并且通过，则不能重复审核
            List<tb_MRP_ReworkEntry> needApprovals = EditEntitys.Where(
            ...
            tb_MRP_ReworkEntryController<tb_MRP_ReworkEntry> ctr = Startup.GetFromFac<tb_MRP_ReworkEntryController<tb_MRP_ReworkEntry>>();
        ...
        /// <returns></returns>
        public async override Task<bool> ReReview(List<tb_MRP_ReworkEntry> EditEntitys)
        {
            if (EditEntitys == null)
            {
                return false;
            }
            foreach (tb_MRP_ReworkEntry EditEntity in EditEntitys)
            {
                #region 反审
                //反审，要审核过，并且通过了，才能反审。
                ...
                if (EditEntity.tb_MRP_ReworkEntryDetails == null || EditEntity.tb_MRP_ReworkEntryDetails.Count == 0)
                {
                ...
                tb_MRP_ReworkEntry oldobj = CloneHelper.DeepCloneObject<tb_MRP_ReworkEntry>(EditEntity);
                command.UndoOperation = delegate ()
                {
                    //Undo操作会执行到的代码 意思是如果退审核，内存中审核的数据要变为空白（之前的样子）
                    CloneHelper.SetValues<tb_MRP_ReworkEntry>(EditEntity, oldobj);
                };

                tb_MRP_ReworkEntryController<tb_MRP_ReworkEntry> ctr = Startup.GetFromFac<tb_MRP_ReworkEntryController<tb_MRP_ReworkEntry>>();
                List<tb_MRP_ReworkEntry> list = new List<tb_MRP_ReworkEntry>();
                list.Add(EditEntity);
        ...
        public async override Task<bool> CloseCase(List<tb_MRP_ReworkEntry> EditEntitys)
        {
            if (EditEntitys == null)
            {
                return false;
            ...
            //已经审核的并且通过的情况才能结案
            List<tb_MRP_ReworkEntry> needCloseCases = EditEntitys.Where(c => c.DataStatus == (int)DataStatus.确认 && c.ApprovalStatus == (int)ApprovalStatus.审核通过 && c.ApprovalResults.HasValue && c.ApprovalResults.Value).ToList();
            ...
            if (rs.Succeeded)
            {
                ...
                {
                    MainForm.Instance.AuditLogHelper.CreateAuditLog<tb_MRP_ReworkEntry>("结案成功", needCloseCases[i]);
                }
                //这里审核完了的话，如果这个单存在于工作流的集合队列中，则向服务器说明审核完成。
                //这里推送到审核，启动工作流  队列应该有一个策略 比方优先级，桌面不动1 3 5分钟 
                //OriginalData od = ActionForClient.工作流审批(pkid, (int)BizType.盘点单, ae.ApprovalResults, ae.ApprovalComments);
