...
namespace RUINORERP.UI.MRP.PQC
{
    ...
    public partial class UCMRPReworkReturnQuery : BaseBillQueryMC<tb_MRP_ReworkReturn, tb_MRP_ReworkReturnDetail>
    {
        ...
        public async override Task<bool> CloseCase(List<tb_MRP_ReworkReturn> EditEntitys)
        {
            ...
            //已经审核的并且通过的情况才能结案
            List<tb_MRP_ReworkReturn> needCloseCases = EditEntitys.Where(c => c.DataStatus == (int)DataStatus.确认 && c.ApprovalStatus == (int)ApprovalStatus.审核通过 && c.ApprovalResults.HasValue && c.ApprovalResults.Value).ToList();
            ...
            if (rs.Succeeded)
            {
                ...
                {
                    MainForm.Instance.AuditLogHelper.CreateAuditLog<tb_MRP_ReworkReturn>("结案成功", needCloseCases[i]);
                }
                //这里审核完了的话，如果这个单存在于工作流的集合队列中，则向服务器说明审核完成。
                //这里推送到审核，启动工作流  队列应该有一个策略 比方优先级，桌面不动1 3 5分钟 
                //OriginalData od = ActionForClient.工作流审批(pkid, (int)BizType.盘点单, ae.ApprovalResults, ae.ApprovalComments);
