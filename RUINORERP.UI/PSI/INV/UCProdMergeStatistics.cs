...
namespace RUINORERP.UI.PSI.INV
{
    ...
    [MenuAttrAssemblyInfo("组合明细统计", ModuleMenuDefine.模块定义.进销存管理, ModuleMenuDefine.供应链管理.库存管理, BizType.组合明细统计)]
    public partial class UCProdMergeStatistics : BaseNavigatorGeneric<View_ProdMergeItems, View_ProdMergeItems>
    {
        public UCProdMergeStatistics()
        {
            InitializeComponent();
            //生成查询条件的相关实体 视图时要指定
            ReladtedEntityType = typeof(View_ProdMergeItems);
        }
        ...
        {
            base._UCMasterQuery.GridRelated.SetRelatedInfo<View_ProdMergeItems, tb_ProdMerge>(c => c.MergeNo, r => r.MergeNo);
            ...
            base._UCOutlookGridGroupAnalysis.GridRelated.SetRelatedInfo<View_ProdMergeItems, tb_ProdMerge>(c => c.MergeNo, r => r.MergeNo);
        }
        ...
        {
            BaseProcessor baseProcessor = Startup.GetFromFacByName<BaseProcessor>(typeof(View_ProdMergeItems).Name + "Processor");
        ...
        public override async void Print(RptMode rptMode)
        {
            ...
            List<View_ProdMergeItems> selectlist = GetSelectResult();
            ...
            {
                _PrintConfig = PrintHelper<View_ProdMergeItems>.GetPrintConfig<View_ProdMergeItems>();
            }


            tb_CustomerVendor customerVendor = new tb_CustomerVendor();
            var dto = QueryDto as View_ProdMergeItems;
            ...
            bool rs = PrintHelper<View_ProdMergeItems>.PrintCustomData<PurEntryStatementByCV>(statementByCV, rptMode, _PrintConfig);
            if (rs)
            {
