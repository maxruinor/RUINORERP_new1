...
namespace RUINORERP.UI.PSI.INV
{
    ...
    [MenuAttrAssemblyInfo("其他入库统计", ModuleMenuDefine.模块定义.进销存管理, ModuleMenuDefine.供应链管理.库存管理, BizType.其他入库统计)]
    public partial class UCStockInStatistics : BaseNavigatorGeneric<View_StockInItems, View_StockInItems>
    {
        public UCStockInStatistics()
        {
            InitializeComponent();
            //生成查询条件的相关实体 视图时要指定
            ReladtedEntityType = typeof(View_StockInItems);
        }
        ...
        {
            base._UCMasterQuery.GridRelated.SetRelatedInfo<View_StockInItems, tb_StockIn>(c => c.BillNo, r => r.BillNo);
            ...
            base._UCOutlookGridGroupAnalysis.GridRelated.SetRelatedInfo<View_StockInItems, tb_StockIn>(c => c.BillNo, r => r.BillNo);
        }
        ...
        {
            BaseProcessor baseProcessor = Startup.GetFromFacByName<BaseProcessor>(typeof(View_StockInItems).Name + "Processor");
        ...
        public override async void Print(RptMode rptMode)
        {
            ...
            List<View_StockInItems> selectlist = GetSelectResult();
            ...
            {
                _PrintConfig = PrintHelper<View_StockInItems>.GetPrintConfig<View_StockInItems>();
            }


            tb_CustomerVendor customerVendor = new tb_CustomerVendor();
            var dto = QueryDto as View_StockInItems;
            ...
            bool rs = PrintHelper<View_StockInItems>.PrintCustomData<PurEntryStatementByCV>(statementByCV, rptMode, _PrintConfig);
            if (rs)
            {
