...
{
    [MenuAttrAssemblyInfo("其他出库单", ModuleMenuDefine.模块定义.进销存管理, ModuleMenuDefine.进销存管理.库存管理, BizType.其他出库单)]
    public partial class UCStockOut : BaseBillEditGeneric<tb_StockOut, tb_StockOutDetail>
    {
        ...
        public override void BindData(tb_StockOut entity, ActionStatus actionStatus = ActionStatus.无操作)
        {
            ...
            else
            {
                ...
                entity.BillNo = BizCodeGenerator.Instance.GetBizBillNo(BizType.其他出库单);
        ...
        private void Sgh_OnLoadMultiRowData(object rows, Position position)
        {
            ...
            if (RowDetails != null)
            {
                ...
                {
                    tb_StockOutDetail bOM_SDetail = mapper.Map<tb_StockOutDetail>(item);
                    bOM_SDetail.Qty = 0;
