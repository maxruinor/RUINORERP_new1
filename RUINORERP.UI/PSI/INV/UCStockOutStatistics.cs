...
namespace RUINORERP.UI.PSI.INV
{
    ...
    [MenuAttrAssemblyInfo("其他出库统计", ModuleMenuDefine.模块定义.进销存管理, ModuleMenuDefine.进销存管理.其他出入库管理, BizType.其他出库统计)]
    public partial class UCStockOutStatistics : BaseNavigatorGeneric<View_StockOutItems, View_StockOutItems>
    {
        public UCStockOutStatistics()
        {
            InitializeComponent();
            //生成查询条件的相关实体 视图时要指定
            ReladtedEntityType = typeof(View_StockOutItems);
        }
        ...
        {
            base._UCMasterQuery.GridRelated.SetRelatedInfo<View_StockOutItems, tb_StockOut>(c => c.BillNo, r => r.BillNo);
            ...
            base._UCOutlookGridGroupAnalysis.GridRelated.SetRelatedInfo<View_StockOutItems, tb_StockOut>(c => c.BillNo, r => r.BillNo);
        }
        ...
        {
            BaseProcessor baseProcessor = Startup.GetFromFacByName<BaseProcessor>(typeof(View_StockOutItems).Name + "Processor");
        ...
        public override async void Print(RptMode rptMode)
        {
            ...
            List<View_StockOutItems> selectlist = GetSelectResult();
            ...
            {
                _PrintConfig = PrintHelper<View_StockOutItems>.GetPrintConfig<View_StockOutItems>();
            }


            tb_CustomerVendor customerVendor = new tb_CustomerVendor();
            var dto = QueryDto as View_StockOutItems;
            ...
            bool rs = PrintHelper<View_StockOutItems>.PrintCustomData<PurEntryStatementByCV>(statementByCV, rptMode, _PrintConfig);
            if (rs)
            {
