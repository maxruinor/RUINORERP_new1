...
namespace RUINORERP.UI.PSI.INV
{
    ...
    [MenuAttrAssemblyInfo("盘点明细统计", ModuleMenuDefine.模块定义.进销存管理, ModuleMenuDefine.供应链管理.库存管理, BizType.盘点明细统计)]
    public partial class UCStocktakeStatistics : BaseNavigatorGeneric<View_StocktakeItems, View_StocktakeItems>
    {
        public UCStocktakeStatistics()
        {
            InitializeComponent();
            //生成查询条件的相关实体 视图时要指定
            ReladtedEntityType = typeof(View_StocktakeItems);
        }
        ...
        {
            base._UCMasterQuery.GridRelated.SetRelatedInfo<View_StocktakeItems, tb_Stocktake>(c => c.CheckNo, r => r.CheckNo);
            ...
            base._UCOutlookGridGroupAnalysis.GridRelated.SetRelatedInfo<View_StocktakeItems, tb_Stocktake>(c => c.CheckNo, r => r.CheckNo);
        }
        ...
        {
            //创建表达式
            var lambda = Expressionable.Create<View_StocktakeItems>()
        ...
        {
            BaseProcessor baseProcessor = Startup.GetFromFacByName<BaseProcessor>(typeof(View_StocktakeItems).Name + "Processor");
            QueryFilter = baseProcessor.GetQueryFilter();
        }

        public override void BuildSummaryCols()
        {
            base.MasterSummaryCols.Add(c => c.CarryinglQty);
            base.MasterSummaryCols.Add(c => c.CheckQty);
            base.MasterSummaryCols.Add(c => c.DiffQty);
            base.MasterSummaryCols.Add(c => c.Cost);
            base.MasterSummaryCols.Add(c => c.CheckSubtotalAmount);
        }
        ...
        public override async void Print(RptMode rptMode)
        {
            ...
            List<View_StocktakeItems> selectlist = GetSelectResult();
            ...
            {
                _PrintConfig = PrintHelper<View_StocktakeItems>.GetPrintConfig<View_StocktakeItems>();
            }


            tb_CustomerVendor customerVendor = new tb_CustomerVendor();
            var dto = QueryDto as View_StocktakeItems;
            ...
            bool rs = PrintHelper<View_StocktakeItems>.PrintCustomData<PurEntryStatementByCV>(statementByCV, rptMode, _PrintConfig);
            if (rs)
            {
