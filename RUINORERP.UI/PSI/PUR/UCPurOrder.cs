...
namespace RUINORERP.UI.PSI.PUR
{
    ...
    public partial class UCPurOrder : BaseBillEditGeneric<tb_PurOrder, tb_PurOrderDetail>, IPublicEntityObject
    {
        ...
        public override async void BindData(tb_PurOrder entity, ActionStatus actionStatus)
        {
            if (entity == null)
            {
            ...
            else
            {
                ...
                if (string.IsNullOrEmpty(entity.PurOrderNo))
                {
                    ...
                    entity.PurOrderNo = await bizCodeService.GenerateBizBillNoAsync(BizType.采购订单);
                }
        ...
        protected async override Task<bool> Save(bool NeedValidated)
        {
            ...
            if (NeedValidated && EditEntity.Paytype_ID.HasValue)
            {
                if (EditEntity.PayStatus == (int)PayStatus.未付款)
                {
                    ...
                    var paymethod = _cacheManager.GetEntity<tb_PaymentMethod>(EditEntity.Paytype_ID.Value);
            ...
            if (NeedValidated)
            {
                ...
                if (EditEntity.PayStatus == (int)PayStatus.全额预付)
                {
                    ...
                    if (EditEntity.Deposit > EditEntity.TotalAmount)
                    {
                        ...
                        string message = $"预付订金金额大于订单本币总金额，当前为超额付款！\n\n" +
                                       $"订单本币总金额：{EditEntity.TotalAmount:N2} 元\n" +
                                       $"预付订金金额：{EditEntity.Deposit:N2} 元\n" +
                        ...
                        {
                            EditEntity.Deposit = EditEntity.TotalAmount;
                            MainForm.Instance.uclog.AddLog($"预付订金已自动调整为订单本币总金额: {EditEntity.TotalAmount:N2} 元");
                        }
                    }
