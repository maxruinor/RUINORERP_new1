    ...
    public partial class UCSaleOutRe : BaseBillEditGeneric<tb_SaleOutRe, tb_SaleOutReDetail>, IPublicEntityObject
    {
        ...
        private async void toolStripButton平台退款_Click(object sender, EventArgs e)
        {
            ...
            if (EditEntity != null)
            {
                ...
                //只有审核状态才可以转换
                //if (EditEntity.DataStatus >= (int)DataStatus.草稿 && EditEntity.ApprovalStatus == (int)ApprovalStatus.已审核 && EditEntity.ApprovalResults.HasValue && EditEntity.ApprovalResults.Value)
                //{
                //判断是否为平台订单
                ...
                if (!saleOutRe.RefundStatus.HasValue)
                {
                ...
                if (saleOutRe.RefundStatus.Value >= (int)RefundStatus.未退款等待退货)
                {
                    ...
                    if (authorizeController.EnableFinancialModule())
                    {
                        if (MainForm.Instance.AppContext.FMConfig.AutoAuditReceiveable)
                        {
                            ...
                            var receivablePayable = await MainForm.Instance.AppContext.Db.Queryable<tb_FM_ReceivablePayable>()
                                            .Includes(c => c.tb_FM_ReceivablePayableDetails)
                                            .Where(c => c.ARAPStatus >= (int)ARAPStatus.待支付 && c.SourceBillId == saleOutRe.SaleOutRe_ID)
                                            .FirstAsync();

                            //目前一个退回单对应一个应收款（负数）其它情况手动完成。暂时
                            if (receivablePayable == null)
                            {
                                ...
                                receivablePayable = await ctrpayable.BuildReceivablePayable(saleOutRe);
                            ...
                            else
                            {
                                ...
                                if (MainForm.Instance.AppContext.FMConfig.AutoAuditReceivePaymentRecordByPlatform)
                                {
                                    ...
                                    tb_FM_PaymentRecord newPaymentRecord = new tb_FM_PaymentRecord();
        ...
        protected async override Task<ReturnResults<tb_SaleOutRe>> Delete()
        {
            ...
            if (MessageBox.Show("系统不建议删除单据资料\r\n确定删除吗？", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Information) == DialogResult.Yes)
            {
                ...
                if (dataStatus == DataStatus.新建 || dataStatus == DataStatus.草稿)
                {
                    ...
                    {
                        if (EditEntity.Created_by.Value != AppContext.CurUserInfo.EmployeeId)
                        {
        ...
        protected async override Task<bool> Save(bool NeedValidated)
        {
            ...
            if (EditEntity.ActionStatus == ActionStatus.新增 || EditEntity.ActionStatus == ActionStatus.修改)
            {
                ...
                {
                    if (MessageBox.Show("退货明细中，录入了相同的产品你确定吗？", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Asterisk, MessageBoxDefaultButton.Button2) == DialogResult.No)
                    {
                        return false;
                    }
        ...
        protected override async Task<bool> Submit()
        {
            if (EditEntity != null)
            {
                ...
                if (MessageBox.Show("【提交】表示你收到了退回的商品，可以进行下一步处理。 你收到退件了吗？", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Asterisk, MessageBoxDefaultButton.Button2) == DialogResult.No)
                {
                    return false;
                }


                if (EditEntity.HasChanged && EditEntity.HasPropertyChanged(nameof(RefundStatus)))
                {
                    System.Windows.Forms.MessageBox.Show("销售退回单，在提交时【退货退款状态】不能修改。", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return false;
                }
