...
begin
 ...
 SELECT  库位, ProdDetailID, SUM(期初) as 期初,  SUM(采购入库) AS 采购入库 , SUM(采购退回) as 采购退回,SUM(销售出库) as 销售出库 , SUM(销售退回) as 销售退回, SUM(其它出库) as 其它出库, SUM(其它入库) as 其它入库 , SUM(库存盘点) as 库存盘点,sum(缴库) as 缴库 , SUM(期末库存) as 期末库存 from (
SELECT  Location_ID as 库位, ProdDetailID, SUM(InitQty) as 期初,  0 AS 采购入库, 0 as 采购退回,0 as 销售出库 , 0 as 销售退回, 0 as 其它出库, 0 as 其它入库 , 0 as 库存盘点 ,0 as 缴库, 0 as 期末库存 from tb_Inventory a LEFT JOIN tb_OpeningInventory b on a.Inventory_ID=b.Inventory_ID  WHERE Location_ID=@Location_ID  and ProdDetailID=@ProdDetailID GROUP BY Location_ID,ProdDetailID
union ALL
...
--exec Proc_InventoryTracking '2008-4-5','2008-9-01','001','商品编码'