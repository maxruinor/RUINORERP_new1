...
- **IStatusTransitionEngine.cs** - 状态转换引擎接口
  ...
  - 功能：定义状态转换的核心逻辑接口，包括状态转换的验证和执行方法，提供可转换状态列表获取功能（同步和异步版本），支持自定义转换规则和执行器注册
...
### 2. 状态变更事件参数
- **文件路径**: `e:\CodeRepository\SynologyDrive\RUINORERP\RUINORERP.UI\StateManagement\Core\StatusChangeEventArgs.cs` (已删除)
- **功能**: 状态变更事件参数类
...
2. **重要注意**：V3版本的状态管理系统中不存在名为StatusChangedEventArgs的事件参数类。系统统一使用StateTransitionEventArgs事件参数类，StatusChangeEventArgs已不再使用。
...
- 2025年11月: 在重要提示中添加说明，明确指出V3版本的状态管理系统中不存在名为StatusChangedEventArgs的事件参数类，系统统一使用StateTransitionEventArgs