...
namespace RUINORERP.UI.SysConfig.BasicDataImport
{
    ...
    public class DynamicImporter
    {
        ...
        public DynamicImporter(ISqlSugarClient db, IUnitOfWorkManage unitOfWorkManage = null, IForeignKeyService foreignKeyService = null, string imageOutputDirectory = null, ImageProcessor imageProcessor = null)
        {
            ...
            string importType = null)
        {
            ...
            // 检查是否有图片字段映射
            var imageMappings = mappings.Where(m => m.ColumnDataSourceType == (int)DataSourceType.ExcelImage).ToList();
        ...
        private object GetCellValueFromMapping(DataRow row, ColumnMapping mapping, List<ColumnMapping> allMappings, int rowNumber, bool isPreprocessed)
        {
            switch (mapping.ColumnDataSourceType)
            {
                ...
                case (int)DataSourceType.ForeignKey:
                    // 数据库表关联引用：根据 IsSelfReference 区分外键关联和自身引用
                    var dbRefConfig = mapping.DataSourceConfig as DatabaseReferenceConfig;
                    if (dbRefConfig != null && dbRefConfig.IsSelfReference)
                    {
                        // 自身表引用或字段复制
        ...
        private object GetFieldCopyCellValue(DataRow row, ColumnMapping mapping, List<ColumnMapping> allMappings)
        {
            ...
            if (fieldCopyConfig == null || !fieldCopyConfig.IsSelfReference || string.IsNullOrEmpty(fieldCopyConfig.ForeignFieldName))
                return null;
        ...
        /// <summary>
        /// 获取自身引用的值（已合并到数据库表关联引用）
        /// </summary>
        private object GetSelfReferenceCellValue(DataRow row, ColumnMapping mapping)
        {
            var selfRefConfig = mapping.DataSourceConfig as DatabaseReferenceConfig;
            if (selfRefConfig == null || !selfRefConfig.IsSelfReference)
                return null;
            ...
            if (!string.IsNullOrEmpty(displayValue) && !string.IsNullOrEmpty(selfRefConfig.ForeignFieldName))
            {
                // 暂时使用显示值，后续在导入过程中处理
                return displayValue;
            }
        ...
        private async System.Threading.Tasks.Task BatchImportEntitiesInternalAsync<T>(List<BaseEntity> entityList, string primaryKeyName, ImportResult result, string importType = null, bool isPreprocessed = false) where T : BaseEntity, new()
        {
            try
            {
                ...
                bool useTransaction = _unitOfWorkManage == null;
                if (useTransaction)
                {
            ...
            List<string> businessKeys) where T : BaseEntity, new()
        {
            ...
            // 从全局配置获取存在性策略
            var strategy = (ExistenceStrategyType)(_currentConfig?.ExistenceStrategy ?? (int)ExistenceStrategyType.Skip);

            switch (strategy)
            {
                case ExistenceStrategyType.Skip:
                    return await FilterExistingRecordsAsync(dbClient, entities, businessKeys);

                case ExistenceStrategyType.Error:
        ...
        private void ProcessFieldCopyField(DataRow sourceRow, DataRow targetRow, ColumnMapping mapping)
        {
            ...
            string copyFromField = fieldCopyConfig.ForeignFieldName;
