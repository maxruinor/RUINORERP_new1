...
{
    public partial class QueryFormGeneric<T> : KryptonForm
    {
        ...
        private List<T> _queryObjects = new List<T>();
        ...
        internal void InitListData()
        {
            ...
            FieldNameList1 = UIHelper.GetFieldNameColList(typeof(T));
            kryptonDataGridView产品.XmlFileName = "QueryFormGeneric_" + typeof(T).Name;
            kryptonDataGridView产品.FieldNameList = FieldNameList1;
            kryptonDataGridView产品.DataSource = null;
            bindingSource1.DataSource = new List<T>();
        ...
        ConcurrentDictionary<string, string> cds = UIHelper.GetFieldNameList<T>();
        ...
        /// 返回查询的对象
        /// </summary>
        public List<T> QueryObjects { get => _queryObjects; set => _queryObjects = value; }
            ...
            if (kryptonDataGridView产品.SelectedRows != null)
            {
                foreach (DataGridViewRow dr in kryptonDataGridView产品.SelectedRows)
                {
                    ...
                    if (MultipleChoices && (bool)dr.Cells["Selected"].Value)
                    {
                        ...
                        QueryObjects.Add((T)dr.DataBoundItem);
                    }
                    else
                    {
                        QueryObjects.Add((T)dr.DataBoundItem); //QueryObjects.Add(dr.DataBoundItem as View_ProdDetail);
                    }
                ...
                if (QueryObjects.Count > 0)
                {
                    ...
                    //else
                    //{
                    //    QueryValue = QueryObjects[0].CNName;
                    //}
                }
        ...
        */
        ...
        private void treeListView1_DoubleClick(object sender, EventArgs e)
        {
            ...
            if (treeListView1.SelectedItems != null)
            {
                ...
                {
                    if (!QueryObjects.Contains((T)item.Tag))
                    {
                        QueryObjects.Add((T)item.Tag);
                    }
                }
            }
            if (QueryObjects.Count > 0)
