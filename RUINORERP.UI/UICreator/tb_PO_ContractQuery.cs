...
{
    /// <summary>
    /// 先采购合同再订单,条款内容后面补充 注意一个合同可以多个发票一个发票也可以多个合同数据查询
    /// </summary>
     [MenuAttribute(typeof(tb_PO_ContractQuery), "先采购合同再订单,条款内容后面补充 注意一个合同可以多个发票一个发票也可以多个合同数据查询", true)]
    public partial class tb_PO_ContractQuery:UserControl
    {
     public tb_PO_ContractQuery() {
       ...
       // RuleFor(tb_PO_Contract => tb_PO_Contract.POContractID).NotEmpty();
