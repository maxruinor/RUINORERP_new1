            ...
            try
            {
                ...
                else
                {
                 ...
                 // .Includes(c => c.tb_productiondemand)
                  ...
                  .Includes(d => d.tb_PurEntries.Where(cc => cc.DataStatus > (int)DataStatus.新建).ToList(), f => f.tb_PurEntryDetails)
                  .Includes(c => c.tb_PurOrderRes, d => d.tb_PurOrderReDetails)
                  .Includes(c => c.tb_PurEntries.Where(cc => cc.DataStatus > (int)DataStatus.新建).ToList(), d => d.tb_PurEntryRes, f => f.tb_PurEntryReDetails)
                   .AsNavQueryable()
                   .Includes(c => c.tb_PurEntries.Where(cc => cc.DataStatus > (int)DataStatus.新建).ToList(), d => d.tb_PurEntryRes, f => f.tb_PurReturnEntries, g => g.tb_PurReturnEntryDetails)
