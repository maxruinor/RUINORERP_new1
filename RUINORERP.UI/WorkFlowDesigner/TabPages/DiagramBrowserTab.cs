...
namespace RUINORERP.UI.WorkFlowDesigner
{
	...
	mediator.parent.RaiseLoading("Loading diagrams in the diagram browser...");
			...
			try
			{
				...
				baseList.Redraw();
                Netron.GraphLib.IO.Reporting.BinaryReporter reporter = new Netron.GraphLib.IO.Reporting.BinaryReporter(path);
				reporter.OnReport+=new InfoDelegate(OnReport);
                Netron.GraphLib.IO.Reporting.BinaryReportCollection col = reporter.Report() as Netron.GraphLib.IO.Reporting.BinaryReportCollection;
				DiagramReport card;
				...
				for(int k = 0; k<col.Count; k++)
				{
					try
					{
						...
						card.LoadDiagram+=new DiagramReport.PathInfo(card_LoadDiagram);
						card.SelectDirectory+=new EventHandler(card_SelectDirectory);
						card.ReloadDirectory+=new EventHandler(card_ReloadDirectory);
						card.DeleteDiagram+=new DiagramReport.PathInfo(card_DeleteDiagram);
						#endregion
					...
					{
						mediator.Output("File '" + new System.IO.FileInfo(report.Path).Name + "' seems not to be a valid diagram file.",Netron.GraphLib.OutputInfoLevels.Info);
						continue;
					}
					baseList.EndUpdate();
			...
			{
				mediator.Output("The directory was not found",OutputInfoLevels.Exception);
				Trace.WriteLine(e1.Message,"DiagramBrowserTab.LoadDirectory");
			}
			catch(System.IO.FileNotFoundException e2)
			{
				mediator.Output("The file was not found",OutputInfoLevels.Exception);
				Trace.WriteLine(e2.Message,"DiagramBrowserTab.LoadDirectory");
			}
			catch(Exception e)
			{
				mediator.Output(e.Message,OutputInfoLevels.Exception);
