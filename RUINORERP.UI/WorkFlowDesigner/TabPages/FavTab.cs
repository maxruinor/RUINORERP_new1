...
namespace RUINORERP.UI.WorkFlowDesigner
{
    ...
    "le-click an item to load it into the canvas.";
		...
		private void Favorites_ItemActivate(object sender, System.EventArgs e)
		{
			...
			if(templates.SelectedItems.Count>0)
			{
				...
				if(bundle!=null)
				{
                    ...
                    Netron.GraphLib.IO.Binary.BinarySerializer.UnwrapBundle(copy, mediator.GraphControl);
		...
		public void LoadDirectory(string path)
		{
			...
			try
			{
				...
				for(int k = 0; k<files.Length; k++)
				{
					try
					{
						...
						mediator.Output("Found template '" + path + "\\" + files[k] + "' in directory.", Netron.GraphLib.OutputInfoLevels.Info);
					}
					catch(Exception exc)
					{
						mediator.Output("File '" + path + "\\" + files[k] + "' seems not to be a valid template file.", Netron.GraphLib.OutputInfoLevels.Info);
			...
			{
				mediator.Output("The directory was not found", Netron.GraphLib.OutputInfoLevels.Exception);
				Trace.WriteLine(e1.Message,"FavTab.LoadDirectory");
			}
			catch(System.IO.FileNotFoundException e2)
			{
				mediator.Output("The file was not found", Netron.GraphLib.OutputInfoLevels.Exception);
				Trace.WriteLine(e2.Message,"FavTab.LoadDirectory");
			}
			catch(Exception e)
			{
				mediator.Output(e.Message, Netron.GraphLib.OutputInfoLevels.Exception);
