# 流程导航图功能使用说明

## 概述

流程导航图功能是ERP系统的重要组成部分，通过可视化的方式展示业务流程，帮助用户快速理解和操作系统。该功能支持多级导航图设计，包括系统总览图、模块级导航图和具体业务流程图。

## 功能特点

### 1. 分级导航体系
- **总图（级别0）**：展示整个ERP系统的主要模块和业务流向
- **模块图（级别1）**：展示特定业务模块内的主要流程
- **业务图（级别2）**：展示具体的业务操作流程
- **子流程图（级别3）**：展示更细粒度的子流程
- **操作图（级别4）**：展示具体操作步骤

### 2. 分级管理功能
- **层级关系管理**：支持设置父流程、获取子流程、层级路径查询
- **层级结构操作**：支持流程在层级中的排序、移动和创建副本
- **验证检查**：自动检测循环引用和层级深度限制（最大5级）
- **树形结构展示**：支持以树形结构查看完整流程层级关系
- **默认流程设置**：可为每个层级设置默认流程
- **模块关联查询**：支持按模块查询相关流程

### 2. 业务节点类型
- **菜单节点**：直接关联到系统菜单，点击可打开对应功能
- **模块节点**：关联到业务模块，点击可进入模块级导航图
- **流程节点**：关联到子流程导航图，支持流程嵌套
- **通用节点**：开始、结束等通用流程节点

### 3. 可视化设计
- 拖拽式节点设计
- 连接线绘制
- 节点属性配置
- 颜色和样式自定义

## 使用流程

### 1. 系统启动时的导航图加载

#### 自动加载逻辑
1. **优先级1**：加载系统默认总览图
2. **优先级2**：加载第一个可用的总览图
3. **优先级3**：根据用户权限加载模块级导航图

#### 代码集成
在主窗体启动时调用：
```csharp
// 在MainForm的Load事件中
private async void MainForm_Load(object sender, EventArgs e)
{
    // 其他初始化代码...
    
    // 初始化流程导航图功能
    await InitializeProcessNavigationAsync();
}
```

### 2. 流程导航图设计

#### 打开设计器
```csharp
// 显示导航设计器
ToggleNavigationDesigner();
```

#### 设计器界面
- **左侧模板面板**：包含预定义的业务节点模板
- **右侧设计面板**：可视化流程设计区域
- **顶部工具栏**：新建、打开、保存、模式切换等操作

#### 节点拖拽设计
1. 从左侧模板面板选择节点
2. 拖拽到右侧设计面板
3. 调整节点位置和大小
4. 配置节点属性
5. 绘制连接线

#### 节点属性配置
双击节点或右键选择"属性"可配置：
- 节点名称和描述
- 业务类型
- 关联菜单/模块/流程
- 节点颜色
- 位置和大小

### 3. 业务模块关联

#### 模块定义
系统通过 `tb_ModuleDefinition` 表定义业务模块：
```sql
-- 示例模块数据
INSERT INTO tb_ModuleDefinition (ModuleNo, ModuleName, Visible, Available) VALUES
('M001', '采购管理', 1, 1),
('M002', '销售管理', 1, 1),
('M003', '库存管理', 1, 1),
('M004', '财务管理', 1, 1),
('M005', '生产管理', 1, 1);
```

#### 菜单关联
节点可以关联到具体菜单：
```csharp
// 创建菜单节点模板
var menuTemplate = new BusinessNodeTemplate
{
    Name = "采购申请",
    BusinessType = ProcessNavigationNodeBusinessType.菜单节点,
    MenuID = 1001, // 对应的菜单ID
    FormName = "FrmPurchaseRequest",
    ClassPath = "RUINORERP.UI.Purchase.FrmPurchaseRequest"
};
```

### 4. 导航图保存和加载

#### 保存导航图
```csharp
// 保存当前导航图
private async void SaveNavigation()
{
    if (CurrentNavigation == null) return;
    
    // 生成XML
    var xml = GenerateGraphXml();
    
    // 保存到数据库
    CurrentNavigation.GraphXml = xml;
    CurrentNavigation.UpdateTime = DateTime.Now;
    
    await _processNavigationService.UpdateAsync(CurrentNavigation);
}
```

#### 加载导航图
```csharp
// 加载导航图内容
private void LoadNavigationContent()
{
    if (string.IsNullOrEmpty(CurrentNavigation.GraphXml))
    {
        _graphControl.Clear();
        return;
    }
    
    using (var stringReader = new StringReader(CurrentNavigation.GraphXml))
    {
        _graphControl.Load(stringReader);
    }
}
```

## 数据库结构

### 主要表结构

#### tb_ProcessNavigation（流程导航图定义表）
| 字段名 | 类型 | 说明 |
|--------|------|------|
| ProcessNavigationID | bigint | 主键ID |
| NavigationName | varchar(200) | 导航图名称 |
| Description | varchar(500) | 描述 |
| ModuleID | bigint | 关联模块ID |
| NavigationLevel | int | 导航图级别（0-总图，1-模块图，2-业务图，3-子流程图，4-操作图） |
| ParentNavigationID | bigint | 父级导航图ID（自引用外键） |
| HierarchyLevel | int | 层级深度（0-4级） |
| SortOrder | int | 在同级中的排序序号 |
| GraphXml | text | 图形XML内容 |
| IsActive | bit | 是否激活 |
| IsDefault | bit | 是否默认 |

### 层级关系完整性约束

为确保流程导航图层级结构的完整性，系统实现了以下约束：

- 层级深度限制：最大支持5级深度（0-4）
- 禁止循环引用：系统自动检测并阻止创建循环引用关系
- 外键约束：ParentNavigationID字段引用ProcessNavigationID
- 索引优化：为ParentNavigationID和NavigationLevel字段创建索引以提高查询性能
- 层级完整性保障：新增存储过程和触发器确保层级数据的一致性

### 新增存储过程和触发器

#### 存储过程

1. **SP_SetProcessNavigationHierarchy**：设置流程导航图层级关系
   - 参数：@ProcessNavigationID, @ParentNavigationID
   - 功能：自动计算并设置层级深度，检测循环引用

2. **SP_GetProcessNavigationHierarchy**：获取导航图的完整层级路径
   - 参数：@ProcessNavigationID
   - 功能：返回从根节点到指定节点的完整路径

3. **SP_GetChildProcessNavigations**：获取所有子导航图
   - 参数：@ParentNavigationID
   - 功能：递归获取指定父导航图的所有子导航图

#### 触发器

1. **TR_ProcessNavigation_AfterUpdate**：层级更新触发器
   - 功能：父导航图更新时自动更新相关子导航图的层级信息

2. **TR_ProcessNavigationNode_UpdateChildNav**：节点更新触发器
   - 功能：确保节点引用的子导航图与当前导航图的层级关系正确

#### tb_ProcessNavigationNode（流程导航图节点表）
| 字段名 | 类型 | 说明 |
|--------|------|------|
| NodeID | bigint | 主键ID |
| ProcessNavID | bigint | 流程导航图ID |
| BusinessType | int | 业务类型 |
| MenuID | bigint | 关联菜单ID |
| ModuleID | bigint | 关联模块ID |
| ChildNavigationID | bigint | 子流程导航图ID |
| NodeColor | varchar(50) | 节点颜色 |
| PositionX | float | X坐标 |
| PositionY | float | Y坐标 |

## 权限控制

### 基于角色的访问控制
```csharp
// 获取用户可访问的模块
private async Task<List<long>> GetUserAccessibleModulesAsync()
{
    var userId = ApplicationContext.Current.UserID;
    
    // 根据用户角色获取可访问模块
    var accessibleModules = await _userModuleService.QueryableAsync()
        .Where(x => x.UserID == userId && x.HasPermission)
        .Select(x => x.ModuleID)
        .ToListAsync();
    
    return accessibleModules;
}
```

### 导航图权限过滤
```csharp
// 加载用户可访问的导航图
var navigations = await _processNavigationService.QueryableAsync()
    .Where(x => x.IsActive && 
               (x.NavigationLevel == 0 || // 总图所有人可见
                accessibleModules.Contains(x.ModuleID.Value))) // 模块图需要权限
    .ToListAsync();
```

## 分级管理功能使用指南

### 1. 设置流程层级关系

#### 创建父子流程关系
```csharp
// 设置流程层级关系
private async Task SetProcessHierarchy(long parentProcessNavID, long childProcessNavID)
{
    await _processNavigationHierarchyManager.SetParentProcessAsync(childProcessNavID, parentProcessNavID);
    
    // 刷新界面显示
    RefreshHierarchyTree();
}
```

#### 查看流程层级树
```csharp
// 显示流程层级树
private void ShowProcessHierarchyTree()
{
    // 获取完整层级树
    var hierarchyTree = _processNavigationHierarchyManager.GetHierarchyTree();
    
    // 绑定到TreeView控件
    BindHierarchyToTreeView(hierarchyTree, tvHierarchy);
}
```

### 2. 流程层级操作

#### 移动流程到新层级
```csharp
// 移动流程到新的父级
private async Task MoveProcessToNewParent(long processNavID, long newParentProcessNavID)
{
    try
    {
        // 验证移动的有效性
        if (await _processNavigationHierarchyManager.ValidateMoveAsync(processNavID, newParentProcessNavID))
        {
            // 执行移动
            await _processNavigationHierarchyManager.MoveProcessAsync(processNavID, newParentProcessNavID);
            MessageBox.Show("流程移动成功！");
        }
        else
        {
            MessageBox.Show("无法移动流程，可能导致循环引用或超出层级限制！");
        }
    }
    catch (Exception ex)
    {
        MessageBox.Show($"移动失败：{ex.Message}");
    }
}
```

#### 修改同级流程排序
```csharp
// 修改同级流程排序
private async Task UpdateProcessSortOrder(long processNavID, int newSortOrder)
{
    await _processNavigationHierarchyManager.UpdateSortOrderAsync(processNavID, newSortOrder);
}
```

### 3. 层级导航操作

#### 获取流程完整路径
```csharp
// 获取流程完整路径
private string GetProcessFullPath(long processNavID)
{
    var path = _processNavigationHierarchyManager.GetProcessPath(processNavID);
    return string.Join(" / ", path);
}
```

#### 在层级间导航
```csharp
// 导航到父流程
private async void NavigateToParentProcess()
{
    if (CurrentProcessNavigation == null) return;
    
    var parentId = await _processNavigationHierarchyManager.GetParentProcessIdAsync(CurrentProcessNavigation.ProcessNavID);
    if (parentId.HasValue)
    {
        var parentProcess = await _processNavigationService.GetByIdAsync(parentId.Value);
        if (parentProcess != null)
        {
            await LoadProcessNavigation(parentProcess);
        }
    }
}

// 导航到子流程
private async void NavigateToChildProcess(long childProcessNavID)
{
    var childProcess = await _processNavigationService.GetByIdAsync(childProcessNavID);
    if (childProcess != null)
    {
        await LoadProcessNavigation(childProcess);
    }
}
```

### 4. 层级管理相关API

#### 检查循环引用
```csharp
// 检查是否会造成循环引用
private bool CheckForCircularReference(long childProcessNavID, long parentProcessNavID)
{
    return _processNavigationHierarchyManager.CheckCircularReference(childProcessNavID, parentProcessNavID);
}
```

#### 获取指定层级的流程列表
```csharp
// 获取指定层级的所有流程
private async Task<List<tb_ProcessNavigation>> GetProcessByLevel(int hierarchyLevel)
{
    return await _processNavigationHierarchyManager.GetProcessesByLevelAsync(hierarchyLevel);
}
```

### 5. 层级结构维护

#### 创建流程副本到新层级
```csharp
// 创建流程副本并设置层级
private async Task<long> CreateProcessCopyInHierarchy(long sourceProcessNavID, long parentProcessNavID, string newName)
{
    // 创建副本
    var newProcessId = await _processNavigationService.CreateCopyAsync(sourceProcessNavID, newName);
    
    // 设置层级关系
    if (newProcessId > 0 && parentProcessNavID > 0)
    {
        await _processNavigationHierarchyManager.SetParentProcessAsync(newProcessId, parentProcessNavID);
    }
    
    return newProcessId;
}
```

## 扩展开发

### 1. 添加新的业务节点类型

#### 扩展枚举
```csharp
public enum ProcessNavigationNodeBusinessType
{
    通用节点 = 0,
    菜单节点 = 1,
    模块节点 = 2,
    流程节点 = 3,
    外部系统节点 = 4,  // 新增
    数据源节点 = 5,    // 新增
    API节点 = 6        // 新增
}
```

#### 添加节点模板
```csharp
// 在BusinessNodeTemplateManager中添加新模板
_moduleTemplates[ERPBusinessModule.系统管理] = new List<BusinessNodeTemplate>
{
    new BusinessNodeTemplate
    {
        Name = "API接口",
        Description = "调用外部API接口",
        BusinessType = ProcessNavigationNodeBusinessType.API节点,
        DefaultColor = Color.Purple,
        Icon = "🔗",
        Category = "接口管理"
    }
};
```

### 2. 自定义节点行为

#### 重写节点点击事件
```csharp
private void ExecuteNodeAction(ProcessNavigationNode node)
{
    switch (node.BusinessType)
    {
        case ProcessNavigationNodeBusinessType.API节点:
            ExecuteApiNode(node);
            break;
        case ProcessNavigationNodeBusinessType.外部系统节点:
            OpenExternalSystem(node);
            break;
        // 其他类型...
    }
}
```

### 3. 添加新的导航图级别

#### 扩展级别枚举
```csharp
public enum ProcessNavigationLevel
{
    总图 = 0,
    模块图 = 1,
    业务图 = 2,
    子流程图 = 3,  // 新增
    操作图 = 4     // 新增
}
```

## 常见问题

### 1. 导航图不显示
- 检查数据库连接是否正常
- 确认 `tb_ProcessNavigation` 表中有数据
- 检查用户权限设置

### 2. 节点点击无响应
- 确认节点已正确关联菜单或模块
- 检查 `MenuHelper.OpenMenu()` 方法是否正常工作
- 验证窗体类路径是否正确

### 3. 设计器无法保存
- 检查数据库写入权限
- 确认XML生成是否正常
- 验证事务提交是否成功

### 4. 模板拖拽失败
- 确认 `BusinessNodeTemplateManager` 已正确初始化
- 检查拖拽事件是否正确绑定
- 验证节点注册是否成功

### 5. 分级管理相关问题
- **无法设置父流程**：检查目标流程是否存在且处于激活状态
- **层级深度超出限制**：系统限制最大层级深度为5级（0-4）
- **循环引用错误**：检查是否尝试创建循环引用关系
- **排序序号冲突**：同级流程不能有重复的排序序号
- **层级移动失败**：可能是权限问题或违反层级规则

## 性能优化

### 1. 数据缓存
```csharp
// 缓存导航图数据
private readonly IMemoryCache _cache;

public async Task<List<tb_ProcessNavigation>> GetCachedNavigationsAsync()
{
    var cacheKey = "ProcessNavigation_List";
    
    if (!_cache.TryGetValue(cacheKey, out List<tb_ProcessNavigation> navigations))
    {
        navigations = await _processNavigationService.QueryableAsync()
            .Where(x => x.IsActive)
            .ToListAsync();
            
        _cache.Set(cacheKey, navigations, TimeSpan.FromMinutes(30));
    }
    
    return navigations;
}
```

### 2. 延迟加载
```csharp
// 双击节点

在设计模式下，双击节点将显示属性对话框，用于编辑节点属性：

```csharp
// 节点双击处理逻辑
private void ProcessNavigationNode_OnMouseUp(object sender, MouseEventArgs e)
{
    if (e.Button == MouseButtons.Left && e.Clicks == 2)
    {
        // 设计模式下显示属性对话框
        ShowPropertiesDialog();
    }
}
```

> 注意：预览模式下的节点点击行为由ProcessNavigationDesigner处理

### 2.3 右键菜单操作
// 延迟加载节点详情
private async void LoadNodeDetailsAsync(ProcessNavigationNode node)
{
    if (node.MenuID != null)
    {
        node.MenuInfo = await _menuInfoService.QueryableByIDAsync(node.MenuID);
    }
}

### 3. 异步操作

```csharp
// 使用异步操作避免UI阻塞
private async void RefreshNavigationAsync()
{
    try
    {
        this.Cursor = Cursors.WaitCursor;
        
        await _processNavigationManager.RefreshAsync();
        
        this.Cursor = Cursors.Default;
    }
    catch (Exception ex)
    {
        this.Cursor = Cursors.Default;
        MessageBox.Show($"刷新失败：{ex.Message}");
    }
}
```

## 总结

流程导航图功能为ERP系统提供了直观的业务流程展示和导航能力。通过合理的设计和实现，可以显著提升用户体验和操作效率。在实际使用中，需要根据具体业务需求进行定制和扩展。

> 注意：本说明文档已更新，与当前代码实现保持一致