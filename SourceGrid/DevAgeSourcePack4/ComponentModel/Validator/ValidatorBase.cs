...
namespace DevAge.ComponentModel.Validator
{
    ...
    public class ValidatorBase : ComponentLight, IValidator
	{
		...
		public object ObjectToValue(object tag_Object, object p_Object)
		{
			Type destinationType = ValueType;
			if (destinationType == null && p_Object != null)
				destinationType = p_Object.GetType();

			ConvertingObjectEventArgs l_Converting = new ConvertingObjectEventArgs(tag_Object,p_Object, destinationType);
			OnConvertingObjectToValue(l_Converting);
			if (l_Converting.ConvertingStatus == ConvertingStatus.Error)
				throw new ConversionErrorException(ValueTypeName, ObjectToStringForError(l_Converting.Value));
		...
		public object ValueToObject(object tag_Value, object p_Value, Type p_ReturnObjectType)
		{
			if (p_ReturnObjectType == null)
				throw new DevAgeApplicationException("Invalid parameter returnObjectType cannot be null");

			ConvertingObjectEventArgs l_Converting = new ConvertingObjectEventArgs(tag_Value,p_Value, p_ReturnObjectType);
			OnConvertingValueToObject(l_Converting);
			if (l_Converting.ConvertingStatus == ConvertingStatus.Error)
				throw new ConversionErrorException(p_ReturnObjectType.Name, ObjectToStringForError(l_Converting.Value));
		...
		/// <param name="p_ReturnObjectType"></param>
		/// <returns></returns>
		public object ValueToObjectWithTagValue(object p_Value, object tag_Value, Type p_ReturnObjectType)
		{
			if (p_ReturnObjectType == null)
				throw new DevAgeApplicationException("Invalid parameter returnObjectType cannot be null");

			ConvertingObjectEventArgs l_Converting = new ConvertingObjectEventArgs(p_Value, tag_Value, p_ReturnObjectType);
			OnConvertingValueToObject(l_Converting);
			if (l_Converting.ConvertingStatus == ConvertingStatus.Error)
				throw new ConversionErrorException(p_ReturnObjectType.Name, ObjectToStringForError(l_Converting.Value));
		...
		public string ValueToString(object tag_Value, object p_Value)
		{
			object tmp = ValueToObject(tag_Value,p_Value, typeof(string));
			if (tmp == null)
				return null;
			else
				return (string)tmp;
		...
		public virtual string ValueToDisplayString(object tag_Value, object p_Value)
		{
			ConvertingObjectEventArgs l_Converting = new ConvertingObjectEventArgs(tag_Value,p_Value, typeof(string));
			OnConvertingValueToDisplayString(l_Converting);
			if (l_Converting.ConvertingStatus == ConvertingStatus.Error)
				throw new ConversionErrorException("display String", ObjectToStringForError(l_Converting.Value));
		...
		/// <summary>
		/// 如果对象对这种类型的验证器有效，则使用转换函数返回true。将转换后的值作为参数返回。
		/// </summary>
		/// <param name="p_Object"></param>
        ...
        /// 此编辑器可以支持的值的列表。如果StandardValuesExclusive为true，则编辑器只能支持其中一个值。
        /// </summary>
        [DefaultValue(null), Browsable(false), DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
