...
namespace SourceGrid.Cells.Editors
{
    ...
    public class EditorBase : DevAge.ComponentModel.Validator.ValidatorTypeConverter
    {
        ...
        public virtual string GetEditedText()
        {
            return null;
        }
        /// <summary>
        ...
        {
            SetCellValue(cellContext, cellContext.Tag, DefaultValue);
        }
        ...
        /// <param name="p_tagNewValue">添加了一个参数 用于传tag值永远是真正的ID</param>
        /// <param name="p_NewValue"></param>
        /// <returns>returns true if the value passed is valid and has been applied to the cell</returns>
        public virtual bool SetCellValue(CellContext cellContext, object p_tagNewValue, object p_NewValue)
        {
            if (EnableEdit)
            {
                if (cellContext.Cell == null)
        ...
        /// <returns></returns>
        public virtual bool SetCellTagValue(CellContext cellContext, object p_NewTagValue)
        {
            if (EnableEdit)
            {
                if (cellContext.Cell == null)
                ...
                ValidatingCellEventArgs l_cancelEvent = new ValidatingCellEventArgs(cellContext, p_NewTagValue);
                OnValidating(l_cancelEvent);

                //check if cancel == true 
                if (l_cancelEvent.Cancel == false)
                {
                    object l_PrevTagValue = cellContext.Cell.Model.ValueModel.GetTagValue(cellContext);
                    try
                    {
                        //前后不一样才给值
                        if (l_PrevTagValue != l_cancelEvent.NewValue)
                        {
                            cellContext.Cell.Model.ValueModel.SetTagValue(cellContext, l_cancelEvent.NewValue);
                        }
                    ...
                    {
                        OnEditException(new ExceptionEventArgs(err));
                        cellContext.Cell.Model.ValueModel.SetTagValue(cellContext, l_PrevTagValue);
                        l_cancelEvent.Cancel = true;//di fatto ?fallita la validazione del dato
                    }
                }
            ...
            if (EnableEdit)
            {
                ...
                object l_PrevValue = cellContext.Cell.Model.ValueModel.GetTagValue(cellContext);
                try
                {
                    cellContext.Cell.Model.ValueModel.SetTagValue(cellContext, p_NewTagValue);
                    return true;
                    //OnValidated(new CellContextEventArgs(cellContext));
                }
                catch (Exception err)
                {
                    OnEditException(new ExceptionEventArgs(err));
                    cellContext.Cell.Model.ValueModel.SetTagValue(cellContext, l_PrevValue);
                    return false;//cancel
                }
