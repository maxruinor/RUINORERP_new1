    ...
    public abstract class EditorControlBase : EditorBase
    {
        ...
        public override bool ApplyEdit()
        {
            ...
            if (IsEditing)
            {
                try
                {
                    object tagvalue = GetEditedTagValue();
                    //编辑结束就运行这里。 如果tag没有变化是不需要执行
                    SetCellTagValue(EditCellContext, tagvalue);
                    
                    bSuccess = SetCellValue(EditCellContext, tagvalue, GetEditedValue());
                    //by watson TODO
        ...
        //public override abstract object GetEditedText();
        ...
        /// </summary>
        /// <returns></returns>
        public override abstract object GetEditedTagValue();
        ...
        /// <summary>
        /// KeyPress事件在两种情况下触发：当Control.KeyPress活动被执行时，
        /// 以及当用户通过按键开始编辑单元格时（调用SendCharToEditor）
        ///将e.Handled属性设置为true以阻止事件
        /// </summary>
