    ...
    public abstract partial class PipeChannel<TPackageInfo> : ChannelBase<TPackageInfo>, IChannel<TPackageInfo>, IChannel, IPipeChannel
    {
        ...
        private bool ReaderBuffer(ref ReadOnlySequence<byte> buffer, out SequencePosition consumed, out SequencePosition examined)
        {
            ...
            while (true)
            {
                ...
                bytesConsumedTotal += bytesConsumed;//这里设置处理掉的数据长度，实际要多少。估计是来自管道协议定义？
                ...
                if (packageInfo == null)
                {
                    // the current pipeline filter needs more data to process当前管道过滤器需要更多数据来处理
                    if (!filterSwitched)
                    {
                        // set consumed  position and then continue to receive...设置消耗位置，然后继续接收...
                        consumed = buffer.GetPosition(bytesConsumedTotal);
                        return true;
                    }

                    // we should reset the previous pipeline filter after switch我们应该在切换后重置之前的管道过滤器
                    currentPipelineFilter.Reset();
                }
                else
                {
                    // reset the pipeline filter after we parse one full package在我们解析一个完整的包后重置管道过滤器
                    currentPipelineFilter.Reset();
