...
namespace WinLib.PushPanelEx.PushPanelEx
{
    /* 作者：Starts_2000
     * 日期：2010-01-09
    ...
    public class ImageDc : IDisposable
    {
        ...
        private void CreateImageDc(int width, int height, IntPtr hBmp)
        {
            ...
            pHdc = WinLib.PushPanelEx.Win32.NativeMethods.CreateDCA("DISPLAY", "", "", 0);
            _pHdc = WinLib.PushPanelEx.Win32.NativeMethods.CreateCompatibleDC(pHdc);
            if (hBmp != IntPtr.Zero)
            {
                _pBmp = hBmp;
            }
            ...
            {
                _pBmp = WinLib.PushPanelEx.Win32.NativeMethods.CreateCompatibleBitmap(pHdc, width, height);
            }
            _pBmpOld = WinLib.PushPanelEx.Win32.NativeMethods.SelectObject(_pHdc, _pBmp);
            ...
            WinLib.PushPanelEx.Win32.NativeMethods.DeleteDC(pHdc);
            pHdc = IntPtr.Zero;
        }
        ...
        private void ImageDestroy()
        {
            if (_pBmpOld != IntPtr.Zero)
            {
                WinLib.PushPanelEx.Win32.NativeMethods.SelectObject(_pHdc, _pBmpOld);
                _pBmpOld = IntPtr.Zero;
            }
            if (_pBmp != IntPtr.Zero)
            {
                WinLib.PushPanelEx.Win32.NativeMethods.DeleteObject(_pBmp);
                _pBmp = IntPtr.Zero;
            }
            if (_pHdc != IntPtr.Zero)
            {
                WinLib.PushPanelEx.Win32.NativeMethods.DeleteDC(_pHdc);
                _pHdc = IntPtr.Zero;
            }
        }
