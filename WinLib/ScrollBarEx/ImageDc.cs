...
namespace WinLib.ScrollBarEx
{
    ...
    public class ImageDc : IDisposable
    {
        ...
        private void CreateImageDc(int width, int height, IntPtr hBmp)
        {
            ...
            pHdc = WinLib.ScrollBarEx.Win32.NativeMethods.CreateDCA("DISPLAY", "", "", 0);
            _pHdc = WinLib.ScrollBarEx.Win32.NativeMethods.CreateCompatibleDC(pHdc);
            if (hBmp != IntPtr.Zero)
            {
                _pBmp = hBmp;
            }
            ...
            {
                _pBmp = WinLib.ScrollBarEx.Win32.NativeMethods.CreateCompatibleBitmap(pHdc, width, height);
            }
            _pBmpOld = WinLib.ScrollBarEx.Win32.NativeMethods.SelectObject(_pHdc, _pBmp);
            ...
            WinLib.ScrollBarEx.Win32.NativeMethods.DeleteDC(pHdc);
            pHdc = IntPtr.Zero;
        }
        ...
        private void ImageDestroy()
        {
            if (_pBmpOld != IntPtr.Zero)
            {
                WinLib.ScrollBarEx.Win32.NativeMethods.SelectObject(_pHdc, _pBmpOld);
                _pBmpOld = IntPtr.Zero;
            }
            if (_pBmp != IntPtr.Zero)
            {
                WinLib.ScrollBarEx.Win32.NativeMethods.DeleteObject(_pBmp);
                _pBmp = IntPtr.Zero;
            }
            if (_pHdc != IntPtr.Zero)
            {
                WinLib.ScrollBarEx.Win32.NativeMethods.DeleteDC(_pHdc);
                _pHdc = IntPtr.Zero;
            }
        }
