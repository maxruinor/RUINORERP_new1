...
namespace WinLib
{
    ...
    public class SkinForm : Form
    {
        ...
        private Point _TopLeftOffset = new Point(24, 32);

        [DefaultValue(typeof(Point), "24, 32")]
        public Point TopLeftOffset
        {
            get { return _TopLeftOffset; }
            set
            {
                if (_TopLeftOffset != value)
                {
                    _TopLeftOffset = value;
                };
            }
        }
        ...
        protected override CreateParams CreateParams
        {
            get
            {
                ...
                if (!DesignMode)
                {
                    cp.Style |= (int)SK.NativeMethods.WindowStyle.WS_THICKFRAME;

                    if (ControlBox)
                    {
                        cp.Style |= (int)SK.NativeMethods.WindowStyle.WS_SYSMENU;
                    }

                    if (MinimizeBox)
                    {
                        cp.Style |= (int)SK.NativeMethods.WindowStyle.WS_MINIMIZEBOX;
                    }

                    if (!MaximizeBox)
                    {
                        cp.Style &= ~(int)SK.NativeMethods.WindowStyle.WS_MAXIMIZEBOX;
                    }

                    if (_inPosChanged)
                    {
                        cp.Style &= ~((int)SK.NativeMethods.WindowStyle.WS_THICKFRAME |
                            (int)SK.NativeMethods.WindowStyle.WS_SYSMENU);
                        cp.ExStyle &= ~((int)SK.NativeMethods.WindowStyleEx.WS_EX_DLGMODALFRAME |
                            (int)SK.NativeMethods.WindowStyleEx.WS_EX_WINDOWEDGE);
                    }
                }
        ...
        protected override void WndProc(ref Message m)
        {
            switch (m.Msg)
            {
                case (int)SK.NativeMethods.WindowMessages.WM_NCHITTEST:
                    WmNcHitTest(ref m);
                    break;
                case (int)SK.NativeMethods.WindowMessages.WM_NCPAINT:
                case (int)SK.NativeMethods.WindowMessages.WM_NCCALCSIZE:
                    break;
                case (int)SK.NativeMethods.WindowMessages.WM_WINDOWPOSCHANGED:
                    _inPosChanged = true;
                    base.WndProc(ref m);
                    _inPosChanged = false;
                    break;
                case (int)SK.NativeMethods.WindowMessages.WM_GETMINMAXINFO:
                    WmGetMinMaxInfo(ref m);
                    break;
                case (int)SK.NativeMethods.WindowMessages.WM_NCACTIVATE:
                    WmNcActive(ref m);
                    break;
                default:
                    base.WndProc(ref m);
        ...
        private void WmNcHitTest(ref Message m)
        {
            ...
            Point point = new Point(
                SK.NativeMethods.LOWORD(wparam),
                SK.NativeMethods.HIWORD(wparam));
            point = PointToClient(point);

            if (IconRect.Contains(point))
            {
                m.Result = new IntPtr(
                    (int)SK.NativeMethods.NCHITTEST.HTSYSMENU);
                return;
            }
            ...
            if (_canResize)
            {
                ...
                {
                    m.Result = new IntPtr(
                        (int)SK.NativeMethods.NCHITTEST.HTTOPLEFT);
                    return;
                }
                ...
                {
                    m.Result = new IntPtr(
                        (int)SK.NativeMethods.NCHITTEST.HTTOPRIGHT);
                    return;
                }
                ...
                {
                    m.Result = new IntPtr(
                        (int)SK.NativeMethods.NCHITTEST.HTBOTTOMLEFT);
                    return;
                }
                ...
                {
                    m.Result = new IntPtr(
                        (int)SK.NativeMethods.NCHITTEST.HTBOTTOMRIGHT);
                    return;
                }
                ...
                {
                    m.Result = new IntPtr(
                        (int)SK.NativeMethods.NCHITTEST.HTTOP);
                    return;
                }
                ...
                {
                    m.Result = new IntPtr(
                        (int)SK.NativeMethods.NCHITTEST.HTBOTTOM);
                    return;
                }
                ...
                {
                    m.Result = new IntPtr(
                       (int)SK.NativeMethods.NCHITTEST.HTLEFT);
                    return;
                }
                ...
                {
                    m.Result = new IntPtr(
                       (int)SK.NativeMethods.NCHITTEST.HTRIGHT);
                    return;
                }
            }
            ...
            if (point.Y < CaptionHeight)
            {
                ...
                {
                    m.Result = new IntPtr(
                      (int)SK.NativeMethods.NCHITTEST.HTCAPTION);
                    return;
                }
            }
            m.Result = new IntPtr(
                     (int)SK.NativeMethods.NCHITTEST.HTCLIENT);
        }

        private void WmGetMinMaxInfo(ref Message m)
        {
            SK.NativeMethods.MINMAXINFO minmax =
                (SK.NativeMethods.MINMAXINFO)Marshal.PtrToStructure(
                m.LParam, typeof(SK.NativeMethods.MINMAXINFO));
        ...
        private void WmNcActive(ref Message m)
        {
            ...
            m.Result = SK.NativeMethods.TRUE;
            base.Invalidate();
        }
