...
using WinLib.SplitContainerExNS;
...
namespace WinLib
{
    ...
    public class SplitContainerEx : SplitContainer
    {
        ...
        private Color _customLineColor = Color.FromArgb(105, 200, 254); // 默认色

        public event PropertyChangedEventHandler PropertyChanged;

        [Category("Custom")]
        [Description("获取或设置自定义颜色")]
        public Color CustomLineColor
        {
            get => _customLineColor;

            set
            {
                if (_customLineColor != value)
                {
                    _customLineColor = value;

                    OnPropertyChanged(nameof(CustomLineColor));
                }
            }
        }
