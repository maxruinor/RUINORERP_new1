...
namespace WinLib
{
    ...
    public class TabControlEx : TabControl
    {
        ...
        {
            return ExTabControl.NativeMethods.FindWindowEx(
                base.Handle,
                IntPtr.Zero,
        ...
        private void DrawTabPages(Graphics g)
        {
            ...
            if (alignHorizontal)
            {
                ...
                if (hasUpDown)
                {
                    if (ExTabControl.NativeMethods.IsWindowVisible(upDownButtonHandle))
                    {
                        ExTabControl.NativeMethods.RECT upDownButtonRect = new ExTabControl.NativeMethods.RECT();
                        ExTabControl.NativeMethods.GetWindowRect(
        ...
        private class UpDownButtonNativeWindow : NativeWindow, IDisposable
        {
            ...
            private bool LeftKeyPressed()
            {
                ...
                {
                    return (ExTabControl.NativeMethods.GetKeyState(ExTabControl.NativeMethods.VK_RBUTTON) < 0);
                }
                else
                {
                    return (ExTabControl.NativeMethods.GetKeyState(ExTabControl.NativeMethods.VK_LBUTTON) < 0);
                }
            }
            ...
            private void DrawUpDownButton()
            {
                ...
                ExTabControl.NativeMethods.RECT rect = new ExTabControl.NativeMethods.RECT();

                ExTabControl.NativeMethods.GetClientRect(base.Handle, ref rect);

                Rectangle clipRect = Rectangle.FromLTRB(
                    rect.Top, rect.Left, rect.Right, rect.Bottom);

                Point cursorPoint = new Point();
                ExTabControl.NativeMethods.GetCursorPos(ref cursorPoint);
                ExTabControl.NativeMethods.GetWindowRect(base.Handle, ref rect);

                mouseOver = ExTabControl.NativeMethods.PtInRect(ref rect, cursorPoint);
            ...
            protected override void WndProc(ref Message m)
            {
                switch (m.Msg)
                {
                    case ExTabControl.NativeMethods.WM_PAINT:
                        if (!_bPainting)
                        {
                            ExTabControl.NativeMethods.PAINTSTRUCT ps = 
                                new ExTabControl.NativeMethods.PAINTSTRUCT();
                            _bPainting = true;
                            ExTabControl.NativeMethods.BeginPaint(m.HWnd, ref ps);
                            DrawUpDownButton();
                            ExTabControl.NativeMethods.EndPaint(m.HWnd, ref ps);
                            _bPainting = false;
                            m.Result = ExTabControl.NativeMethods.TRUE;
                        }
                        else
                        {
                            base.WndProc(ref m);
