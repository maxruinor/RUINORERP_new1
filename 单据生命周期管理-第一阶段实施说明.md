...
是基于开源的：.net 开源工作流引擎 Workflow Core的基础来编写代码。
...
具体的审核机制 要利用目前 业务层的单据中的审核方法。审核过程是一个很复杂的过程。会关系很多数据的处理。并不是下面这样简单的修改一个字段属性而已。所以你需要进一步去理解tb_SaleOrderControllerPartial.cs 如销售订单审核等。很多业务的审核。签名都是一样的。 再修改方案。
