# 图片管理优化方案 - 完整实现指南

## 1. 方案概述

针对销售订单等业务单据中的图片管理,实现完整的图片生命周期管理,包括:

- ✅ 图片上传/下载优化
- ✅ 图片更新与版本管理
- ✅ 过期文件自动识别与清理
- ✅ 孤立文件检测与处理
- ✅ 业务关联关系维护

## 2. 核心架构设计

### 2.1 文件状态管理

```
文件状态 (tb_FS_FileStorageInfo.Status):
├── 0 = Active (正常) - 文件有效且有业务关联
├── 1 = Expired (过期) - 超过ExpireTime
├── 2 = Orphaned (孤立) - 无业务关联
└── 3 = Deleted (已删除) - 逻辑删除
```

### 2.2 图片更新策略

```csharp
/// <summary>
/// 三种图片更新策略
/// </summary>
public enum UpdateStrategy
{
    /// <summary>
    /// 仅新增 - 保留旧文件,创建新关联(默认)
    /// 适用场景: 需要保留历史记录
    /// </summary>
    AppendOnly = 0,

    /// <summary>
    /// 替换模式 - 保留历史版本,更新关联到新文件
    /// 适用场景: 需要历史追溯但只显示最新版本
    /// </summary>
    Replace = 1,

    /// <summary>
    /// 覆盖模式 - 删除旧文件,创建新文件
    /// 适用场景: 不需要历史记录
    /// </summary>
    Overwrite = 2
}
```

## 3. 服务端实现

### 3.1 依赖注入配置

**已添加的DI注册:**

```csharp
// 服务端 DI配置: RUINORERP.Server/Network/DI/NetworkServicesDependencyInjection.cs

// 在 AddNetworkServices 方法中添加:
services.AddTransient<FileUpdateService>();
services.AddTransient<FileCleanupService>();

// 在 ConfigureNetworkServicesContainer 方法中添加:
builder.RegisterType<FileUpdateService>().AsSelf().InstancePerDependency();
builder.RegisterType<FileCleanupService>().AsSelf().InstancePerDependency();

// FileCommandHandler 通过构造函数注入:
public FileCommandHandler(
    FileUpdateService fileUpdateService,
    FileCleanupService fileCleanupService,
    // ...其他参数
)
{
    _fileUpdateService = fileUpdateService ?? throw new ArgumentNullException(nameof(fileUpdateService));
    _fileCleanupService = fileCleanupService ?? throw new ArgumentNullException(nameof(fileCleanupService));
}
```

**FileStorageHelper 扩展:**

已添加`GetStoragePath()`方法,用于获取解析后的存储根路径:

```csharp
// RUINORERP.Server/Helpers/FileStorageHelper.cs
public static string GetStoragePath()
{
    if (_serverConfig == null || string.IsNullOrEmpty(_serverConfig.FileStoragePath))
        return string.Empty;

    return ResolveEnvironmentVariables(_serverConfig.FileStoragePath);
}
```

### 3.2 FileCleanupService - 文件清理服务

**功能职责:**
- 自动清理过期文件(每天凌晨2点)
- 识别并清理孤立文件
- 清理物理目录中的孤立文件
- 提供手动清理接口
- 统计文件存储使用情况

**关键方法:**

```csharp
// 清理过期文件
await cleanupService.CleanupExpiredFilesAsync(
    daysThreshold: 30,      // 过期天数阈值
    physicalDelete: false   // 是否物理删除
);

// 清理孤立文件
await cleanupService.CleanupOrphanedFilesAsync(
    daysThreshold: 7,      // 孤立天数阈值
    physicalDelete: false   // 是否物理删除
);

// 清理物理目录中的孤立文件
await cleanupService.CleanupPhysicalOrphanedFilesAsync();

// 获取文件统计信息
var stats = await cleanupService.GetCleanupStatisticsAsync();
Console.WriteLine($"总文件数: {stats.TotalFiles}");
Console.WriteLine($"过期文件: {stats.ExpiredFiles}");
Console.WriteLine($"孤立文件: {stats.OrphanedFiles}");
Console.WriteLine($"总存储: {stats.TotalStorageSizeFormatted}");
```

**清理策略:**
1. **过期文件** - 超过ExpireTime的文件
   - 逻辑删除(默认): 标记Status=1,保留物理文件
   - 物理删除: 检查无业务关联后删除物理文件和记录

2. **孤立文件** - 无业务关联且超过阈值的文件
   - 逻辑删除(默认): 标记Status=2
   - 物理删除: 直接删除

3. **物理孤立文件** - 数据库中不存在但物理目录存在的文件
   - 超过30天未访问自动删除
   - 跳过系统文件(.gitkeep, .DS_Store等)

### 3.2 FileUpdateService - 文件更新服务

**功能职责:**
- 处理图片的替换和更新
- 维护业务关联关系
- 支持版本历史记录
- 提供批量更新功能

**更新流程:**

```csharp
// 单个文件更新
var (newFileInfo, newRelation, oldFiles) = await updateService.UpdateBusinessFileAsync(
    businessType: (int)BizType.销售订单,
    businessNo: "SO20250119001",
    businessId: 1001,
    relatedField: "VoucherImage",
    newFileData: imageData,
    newFileName: "凭证图片.jpg",
    strategy: FileUpdateService.UpdateStrategy.Replace,  // 更新策略
    userId: currentUser.UserId,
    isDetailTable: false,
    detailId: null
);

// 批量文件更新
var result = await updateService.BatchUpdateBusinessFilesAsync(
    businessType: (int)BizType.销售订单,
    businessNo: "SO20250119001",
    businessId: 1001,
    relatedField: "VoucherImage",
    newFiles: new List<(byte[], string)> {
        (imageData1, "凭证1.jpg"),
        (imageData2, "凭证2.jpg")
    },
    strategy: FileUpdateService.UpdateStrategy.Replace
);

Console.WriteLine($"成功: {result.SuccessFiles.Count}, 失败: {result.FailedFiles.Count}");
```

**策略详解:**

#### 策略1: AppendOnly (仅新增)
```sql
-- 保留所有文件和关联关系
-- 不删除任何旧数据
-- 仅添加新的关联记录
INSERT INTO tb_FS_BusinessRelation (...)

优点: 完整的历史记录
缺点: 存储空间占用大
适用: 需要审计追踪的场景
```

#### 策略2: Replace (替换)
```sql
-- 标记旧关联为不活跃
UPDATE tb_FS_BusinessRelation 
SET IsActive = 0 
WHERE BusinessId = ? AND RelatedField = ?

-- 创建新的活跃关联
INSERT INTO tb_FS_BusinessRelation (..., IsActive = 1)

优点: 保留历史但查询最新版本高效
缺点: 仍占用存储空间
适用: 需要历史追溯但主要使用最新版本
```

#### 策略3: Overwrite (覆盖)
```sql
-- 删除旧关联
DELETE FROM tb_FS_BusinessRelation WHERE ...

-- 逻辑删除旧文件
UPDATE tb_FS_FileStorageInfo SET isdeleted = 1 WHERE ...

-- 删除物理文件
DELETE FROM disk WHERE FileId = ...

-- 创建新文件和关联
INSERT INTO tb_FS_FileStorageInfo (...)
INSERT INTO tb_FS_BusinessRelation (...)

优点: 节省存储空间
缺点: 丢失历史记录
适用: 不需要历史记录的场景
```

### 3.3 FileCommandHandler 集成

已在FileCommandHandler中集成了FileUpdateService和FileCleanupService:

```csharp
public class FileCommandHandler : BaseCommandHandler
{
    private readonly FileUpdateService _fileUpdateService;
    private readonly FileCleanupService _fileCleanupService;

    // 支持的命令扩展
    SetSupportedCommands(
        FileCommands.FileUpload,
        FileCommands.FileDownload,
        FileCommands.FileDelete,
        // ...
        // 未来可扩展:
        // FileCommands.FileUpdate,
        // FileCommands.FileCleanup,
        // FileCommands.FileVersionHistory
    );
}
```

## 4. 客户端实现

### 4.1 客户端DI配置

**已添加的DI注册:**

```csharp
// 客户端 DI配置: RUINORERP.UI/Network/DI/NetworkServicesDependencyInjection.cs

// 在 ConfigureNetworkServicesContainer 方法中添加:
builder.RegisterType<FileUpdateClientService>()
    .AsSelf()
    .InstancePerLifetimeScope()
    .PropertiesAutowired();

// FileManagementService 已在前面注册:
builder.RegisterType<FileManagementService>()
    .AsSelf()
    .InstancePerLifetimeScope()
    .PropertiesAutowired();
```

**DI注入说明:**

`FileUpdateClientService`通过构造函数注入依赖:

```csharp
public sealed class FileUpdateClientService : IDisposable
{
    private readonly ClientCommunicationService _communicationService;
    private readonly FileManagementService _fileManagementService;
    private readonly ILogger<FileUpdateClientService> _logger;

    public FileUpdateClientService(
        ClientCommunicationService communicationService,
        FileManagementService fileManagementService,
        ILogger<FileUpdateClientService> logger = null)
    {
        _communicationService = communicationService;
        _fileManagementService = fileManagementService;
        _logger = logger;
    }
}
```

**使用方式:**

```csharp
// 在窗体或服务中通过DI注入
public class UCSaleOrder : BaseBillEditGeneric
{
    private readonly FileUpdateClientService _fileUpdateService;

    public UCSaleOrder(FileUpdateClientService fileUpdateService)
    {
        _fileUpdateService = fileUpdateService;
    }
}

// 或者通过Startup获取服务
var fileUpdateService = Startup.GetFromFac<FileUpdateClientService>();
```

### 4.2 FileUpdateClientService - 文件更新客户端服务

**功能职责:**
- 提供简单的API调用服务端更新功能
- 支持单个和批量文件更新
- 提供更新策略选择
- 异常处理和日志记录

**使用示例:**

```csharp
// 获取服务
var updateService = Startup.GetFromFac<FileUpdateClientService>();

// 更新单个图片
var result = await updateService.UpdateBusinessFileAsync(
    businessType: (int)BizType.销售订单,
    businessNo: saleOrder.SOrderNo,
    businessId: saleOrder.PrimaryKeyID,
    relatedField: "VoucherImage",
    newFileData: File.ReadAllBytes(@"C:\新凭证.jpg"),
    newFileName: "新凭证.jpg",
    strategy: FileUpdateStrategy.Replace
);

if (result.IsSuccess)
{
    Console.WriteLine($"文件更新成功,新文件ID: {result.NewFileId}");
}
else
{
    Console.WriteLine($"文件更新失败: {result.Message}");
}

// 批量更新图片
var batchResult = await updateService.BatchUpdateBusinessFilesAsync(
    businessType: (int)BizType.销售订单,
    businessNo: saleOrder.SOrderNo,
    businessId: saleOrder.PrimaryKeyID,
    relatedField: "PaymentImagePath",
    newFiles: new List<(byte[], string)> {
        (File.ReadAllBytes(@"C:\凭证1.jpg"), "凭证1.jpg"),
        (File.ReadAllBytes(@"C:\凭证2.jpg"), "凭证2.jpg")
    },
    strategy: FileUpdateStrategy.AppendOnly
);
```

### 4.2 FileManagementController 扩展

在FileManagementController中添加更新方法:

```csharp
/// <summary>
/// 更新业务单据的图片
/// </summary>
public async Task<FileUploadResponse> UpdateBusinessImageAsync(
    BaseEntity entity,
    string relatedField,
    byte[] newImageData,
    string newImageName,
    FileUpdateStrategy strategy = FileUpdateStrategy.Replace,
    long? oldFileId = null)
{
    // 参数验证
    if (entity == null)
        throw new ArgumentNullException(nameof(entity));

    if (newImageData == null || newImageData.Length == 0)
        throw new ArgumentException("图片数据不能为空");

    try
    {
        // 获取实体信息
        var entityInfo = _mapper.GetEntityInfo(entity.GetType());
        if (entityInfo == null)
            throw new ArgumentException("无效的业务实体类型");

        string businessNo = entity.GetPropertyValue<string>(entityInfo.NoField).ToString();
        int businessType = (int)entityInfo.BizType;

        // 获取文件更新服务
        var updateService = _appContext.GetRequiredService<FileUpdateClientService>();

        // 执行更新
        var updateResult = await updateService.UpdateBusinessFileAsync(
            businessType: businessType,
            businessNo: businessNo,
            businessId: entity.PrimaryKeyID,
            relatedField: relatedField,
            newFileData: newImageData,
            newFileName: newImageName,
            strategy: strategy
        );

        if (updateResult.IsSuccess)
        {
            // 返回标准响应格式
            var response = new FileUploadResponse();
            response.IsSuccess = true;
            response.Message = updateResult.Message;

            // 查询新文件信息并添加到响应
            var fileStorageInfo = new tb_FS_FileStorageInfo
            {
                FileId = updateResult.NewFileId,
                OriginalFileName = updateResult.NewFileName,
                FileSize = updateResult.NewFileSize
            };
            response.FileStorageInfos.Add(fileStorageInfo);

            return response;
        }
        else
        {
            return ResponseFactory.CreateSpecificErrorResponse<FileUploadResponse>(updateResult.Message);
        }
    }
    catch (Exception ex)
    {
        _logger.LogError(ex, "更新业务图片失败");
        return ResponseFactory.CreateSpecificErrorResponse<FileUploadResponse>($"更新业务图片失败: {ex.Message}");
    }
}
```

## 5. 业务单据集成示例

### 5.1 在销售订单中使用

```csharp
// UCSaleOrder.cs 中使用
public partial class UCSaleOrder : BaseBillEditGeneric<tb_SaleOrder, tb_SaleOrderDetail>
{
    // 获取服务
    private readonly FileManagementController _fileController;
    private readonly FileUpdateClientService _fileUpdateService;

    /// <summary>
    /// 更新销售订单凭证图片
    /// </summary>
    private async Task UpdateVoucherImageAsync(string imagePath)
    {
        if (!File.Exists(imagePath))
        {
            MessageBox.Show("文件不存在", "提示");
            return;
        }

        // 选择更新策略
        var strategy = FileUpdateStrategy.Replace; // 替换模式,保留历史

        // 读取图片数据
        var imageData = File.ReadAllBytes(imagePath);
        var fileName = Path.GetFileName(imagePath);

        // 更新图片
        var result = await _fileUpdateService.UpdateBusinessFileAsync(
            businessType: (int)BizType.销售订单,
            businessNo: CurrentEntity.SOrderNo,
            businessId: CurrentEntity.PrimaryKeyID,
            relatedField: "VoucherImage",
            newFileData: imageData,
            newFileName: fileName,
            strategy: strategy
        );

        if (result.IsSuccess)
        {
            MessageBox.Show("凭证图片更新成功", "提示");
            // 刷新显示
            await LoadVoucherImagesAsync();
        }
        else
        {
            MessageBox.Show($"凭证图片更新失败: {result.Message}", "错误");
        }
    }

    /// <summary>
    /// 批量更新付款凭证图片
    /// </summary>
    private async Task BatchUpdatePaymentImagesAsync(List<string> imagePaths)
    {
        var files = new List<(byte[], string)>();
        foreach (var path in imagePaths)
        {
            if (File.Exists(path))
            {
                files.Add((File.ReadAllBytes(path), Path.GetFileName(path)));
            }
        }

        if (files.Count == 0)
        {
            MessageBox.Show("没有有效的图片文件", "提示");
            return;
        }

        var result = await _fileUpdateService.BatchUpdateBusinessFilesAsync(
            businessType: (int)BizType.销售订单,
            businessNo: CurrentEntity.SOrderNo,
            businessId: CurrentEntity.PrimaryKeyID,
            relatedField: "PaymentImagePath",
            newFiles: files,
            strategy: FileUpdateStrategy.AppendOnly // 保留所有历史图片
        );

        if (result.IsSuccess)
        {
            MessageBox.Show($"成功更新 {result.SuccessFiles.Count} 张图片", "提示");
            await LoadPaymentImagesAsync();
        }
        else
        {
            MessageBox.Show($"部分图片更新失败,成功:{result.SuccessFiles.Count}, 失败:{result.FailedFiles.Count}", "警告");
        }
    }
}
```

### 5.2 在BaseBillEditGeneric中通用集成

```csharp
// BaseBillEditGeneric.cs 中添加通用方法

/// <summary>
/// 更新业务单据图片的通用方法
/// </summary>
/// <param name="relatedField">关联字段名</param>
/// <param name="imagePath">图片路径</param>
/// <param name="strategy">更新策略</param>
/// <returns>是否更新成功</returns>
protected async Task<bool> UpdateBillImageAsync(
    string relatedField,
    string imagePath,
    FileUpdateStrategy strategy = FileUpdateStrategy.Replace)
{
    if (CurrentEntity == null)
    {
        MainForm.Instance?.ShowStatusText("当前没有加载的业务单据");
        return false;
    }

    if (!File.Exists(imagePath))
    {
        MainForm.Instance?.ShowStatusText("图片文件不存在");
        return false;
    }

    try
    {
        var fileService = Startup.GetFromFac<FileUpdateClientService>();
        
        var entityInfo = _mapper.GetEntityInfo(CurrentEntity.GetType());
        var businessNo = CurrentEntity.GetPropertyValue<string>(entityInfo.NoField).ToString();
        var businessType = (int)entityInfo.BizType;

        var imageData = File.ReadAllBytes(imagePath);
        var fileName = Path.GetFileName(imagePath);

        var result = await fileService.UpdateBusinessFileAsync(
            businessType: businessType,
            businessNo: businessNo,
            businessId: CurrentEntity.PrimaryKeyID,
            relatedField: relatedField,
            newFileData: imageData,
            newFileName: fileName,
            strategy: strategy
        );

        if (result.IsSuccess)
        {
            MainForm.Instance?.ShowStatusText("图片更新成功");
            return true;
        }
        else
        {
            MainForm.Instance?.ShowStatusText($"图片更新失败: {result.Message}");
            return false;
        }
    }
    catch (Exception ex)
    {
        MainForm.Instance?.logger?.LogError(ex, "更新业务图片失败");
        MainForm.Instance?.ShowStatusText($"图片更新失败: {ex.Message}");
        return false;
    }
}
```

## 6. 定时任务配置

### 6.1 启用自动清理

FileCleanupService在启动时会自动注册定时任务,无需额外配置:

```csharp
// Startup.cs 或 Program.cs 中注册服务
services.AddSingleton<FileCleanupService>();
services.AddSingleton<FileUpdateService>();
```

清理时间: 每天凌晨2:00执行

### 6.2 手动触发清理

```csharp
// 在管理界面或定时任务中手动触发
var cleanupService = Startup.GetFromFac<FileCleanupService>();

// 清理过期文件
var expiredCount = await cleanupService.CleanupExpiredFilesAsync(
    daysThreshold: 30,
    physicalDelete: false
);

// 清理孤立文件
var orphanedCount = await cleanupService.CleanupOrphanedFilesAsync(
    daysThreshold: 7,
    physicalDelete: false
);

// 清理物理孤立文件
var physicalCount = await cleanupService.CleanupPhysicalOrphanedFilesAsync();

Console.WriteLine($"清理完成 - 过期:{expiredCount}, 孤立:{orphanedCount}, 物理:{physicalCount}");
```

## 7. 配置选项

### 7.1 服务器配置

```json
{
  "ServerGlobalConfig": {
    "FileStoragePath": "D:\\FileStorage",
    "EnableAutoCleanup": true,
    "ExpiredFileDaysThreshold": 30,
    "OrphanedFileDaysThreshold": 7,
    "PhysicalOrphanedFileDaysThreshold": 30,
    "MaxStorageSizeGB": 100
  }
}
```

### 7.2 更新策略配置

可以根据业务类型配置默认的更新策略:

```csharp
public class FileUpdateConfig
{
    /// <summary>
    /// 各业务类型的默认更新策略
    /// </summary>
    public Dictionary<BizType, FileUpdateStrategy> DefaultStrategies { get; set; } =
        new Dictionary<BizType, FileUpdateStrategy>
        {
            // 销售订单 - 默认使用替换模式,保留历史
            [BizType.销售订单] = FileUpdateStrategy.Replace,
            
            // 付款单 - 默认使用仅新增模式,保留所有凭证
            [BizType.付款单] = FileUpdateStrategy.AppendOnly,
            
            // 产品图片 - 默认使用覆盖模式,节省空间
            [BizType.产品信息] = FileUpdateStrategy.Overwrite
        };
}
```

## 8. 监控与统计

### 8.1 获取存储统计信息

```csharp
var cleanupService = Startup.GetFromFac<FileCleanupService>();
var stats = await cleanupService.GetCleanupStatisticsAsync();

// 显示统计信息
Console.WriteLine($"=== 文件存储统计 ===");
Console.WriteLine($"总文件数: {stats.TotalFiles}");
Console.WriteLine($"正常文件: {stats.ActiveFiles}");
Console.WriteLine($"过期文件: {stats.ExpiredFiles}");
Console.WriteLine($"孤立文件: {stats.OrphanedFiles}");
Console.WriteLine($"总存储: {stats.TotalStorageSizeFormatted}");
```

### 8.2 存储空间预警

```csharp
// 检查存储空间使用率
var stats = await cleanupService.GetCleanupStatisticsAsync();
var maxStorageBytes = _serverConfig.MaxStorageSizeGB * 1024L * 1024 * 1024;
var usagePercentage = (double)stats.TotalStorageSize / maxStorageBytes * 100;

if (usagePercentage > 90)
{
    _logger.LogWarning("存储空间使用率超过90%,建议清理过期文件");
    // 发送告警通知
}

if (usagePercentage > 95)
{
    _logger.LogError("存储空间使用率超过95%,紧急清理");
    // 自动触发清理
    await cleanupService.CleanupExpiredFilesAsync(daysThreshold: 7, physicalDelete: true);
}
```

## 9. 常见问题解答

### Q1: 图片更新后旧文件会立即删除吗?

A: 取决于使用的更新策略:
- **AppendOnly**: 旧文件保留,不删除
- **Replace**: 旧文件保留,仅标记关联为不活跃
- **Overwrite**: 旧文件物理删除

### Q2: 过期文件什么时候会被清理?

A: FileCleanupService每天凌晨2:00自动清理,也可手动触发清理:
- 默认过期阈值: 30天
- 可配置: `ExpiredFileDaysThreshold`

### Q3: 如何恢复被删除的文件?

A: 如果是逻辑删除(Status=1/2/3),可以通过FileCleanupService.RestoreFilesAsync恢复:
```csharp
await cleanupService.RestoreFilesAsync(new List<long> { fileId1, fileId2 });
```

如果是物理删除,则无法恢复,建议定期备份。

### Q4: 孤立文件是如何识别的?

A: 孤立文件识别条件:
1. 数据库中存在记录(tb_FS_FileStorageInfo)
2. 但没有业务关联记录(tb_FS_BusinessRelation)
3. 且创建时间超过阈值(默认7天)

### Q5: 如何查看文件的版本历史?

A: 使用FileUpdateService.GetFileVersionHistoryAsync:
```csharp
var history = await updateService.GetFileVersionHistoryAsync(
    businessType: (int)BizType.销售订单,
    businessId: saleOrder.PrimaryKeyID,
    relatedField: "VoucherImage"
);

foreach (var version in history)
{
    Console.WriteLine($"版本: {version.VersionNo}, 文件: {version.FileName}, 活跃: {version.IsActive}");
}
```

## 10. 最佳实践建议

### 10.1 更新策略选择

| 业务类型 | 推荐策略 | 原因 |
|---------|---------|------|
| 销售订单凭证 | Replace | 需要历史追溯,主要使用最新版本 |
| 付款单凭证 | AppendOnly | 法律要求保留所有原始凭证 |
| 产品图片 | Overwrite | 不需要历史,节省存储空间 |
| 合同附件 | Replace | 合同可能有修改版本 |
| 其他单据 | AppendOnly | 保守选择,保留所有记录 |

### 10.2 性能优化建议

1. **批量操作**: 尽量使用批量上传/更新接口
2. **文件压缩**: 上传前压缩大图片
3. **异步处理**: 所有文件操作使用异步方法
4. **缓存机制**: 频繁访问的文件可考虑缓存

### 10.3 安全建议

1. **权限验证**: 所有文件操作需要权限检查
2. **文件大小限制**: 限制单个文件最大10MB
3. **文件类型验证**: 只允许特定格式(jpg,png,pdf等)
4. **定期备份**: 重要文件定期备份

## 11. 实施步骤

### 阶段1: 基础功能实现(已完成 ✅)
- ✅ FileCleanupService实现
- ✅ FileUpdateService实现
- ✅ FileUpdateClientService实现
- ✅ FileCleanupWorkflow工作流实现
- ✅ FileStorageMonitorService监控服务实现
- ✅ DI配置完成
- ✅ 定时任务调度配置

### 阶段2: 集成与测试(已完成 ✅)
- ✅ FileCommandHandler中集成新服务
- ✅ BaseBillEditGeneric添加通用文件更新方法
- ✅ 工作流注册和调度
- ✅ 服务端DI注册
- ✅ 客户端DI注册

### 阶段3: 测试验证(待实施 ⏳)
- ⏳ 单元测试编写
- ⏳ 三种更新策略测试
- ⏳ 清理功能测试
- ⏳ 并发场景测试

### 阶段4: 部署与优化(待实施 ⏳)
- ⏳ 部署到测试环境
- ⏳ 性能测试与优化
- ⏳ 生产环境部署
- ⏳ 监控与维护

## 12. 已完成的工作清单

### 12.1 服务端实现

#### 文件创建:
1. **FileCleanupService.cs** - 文件清理服务
   - 过期文件清理
   - 孤立文件清理
   - 物理孤立文件清理
   - 统计信息获取

2. **FileUpdateService.cs** - 文件更新服务
   - 三种更新策略(AppendOnly, Replace, Overwrite)
   - 单个和批量文件更新
   - 版本历史管理

3. **FileCleanupWorkflow.cs** - 文件清理工作流
   - 工作流步骤定义
   - 每天凌晨2点自动执行
   - 清理结果统计

4. **FileStorageMonitorService.cs** - 文件存储监控服务
   - 存储空间监控
   - 预警阈值检测
   - 自动清理触发

#### DI配置:
5. **NetworkServicesDependencyInjection.cs**
   - 注册FileUpdateService
   - 注册FileCleanupService
   - 注册FileStorageMonitorService

6. **Startup.cs**
   - 注册FileCleanupWorkflow

7. **frmMainNew.cs**
   - 调度FileCleanupWorkflow

8. **FileStorageHelper.cs**
   - 添加GetStoragePath()方法

### 12.2 客户端实现

#### 文件创建:
1. **FileUpdateClientService.cs** - 客户端文件更新服务
   - 通过DI注入依赖
   - 异常处理和日志记录

#### DI配置:
2. **NetworkServicesDependencyInjection.cs**
   - 注册FileUpdateClientService
   - 注册FileManagementService

3. **BaseBillEditGeneric.cs**
   - UpdateBillImageAsync() - 单个图片更新
   - BatchUpdateBillImagesAsync() - 批量图片更新
   - UpdateBillImageViaDialogAsync() - 通过对话框更新

### 12.3 使用示例

#### 服务端自动清理:
```csharp
// 每天凌晨2点自动执行,无需手动干预
// FileCleanupWorkflow已自动调度

// 手动触发清理:
var cleanupService = Startup.GetFromFac<FileCleanupService>();
await cleanupService.CleanupExpiredFilesAsync(daysThreshold: 30);
await cleanupService.CleanupOrphanedFilesAsync(daysThreshold: 7);
```

#### 客户端更新图片:
```csharp
// 在任意继承BaseBillEditGeneric的窗体中:

// 方式1: 更新单个图片
await UpdateBillImageAsync(
    relatedField: "VoucherImage",
    imagePath: @"C:\凭证.jpg",
    strategy: FileUpdateStrategy.Replace
);

// 方式2: 批量更新图片
var result = await BatchUpdateBillImagesAsync(
    relatedField: "PaymentImagePath",
    imagePaths: new List<string> { @"C:\凭证1.jpg", @"C:\凭证2.jpg" },
    strategy: FileUpdateStrategy.AppendOnly
);

// 方式3: 通过文件选择器
await UpdateBillImageViaDialogAsync(
    relatedField: "VoucherImage",
    strategy: FileUpdateStrategy.Replace,
    multiSelect: true
);
```

#### 存储空间监控:
```csharp
// 启动监控(通常在Startup中自动启动)
var monitorService = Startup.GetFromFac<FileStorageMonitorService>();
monitorService.StartMonitoring();

// 获取监控信息
var monitorInfo = await monitorService.GetMonitorInfoAsync();
Console.WriteLine($"磁盘使用率: {monitorInfo.DiskUsagePercentage:F2}%");
Console.WriteLine($"文件存储使用率: {monitorInfo.FileStorageUsagePercentage:F2}%");
Console.WriteLine($"总文件数: {monitorInfo.TotalFiles}");
Console.WriteLine($"过期文件: {monitorInfo.ExpiredFiles}");
Console.WriteLine($"孤立文件: {monitorInfo.OrphanedFiles}");
```

## 12. 总结

本优化方案提供了完整的图片生命周期管理:

1. **灵活的更新策略**: 支持三种策略,满足不同业务需求
2. **自动清理机制**: 定时清理过期和孤立文件,节省存储空间
3. **完整的版本管理**: 保留文件版本历史,支持追溯
4. **简单的API设计**: 客户端调用简单,易于集成
5. **可配置性**: 支持灵活配置,适应不同场景

**关键优势:**
- 🚀 提升存储空间利用率
- 📊 提供完整的文件统计和监控
- 🔒 保证数据一致性和可追溯性
- 💾 支持多种更新策略,灵活适应业务需求
- ♻️ 自动清理过期和孤立文件,减少人工干预
